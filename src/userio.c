...
#define FILENAME_LIMIT 24

int vprintmsg(const char *txt, va_list args, int wait)
{
    char buffer[10240];
    vsprintf(buffer, txt, args);
    return w_okcancel(buffer, wait ? TXT_OK : NULL,
		      wait > 1 ? TXT_CANCEL : NULL, NULL);
}
	     ...
	     float *input)
{
    ...
    bs.max_length = length;
    bs.str = str;
    bs.allowed_char = isfloat;
    bs.l_char_entered = bs.r_char_entered = bs.l_string_entered =
	bs.r_string_entered = NULL;
    checkmem(b =
	     w_addstdbutton(NULL, w_b_string, 0, 0, -1, -1, txt, &bs, 0));
    w.xpos = w.ypos = -1;
    ...
    w.refresh = wr_normal;
    w.hotkey = 0;
    w.refresh_routine = w.close_routine = NULL;
...
void uio_drawpath(struct w_window *w, int x, int y, int xsize)
{
    ...
    p = buffer;
    while (strlen(p) > 0 && ws_pixstrlen(p) > xsize)
	p++;
    if (strlen(p) >= 1) {
	if (p != buffer)
	    p[0] = p[1] = p[2] = '.';
...
void uio_files_one_up(struct w_button *b)
{
    uio_updown(b->data, 5, -1);
}

void uio_files_one_down(struct w_button *b)
{
    uio_updown(b->data, 5, 1);
}

void uio_files_page_up(struct w_button *b)
{
    uio_updown(b->data, 5, -2);
}

void uio_files_page_down(struct w_button *b)
{
    uio_updown(b->data, 5, 2);
}

void uio_dirs_one_up(struct w_button *b)
{
    uio_updown(b->data, 2, -1);
}

void uio_dirs_one_down(struct w_button *b)
{
    uio_updown(b->data, 2, 1);
}

void uio_dirs_page_up(struct w_button *b)
{
    uio_updown(b->data, 2, -2);
}

void uio_dirs_page_down(struct w_button *b)
{
    uio_updown(b->data, 2, 2);
}
...
void uio_changedir(struct w_button **b, char *oldpath)
{
    ...
    int i, no, dno;
    char **files, **dirs, *t;
    uio_drawpath(b[0]->w, b[0]->xpos, b[0]->ypos + b[0]->ysize + 1,
		 b[0]->xsize);
    ...
    FREE(b_files->strings);
    FREE(b_dirs->strings);
    b_files->no_strings = no;
    ...
    if (i < bd->no_strings) {
		...
		for (i = 0, num_sel = 0; i < bd->no_strings; i++) {
			...
			if (bd->max_selected == 1) {
				strcpy(b_arr[0]->d.str->str, bd->strings[i]);
				break;
			}
		}
...
/*	
	if (!strcmp(b_dirs->strings[sel], "..")) {
		do {
			current_path[strlen(current_path) - 1] = 0;
		} while (current_path[strlen(current_path) - 1] != '/');
		//current_path[strlen(current_path) - 1] = 0;
	} else {
*/
...
int makefilebuttons(struct w_window *w, int x, int y, int ysize,
		    char **files, int nofiles, char **dirs, int nodirs,
		    const char *defname, struct w_button **b)
{
    ...
    int i, no_rows, xsize;
    if (files)
	qsort(files, nofiles, sizeof(char *), qs_compstrs);
    if (dirs)
    ...
    if (defname)
	strncpy(b_name->str, defname, b_name->max_length);
    else
	b_name->str[0] = 0;
    b_name->allowed_char = isgraph;
    b_name->l_char_entered = b_name->r_char_entered = NULL;
    b_name->l_string_entered = b_name->r_string_entered =
	uio_string_getfname;
    checkmem(b_fnames = MALLOC(sizeof(struct w_b_strlist)));
    b_fnames->no_strings = nofiles;
    b_fnames->strings = files;
    checkmem(b_fnames->sel_arr = MALLOC(nofiles));
    for (i = 0; i < nofiles; i++)
	b_fnames->sel_arr[i] = 0;
    b_fnames->no_rows = no_rows;
    b_fnames->l_string_selected = b_fnames->r_string_selected =
	uio_sel_fname;
    ...
    if (defname) {
	for (i = 0; i < nofiles; i++)
	    if (compstrs(files[i], defname) >= 0)
		break;
	if (i < nofiles && compstrs(files[i], defname) == 0)
	    b_fnames->sel_arr[i] = 1;
    ...
    *b_down2 = *b_down;
    ...
    xsize = 20 + ws_pixstrlen("M") * FILENAME_LIMIT * 2;
    path_max_length = xsize - 6;
    checkmem(b[0] =
	     w_addstdbutton(w, w_b_string, x, y, xsize, -1, NULL, b_name,
			    1));
    checkmem(b[1] =
	     w_addstdbutton(w, w_b_press, x,
			    b[0]->ypos + b[0]->ysize + w_titlebarheight() +
			    2, xsize / 2, -1, "/\\", b_up2, 1));
    checkmem(b[4] =
	     w_addstdbutton(w, w_b_press, x + xsize / 2, b[1]->ypos,
			    xsize / 2, -1, "/\\", b_up, 1));
    checkmem(b[2] =
	     w_addstdbutton(w, w_b_list, x, b[1]->ypos + b[1]->ysize,
			    xsize / 2, no_rows * (w_titlebarheight() + 2),
			    NULL, b_dirs, 1));
    checkmem(b[5] =
	     w_addstdbutton(w, w_b_list, x + xsize / 2,
			    b[1]->ypos + b[1]->ysize, xsize / 2,
			    no_rows * (w_titlebarheight() + 2), NULL,
			    b_fnames, 1));
    checkmem(b[3] =
	     w_addstdbutton(w, w_b_press, x, b[2]->ypos + b[2]->ysize,
			    xsize / 2, -1, "\\/", b_down2, 1));
    checkmem(b[6] =
	     w_addstdbutton(w, w_b_press, x + xsize / 2,
			    b[2]->ypos + b[2]->ysize, xsize / 2, -1, "\\/",
			    b_down, 1));
    for (i = 0; i < 7; i++)
	b[i]->data = b;
    return xsize;
		  ...
		  const char *title, int overwrite_warning)
{
    ...
    my_assert(path != NULL);
    if (defname)
	if ((pnt = strrchr(defname, '\\')) == NULL
	    && (pnt = strrchr(defname, '/')) == NULL)
	    pnt = (char *) defname;
	else
	    pnt++;
    else
    ...
    w.xpos = w.ypos = -1;
    w.xsize = 26 + ws_pixstrlen("M") * FILENAME_LIMIT * 2;
    ...
    b_ok.delay = 0;
    b_ok.repeat = -1;
    b_ok.l_pressed_routine = b_ok.r_pressed_routine = b_ok.l_routine =
	b_ok.r_routine = NULL;
    b_cancel = b_ok;
    my_assert((xsize =
	       makefilebuttons(ow, 0, w_titlebarheight() + 2,
			       w_ywininsize(ow), files, no, dirs, dno, pnt,
			       b)) > 0);
    w_resizeinwin(ow, xsize, b[6]->ypos + b[6]->ysize);
    checkmem(b[7] =
	     w_addstdbutton(ow, w_b_press, 0, 0, w_xwininsize(ow) / 2,
			    w_titlebarheight() + 2, TXT_OK, &b_ok, 1));
    checkmem(b[8] =
	     w_addstdbutton(ow, w_b_press, b[7]->xsize, 0,
			    w_xwininsize(ow) / 2, b[7]->ysize, TXT_CANCEL,
			    &b_cancel, 1));
    uio_drawpath(ow, b[0]->xpos, b[0]->ypos + b[0]->ysize + 1,
		 w_xwininsize(ow));
    ...
    for (i = 0; i < b[5]->d.sls->no_strings; i++)
	FREE(b[5]->d.sls->strings[i]);
    FREE(b[5]->d.sls->strings);
    for (i = 0; i < 7; i++)
	FREE(b[i]->d.d);
    w_closewindow(ow);
    if (ok_cancel != 0 || strlen(fname) == 0) {
	FREE(fname);
	return NULL;
    }
    if (fname != NULL) {
...
int uio_getallhoglevels(FILE * hogfile, struct hoglevel ***hognames)
{
    ...
    *hognames = NULL;
	...
	waitmsg(TXT_NOHOGSIGNATURE);
	return 0;
    }
    signature[3] = 0;
    if (strcmp(signature, "DHF")) {
	waitmsg(TXT_NOHOGSIGNATURE);
	return 0;
    }
	...
	if (strcmp(name, FIRSTHOGFILE) == 0)
	    origdescent = 1;
	if (fread(&size, 4, 1, hogfile) != 1)
	    break;
	if (strlen(name) >= 5
	    && strstr(init.alllevelexts,
		      &name[strlen(name) - 3]) != NULL) {
	    checkmem(*hognames =
		     REALLOC(*hognames,
			     sizeof(struct hoglevel *) * (++hognum)));
	    checkmem((*hognames)[hognum - 1] =
		     MALLOC(sizeof(struct hoglevel)));
	...
	if (fseek(hogfile, size, SEEK_CUR))
	    break;
    }
...
void uio_hoglev_one_up(struct w_button *b)
{
    uio_updown(b->data, 11, -1);
}

void uio_hoglev_one_down(struct w_button *b)
{
    uio_updown(b->data, 11, 1);
}

void uio_hoglev_page_up(struct w_button *b)
{
    uio_updown(b->data, 11, -2);
}

void uio_hoglev_page_down(struct w_button *b)
{
    uio_updown(b->data, 11, 2);
}
...
void uio_rebuild_hoglist(struct w_button *b, int curpnt)
{
	...
	for (i = 0; i < bh->no_strings; i++) {
		...
		bh->strings[i][strlen(bh->strings[i]) - 2] =
		    hoglev_arr[i]->secret / 10 + '0';
		bh->strings[i][strlen(bh->strings[i]) - 1] =
		    hoglev_arr[i]->secret % 10 + '0';
	    }
	}
    }
...
void uio_hogfile_exclude(struct w_button *b)
{
	...
	if (bh->sel_arr[i]) {
		...
		for (j = 0; j < hoglevnum; j++)
		    ...
		    } else if (hoglev_arr[j]->secret > i)
			hoglev_arr[j]->secret--;
		memmove(&hoglev_arr[i - removed],
			&hoglev_arr[i - removed + 1],
			(hoglevnum - i + removed -
			 1) * sizeof(struct hoglevel *));
		checkmem(hoglev_arr =
			 REALLOC(hoglev_arr,
				 (--hoglevnum) *
...
void uio_hogfile_addlev(struct w_button *b, int secret)
{
	...
	*bh = ((struct w_button **) b->data)[11]->d.sls;
	...
	for (i = 0, startlev = bh->no_strings; i < bh->no_strings; i++) {
	    if (bh->sel_arr[i] || hoglev_arr[i]->secret) {
		startlev = i;
		break;
	    }
    } else
	startlev = bh->no_strings;
    for (i = 0, num_sel = 0; i < bf->no_strings; i++)
	num_sel += bf->sel_arr[i];
    if (!num_sel)
	return;
    hoglevnum += num_sel;
	...
	if (hoglev_arr[i]->secret > startlev)
	    hoglev_arr[i]->secret += num_sel;
    for (i = 0; i < bf->no_strings; i++)
	if (bf->sel_arr[i]) {
	    bf->sel_arr[i] = 0;
	    checkmem(hoglev_arr[startlev] =
		     MALLOC(sizeof(struct hoglevel)));
    ...
    uio_rebuild_hoglist(((struct w_button **) b->data)[11],
			startlev - num_sel);
...
{
    int i, no;
    if (sscanf(b->d.str->str, " %d", &no) != 1)
	return;
    if (no < 1 || no > hoglevnum)
	return;
...
void uio_hogfile_extract(struct w_button *b)
{
	...
	*bf = ((struct w_button **) b->data)[5]->d.sls;
	...
	if (bh->sel_arr[i] && hoglev_arr[i]->size > 0) {
		...
		&& !yesnomsg(TXT_OVERWRITEFILE, fullpath)) {
		fclose(f);
		return;
	    }
	    if (f)
		...
		waitmsg(TXT_CANTOPENHOGFILE, hogfilename);
		return;
	    }
	    if (fseek(f, hoglev_arr[i]->pos, SEEK_SET)) {
		waitmsg(TXT_CANTFINDLEVINHOG, hoglev_arr[i]->name,
			hogfilename);
		fclose(f);
		return;
	    }
	    checkmem(levdat = MALLOC(hoglev_arr[i]->size));
	    if (fread(levdat, 1, hoglev_arr[i]->size, f) !=
		hoglev_arr[i]->size) {
		waitmsg(TXT_CANTFINDLEVINHOG, hoglev_arr[i]->name,
			hogfilename);
		fclose(f);
		...
		waitmsg(TXT_CANTWRITEHOGLEV, hoglev_arr[i]->name,
			fullpath);
		FREE(levdat);
		return;
	    }
	    if (fwrite(levdat, 1, hoglev_arr[i]->size, f) !=
		hoglev_arr[i]->size) {
		waitmsg(TXT_CANTWRITEHOGLEV, hoglev_arr[i]->name,
			fullpath);
		FREE(levdat);
    ...
    FREE(bf->sel_arr);
    bf->strings =
	ws_getallfilenames(current_path, current_ext, &bf->no_strings);
    my_assert(bf->no_strings >= 0);
    if (bf->strings)
	qsort(bf->strings, bf->no_strings, sizeof(char *), qs_compstrs);
    bf->start_no = 0;
...
void uio_hogfile_write(struct w_button *b)
{
    ...
    struct w_button **b_arr = b->data;
    if (hoglevnum <= 0) {
	printmsg(TXT_NOHOGLEVELS);
	return;
    }
    for (i = 0; i < strlen(b_arr[17]->d.str->str); i++)
	if (b_arr[17]->d.str->str[i] != ' ')
	    break;
    if (i == strlen(b_arr[17]->d.str->str)) {
	printmsg(TXT_NOMISSIONNAME);
	return;
    }
    strcpy(buffer, current_path);
    strcpy(ext, current_ext);
    newhogname =
	getfilename(&init.missionpath, hogfilename, "HOG", "HOG-File", 1);
    strcpy(current_path, buffer);
    strcpy(current_ext, ext);
    if (newhogname == NULL)
	return;
    if (hogfilename && strcmp(newhogname, hogfilename) == 0) {
	strcpy(&newhogname[strlen(newhogname) - 3], "NEW");
	samefile = 1;
    }
    ws_splitpath(newhogname, NULL, NULL, msnfname, NULL);
    if (strcmp(msnfname, "DESCENT") == 0
	|| strcmp(msnfname, "DESCENT2") == 0) {
	printmsg(TXT_CANTOVERWRITEORIGHOG);
	return;
    }
    if ((df = fopen(newhogname, "wb")) == NULL) {
	waitmsg(TXT_CANTOPENHOGFILE, newhogname);
	FREE(newhogname);
	return;
    }
    if (fwrite("DHF", 1, 3, df) != 3) {
	waitmsg(TXT_CANTWRITESIGNATURE, newhogname);
	fclose(df);
	FREE(newhogname);
	return;
    }
    strcpy(msnfname, newhogname);
    strcpy(&msnfname[strlen(msnfname) - 3], MSNEXT);
    if ((mf = fopen(msnfname, "w")) == NULL) {
	waitmsg(TXT_CANTOPENMSNFILE, msnfname);
	fclose(df);
	FREE(newhogname);
	return;
    }
	...
	if (hoglev_arr[i]->size > 0) {
		...
		checkmem(levdat = MALLOC(hoglev_arr[i]->size));
		if (fread(levdat, 1, hoglev_arr[i]->size, sf) !=
		    hoglev_arr[i]->size) {
		    waitmsg(TXT_CANTREADLEVFROMHOG, hoglev_arr[i]->name,
			    hogfilename);
		    FREE(levdat);
		    levdat = NULL;
		}
		...
		if (fseek(lf, 0, SEEK_END))
			...
			waitmsg(TXT_CANTREADLEVEL,
				hoglev_arr[i]->fullpath);
		    else {
			checkmem(levdat = MALLOC(hoglev_arr[i]->size));
			if (fread(levdat, 1, hoglev_arr[i]->size, lf) !=
			    hoglev_arr[i]->size) {
			    waitmsg(TXT_CANTREADLEVEL,
				    hoglev_arr[i]->fullpath);
			    FREE(levdat);
	...
	if (levdat) {
	    for (j = strlen(hoglev_arr[i]->name) + 1; j < 13; j++)
		hoglev_arr[i]->name[j] = 0;
	    if (fwrite(hoglev_arr[i]->name, 1, 13, df) != 13 ||
		fwrite(&hoglev_arr[i]->size, 4, 1, df) != 1 ||
		fwrite(levdat, 1, hoglev_arr[i]->size,
		       df) != hoglev_arr[i]->size)
		waitmsg(TXT_CANTWRITEHOGLEV, hoglev_arr[i]->name,
			newhogname);
	    FREE(levdat);
	    levdat = NULL;
...
void uio_hogfile_read(struct w_button *b)
{
    ...
    strcpy(ext, current_ext);
    newhogname =
	getfilename(&init.missionpath, hogfilename, "HOG", "HOG-File", 0);
    strcpy(current_path, buffer);
    strcpy(current_ext, ext);
    if (newhogname == NULL)
	return;
    ...
    if (hoglevnum > 0)
	w_activatebutton(b_arr[13]);
    if (!origdescent && hoglevnum > 0) {
	strcpy(msnname, hogfilename);
	strcpy(&msnname[strlen(msnname) - 3], MSNEXT);
	levnum = 0;
	if ((f = fopen(msnname, "r")) != NULL) {
		...
		if (equal) {
			...
			strncpy(b_arr[17]->d.str->str, start,
				b_arr[17]->d.str->max_length);
			b_arr[17]->d.str->str[b_arr[17]->d.str->
					      max_length] = 0;
			w_drawbutton(b_arr[17]);
		    } else if (strstr(buffer, "TYPE") != NULL) {
			if (strstr(start, "ANARCHY") != NULL)
			    b_arr[18]->d.s->on = 1;
			...
			*pos = 0;
			if (sscanf(pos + 1, " %d", &secret_num) != 1)
			    secret_num = 0;
		    } else
			secret_num = 0;
		    for (i = 0; i < hoglevnum; i++)
			if (strcmp(hoglev_arr[i]->name, start) == 0)
			    break;
		    if (i < hoglevnum) {
			swap = hoglev_arr[levnum];
			hoglev_arr[levnum++] = hoglev_arr[i];
			hoglev_arr[i] = swap;
    ...
    if (nomsnfile && !origdescent)
	printmsg(TXT_CANTOPENMSNFILE, msnname);
}
...
void hogfilemanager(void)
{
    ...
    struct w_b_press b_close, b_read, b_write, b_extract, b_include,
	b_exclude, b_hf_up, b_hf_down;
    struct w_b_strlist b_hoglevels;
    struct w_b_string b_hoglevname, b_msnname, b_secret;
    struct w_b_switch b_anarchy;
    hoglevnum = 0;
    hoglev_arr = NULL;
    strcpy(current_path, init.levelpath);
    strcpy(current_ext, init.levelext);
    files = ws_getallfilenames(current_path, current_ext, &no);
    dirs = ws_getalldirs(current_path, "*", &dno);
    if (no < 0 || dno < 0) {
	if (no == -2 || dno == -2)
    ...
    w.click_routine = NULL;
    checkmem(ow = w_openwindow(&w));
    b_msnname.max_length = MAX_MSNNAME_LEN;
    b_msnname.offset = 0;
    b_msnname.str = msnname;
    msnname[0] = 0;
    ...
    checkmem(b_hoglevname.str = MALLOC(13));
    b_hoglevname.str[0] = 0;
    b_hoglevname.allowed_char = isgraph;
    b_hoglevname.l_char_entered = b_hoglevname.r_char_entered = NULL;
    b_hoglevname.l_string_entered = b_hoglevname.r_string_entered =
	uio_sel_hoglevs;
    checkmem(b[9] =
	     w_addstdbutton(ow, w_b_string, 0, w_titlebarheight() + 6,
    ...
    b_hf_down.delay = 40;
    b_hf_down.repeat = 15;
    b_hf_down.l_pressed_routine = b_hf_down.l_routine =
	uio_hoglev_one_down;
    b_hf_down.r_pressed_routine = b_hf_down.r_routine =
	uio_hoglev_page_down;
    no_rows = w_ywininsize(ow) / (w_titlebarheight() + 2) - 4;
    b_hoglevels.no_strings = 0;
    b_hoglevels.strings = NULL;
    b_hoglevels.no_rows = no_rows;
    b_hoglevels.start_no = 0;
    checkmem(b_hoglevels.sel_arr = MALLOC(hoglevnum));
    for (i = 0; i < hoglevnum; i++)
	b_hoglevels.sel_arr[i] = 0;
    b_hoglevels.l_string_selected = b_hoglevels.r_string_selected = NULL;
    b_hoglevels.max_selected = b_hoglevels.no_strings;
    checkmem(b[10] =
	     w_addstdbutton(ow, w_b_press, 0, b[9]->ypos + b[9]->ysize,
			    b[9]->xsize, -1, "/\\", &b_hf_up, 1));
    checkmem(b[11] =
	     w_addstdbutton(ow, w_b_list, 0, b[10]->ypos + b[10]->ysize,
			    b[9]->xsize,
			    no_rows * (w_titlebarheight() + 2), NULL,
			    &b_hoglevels, 1));
    checkmem(b[12] =
	     w_addstdbutton(ow, w_b_press, 0, b[11]->ypos + b[11]->ysize,
    ...
    b_close.delay = 0;
    b_close.repeat = -1;
    b_close.l_pressed_routine = b_close.r_pressed_routine =
	b_close.l_routine = b_close.r_routine = NULL;
    ...
    b_anarchy.on = 0;
    b_anarchy.l_routine = b_anarchy.r_routine = NULL;
    b_secret.max_length = 2;
    b_secret.offset = 0;
    b_secret.str = secretno;
    b_secret.allowed_char = isdigit;
    secretno[0] = 0;
    ...
    checkmem(b[16] =
	     w_addstdbutton(ow, w_b_press, b[9]->xpos + b[9]->xsize + 5,
			    b[12]->ypos - 30, 90, -1, TXT_BUT_CLOSE,
			    &b_close, 1));
    checkmem(b[7] =
	     w_addstdbutton(ow, w_b_press, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 4, 90, -1,
			    TXT_BUT_OPENHOG, &b_read, 1));
    checkmem(b[8] =
	     w_addstdbutton(ow, w_b_press, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 3, 90, -1,
			    TXT_BUT_SAVEHOG, &b_write, 1));
    checkmem(b[13] =
	     w_addstdbutton(ow, w_b_press, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 6, 90, -1,
			    TXT_BUT_EXTRACT, &b_extract, 1));
    checkmem(b[15] =
	     w_addstdbutton(ow, w_b_press, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 8, 90, -1,
			    "->", &b_exclude, 1));
    checkmem(b[14] =
	     w_addstdbutton(ow, w_b_press, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 9, 90, -1,
			    "<-", &b_include, 1));
    checkmem(b[19] =
	     w_addstdbutton(ow, w_b_string, b[16]->xpos,
			    b[16]->ypos - (b[16]->ysize + 2) * 10 - 5, 90,
			    -1, "<-Secret:", &b_secret, 1));
    b[7]->data = b[8]->data = b[13]->data = b[14]->data = b[15]->data =
	b[19]->data = b;
    my_assert(makefilebuttons
	      (ow, b[7]->xpos + b[7]->xsize + 5, b[9]->ypos,
	       w_ywininsize(ow), files, no, dirs, dno, NULL, b) > 0);
    checkmem(b[17] =
	     w_addstdbutton(ow, w_b_string, 0, 0, w_xwininsize(ow) - 90,
			    b[9]->ypos, TXT_BUT_MISSIONNAME, &b_msnname,
			    1));
    checkmem(b[18] =
	     w_addstdbutton(ow, w_b_switch, b[17]->xsize, 0, 90,
			    b[9]->ypos, TXT_BUT_ANARCHY, &b_anarchy, 1));
	...
	FREE(b[5]->d.sls->strings[i]);
    FREE(b[5]->d.sls->strings);
    FREE(b_hoglevels.sel_arr);
    FREE(b_hoglevname.str);
    for (i = 0; i < 7; i++)
	FREE(b[i]->d.d);
    for (i = 0; i < hoglevnum; i++)
	FREE(hoglev_arr[i]);
