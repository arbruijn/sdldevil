...
void b_changemode(struct w_button *b)
{
	...
	view.currmode = b->d.ls->selected;
	shrinkopt_win(view.currmode);
    }
}
...
void b_changemovemode(struct w_button *b)
{
    ...
    else if (b->d.ls->selected == mt_texture)
	if (fb_isactive()) {
	    fb_tofront();
	    view.movemode = mt_texture;
	} else
	    changemovemode(view.movemode ==
			   mt_texture ? mt_you : view.movemode);
    else
	view.movemode = b->d.ls->selected;
}

void b_move0(struct w_button *b)
{
    b_move(2, -1);
}

void b_move1(struct w_button *b)
{
    b_move(1, 1);
}

void b_move2(struct w_button *b)
{
    b_move(2, 1);
}

void b_move3(struct w_button *b)
{
    b_move(0, -1);
}

void b_move4(struct w_button *b)
{
    b_move(1, -1);
}

void b_move5(struct w_button *b)
{
    b_move(0, 1);
}
...
void b_turn0(struct w_button *b)
{
    b_turn(0, 1, 2, 1);
}

void b_turn1(struct w_button *b)
{
    b_turn(1, 2, 0, -1);
}

void b_turn2(struct w_button *b)
{
    b_turn(0, 1, 2, -1);
}

void b_turn3(struct w_button *b)
{
    b_turn(2, 0, 1, -1);
}

void b_turn4(struct w_button *b)
{
    b_turn(1, 2, 0, 1);
}

void b_turn5(struct w_button *b)
{
    b_turn(2, 0, 1, 1);
}
...
void b_changelevel(struct w_button *b)
{
	...
	if (!strcmp
	    (n->d.lev->fullname, b->d.ls->options[b->d.ls->selected])) {
...
void b_changepigfile(struct w_button *b)
{
    ...
    FREE(l->pigname);
    checkmem(l->pigname =
	     MALLOC(strlen(b->d.ls->options[b->d.ls->selected]) + 5));
...
void changepogfile(char *pogfilename)
{
	...
	if ((pogname = strrchr(pogfilename, '/')) != NULL)
	    pogname++;
	else
	    pogname = pogfilename;
    }
    if (pig.current_pogname) {
	FREE(pig.current_pogname);
...
int mbls[6][110] =
    { {0, 9, -8, 1, 0, 0, 8, -9, 0, -1, 0, -8, 10, -4, -2, 0, -8, 10, 6, 0,
       0, -4, -2, 6, 0,
       0, 5, 10, 10, 5, 0, 10, 5, 10, -5, 0, 10, -5, 5, -10, 0, 5, -10, -5,
       -10,
...
{0, -1, 2, -1, 10, 0, 1, 2, 1, 10, 0, 0, -10, 5, 2, 0, 0, -10, -5, 2, 0, 5,
 2, -5, 2,
 0, 5, -10, 10, -5, 0, 10, -5, 10, 4, 0, 10, 4, 8, 6,
 0, 4, -8, 8, -4, 0, 8, -4, 8, 3, 0, 8, 3, 7, 4,
 0, 0, 10, 10, 10, 0, 0, 10, 4, 2, 0, 4, 2, 10, 10},
{0, -9, 8, -1, 0, 0, -8, 9, 0, 1, 0, 8, -10, 4, 2, 0, 8, -10, -6, 0, 0, 4,
 2, -6, 0},
{0, 2, 1, 10, 1, 0, 2, -1, 10, -1, 0, -10, 0, 2, 5, 0, -10, 0, 2, -5, 0, 2,
 5, 2, -5},
{0, -1, -2, -1, -10, 0, 1, -2, 1, -10, 0, 0, 10, 5, -2, 0, 0, 10, -5, -2,
 0, 5, -2, -5, -2},
{0, -2, 1, -10, 1, 0, -2, -1, -10, -1, 0, 10, 0, -2, 5, 0, 10, 0, -2, -5,
 0, -2, 5, -2, -5}
};

const char *movemodes[mt_number] =
    { TXT_MOVEYOU, TXT_MOVEOBJ, TXT_MOVECURRENT,
    TXT_MOVETXT
};

uint32_t *movebuttondata[6];
...
void drawallbuttons(struct w_window *w)
{
	...
	for (i = 0; i < pigfile.num_options; i++) {
		...
		*((char *) pigfile.options[i] + j) =
		...
		//printf ("====%s=%s==", pignames[i], l->pigname);
	    if (l && !strcmp(pignames[i], l->pigname))
		pigfile.selected = i;
	    FREE(pignames[i]);
	}
	FREE(pignames);
	pigfile.select_lroutine = pigfile.select_rroutine =
	    b_changepigfile;
    }
    ...
    checkmem(view.levelbutton =
	     w_addstdbutton(w, w_b_choose, 0, y, bxsize * 3, -1, TXT_LEVEL,
			    view.b_levels, 1));
    y += view.levelbutton->ysize;
    if (init.d_ver >= d2_10_reg) {
	checkmem(b3 = b_pigfile =
		 w_addstdbutton(w, w_b_choose, 0, y, bxsize * 3, -1,
				TXT_LEVELPIGFILE, &pigfile, 1));
	y += b3->ysize;
	...
	for (j = 0; j < 22; j++) {
	    ...
	    for (k = 1; k < 5; k += 2) {
		mbls[i][j * 5 + k] += j < 5 ? 35 : i == 1
		    || i == 4 ? 4 : 11;
		mbls[i][j * 5 + k + 1] += j >= 5 && (i == 3
						     || i == 5) ? 18 : 11;
	    }
	}
    for (j = 0; j < 2; j++)
...
void changepigfile(char *palname)
{
    ...
    if (!b_pigfile)
	return;
    for (i = 0, b_pigfile->d.ls->selected = 0;
	 i < b_pigfile->d.ls->num_options; i++)
	if (!compstrs(palname, b_pigfile->d.ls->options[i])) {
	    b_pigfile->d.ls->selected = i;
	    w_drawbutton(b_pigfile);
	    break;
	}
}
...
void initpalette(void)
{
	...
	view.color[i] = w_makecolor((125 * i) / GRAYSCALE + 75, (125 * i) / GRAYSCALE + 75, (125 * i) / GRAYSCALE + 75);	/* walls */
...
void newpalette(unsigned char *palette)
{
    ...
    my_assert((f = fopen(pig.brokenbulbname, "rb")) != NULL &&
	      fread(pig.brokenbulb, 1, BULBSIZE * BULBSIZE,
		    f) == BULBSIZE * BULBSIZE);
    fclose(f);
   /* FFE no need to redraw the pixels of the bitmap since we are on
	truecolor 
    ...
    */
	...
	w_initkbstatcursor(control_cursor, cntrlcursor_xsize,
			   cntrlcursor_ysize, CNTRLCURSOR_HOT_X,
			   CNTRLCURSOR_HOT_Y, colortable, ws_ks_ctrl,
	...
	w_initkbstatcursor(alt_cursor, altcursor_xsize, altcursor_ysize,
			   ...
			   ws_ks_alt, ws_ks_ctrl | ws_ks_alt);
    }
    */
    plotlevel();
}

struct w_window button_win =
    { 0, 481, 20, 150, 135, -1, -1, 0, TXT_BUTTONS, 0, "",
...
#if defined(GNU_C) && defined(GO32)
/*
    if (showtitle && ws_initgrfx(640, 480, 32, init.fontname)
	&& titlescreen())
	ws_getevent(&ws, 1);
    ws_textmode();
*/
#endif
    if (!w_initwins(init.xres, init.yres, 32, init.fontname)) {
