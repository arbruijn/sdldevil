...
void help_btop(struct w_button *b) {
    cur_help_pos = 0;
    help_refresh(b->w);
}
...
void help_bbottom(struct w_button *b) {
    ...
    if (cur_help_pos < 0) cur_help_pos = 0;
    help_refresh(b->w);
}
...
void dec_help(int ec) {
    ...
    helpwin.ysize = w_ymaxwinsize();
    helpwin.shrunk = 0;
    helpwin.title = NULL;
    helpwin.hotkey = 0;
    helpwin.buttons = wb_none;
    helpwin.refresh = wr_normal;
    helpwin.refresh_routine = NULL;
    helpwin.close_routine = NULL;
    helpwin.click_routine = NULL;
    checkmem(w = w_openwindow(&helpwin));
    up.delay = 10;
    up.repeat = 5;
    up.l_pressed_routine = up.l_routine = help_bup;
    up.r_pressed_routine = NULL;
    up.r_routine = help_btop;
    down = up;
    down.l_pressed_routine = down.l_routine = help_bdown;
...
void dec_cubemode(int ec) {
    shrinkopt_win(tt_cube);
    changecurrmode(tt_cube);
}

void dec_sidemode(int ec) {
    shrinkopt_win(tt_wall);
    changecurrmode(tt_wall);
}

void dec_pntmode(int ec) {
    shrinkopt_win(tt_pnt);
    changecurrmode(tt_pnt);
}

void dec_edgemode(int ec) {
    shrinkopt_win(tt_edge);
    changecurrmode(tt_edge);
}

void dec_thingmode(int ec) {
    shrinkopt_win(tt_thing);
    changecurrmode(tt_thing);
}

void dec_wallmode(int ec) {
    shrinkopt_win(tt_door);
    changecurrmode(tt_door);
}
...
void dec_movefactor(int ec) {
    ...
    switch (view.movemode) {
        case mt_you: view.movefactor *= f;
            break;
        case mt_obj: view.pmovefactor *= f;
            break;
...
void dec_rotangle(int ec) {
    ...
    switch (view.movemode) {
        case mt_you: view.rotangle *= f;
            break;
        case mt_obj: view.protangle *= f;
            break;
        case mt_current: case mt_texture: break;
        default: my_assert(0);
    }
...
void dec_changefullname(int ec) {
    ...
    if (i >= view.b_levels->num_options) {
        waitmsg("Error searching levelname...");
        return;
    }
    checkmem(s.str = MALLOC(s.max_length + 1));
    if (l->fullname) {
        strncpy(s.str, l->fullname, s.max_length);
        s.str[s.max_length] = 0;
    } else s.str[0] = 0;
...
void dec_drawwhat(int ec) {
    switch (ec) {
        case ec_drawalllines: view.drawwhat ^= DW_ALLLINES;
            if (!(view.drawwhat & DW_ALLLINES)) initdescmap();
            break;
        default: my_assert(0);
    }
    plotlevel();
...
void dec_missionmanager(int ec) {
    hogfilemanager();
}
...
void dec_loadwinpos(int ec) {
    ...
    if ((f = fopen(fname, "r")) == NULL) {
        printmsg(TXT_CANTREADWINPOS, fname);
        return;
    }
    w_readcfg(f);
    for (i = 0; i < tlw_num; i++) {
        if (fscanf(f, " %d", &tw_savedata[i].zoom) != 1) {
            waitmsg(TXT_CANTREADWINPOS);
            return;
        }
...
void dec_dummy(int ec) {
    waitmsg("Functioncode %x not used");
}

void (*do_event[ec_num_of_codes])(int ec) ={dec_quit, dec_loadlevel, dec_savelevel, dec_makemacro, dec_savemacro,
