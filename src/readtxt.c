...
void readpogfile(FILE * f)
{
    ...
    checkmem(texture_index =
	     MALLOC(sizeof(int16_t) * head.num_textures));
    if (fread(texture_index, sizeof(int16_t), head.num_textures, f) !=
	head.num_textures) {
	FREE(texture_index);
	printmsg(TXT_POGERRORINHEADER);
	return;
	...
	pt.data = NULL;
	pt.offset +=
	    head.num_textures * (18 + 2) + sizeof(struct POG_header);
...
int readtxts(FILE * f)
{
    ...
    case d2_11_reg:
    case d2_12_reg:
	if (strncmp(head.c, "PPIG", 4))
	    return 0;
	if (fread(&numshort, sizeof(uint32_t), (size_t) 1, f) != 1)
	    return 0;
	numlong = 0;
	break;
    case d1_10_sw:
    case d1_10_reg:
    case d1_14_reg:
	if (head.ul[0] > 0x10000) {	/* D1 1.4 */
	    fseek(f, head.ul[0], SEEK_SET);
	    if (fread(&numshort, sizeof(uint32_t), (size_t) 1, f) !=
		1)
		return 0;
	    if (fread(&numlong, sizeof(uint32_t), (size_t) 1, f) != 1)
		return 0;
	} else {		/* D1 1.0 */
	    numshort = head.ul[0];
	    numlong = head.ul[1];
    ...
    checkmem(pig.pig_txts =
	     MALLOC(sizeof(struct pig_txt) * pig.num_pigtxts));
    for (i = 0; i < pig.num_pigtxts; i++) {
	if (init.d_ver >= d2_10_sw) {
	    if (fread(&pig.pig_txts[i], 18, 1, f) != 1)
		return 0;
	} else {
	    ...
	    pig.pig_txts[i].rlxsize =
		D1_txt.ruxsize == 0x80 ? 0x40 : D1_txt.rxsize;
	...
	pig.pig_txts[i].xsize =
	    pig.pig_txts[i].rlxsize + (pig.pig_txts[i].ruxsize << 8);
	pig.pig_txts[i].ysize = (unsigned short) pig.pig_txts[i].rysize;
	// FFE the memory gets allocated above, so we initialize these 
	// fields - especially the data pointer which occationnally points	
	// to garbage and is getting freed and therefore it crashes
	//if (newpig) {
	...
	if (init_test & 1)
	    fprintf(errf, "Found %d: %s %u %u %u %u %lx\n", i,
		    pig.pig_txts[i].name, (unsigned) pig.pig_txts[i].xsize,
		    (unsigned) pig.pig_txts[i].ysize,
		    (unsigned) pig.pig_txts[i].type1,
		    (unsigned) pig.pig_txts[i].type2,
...
int readcustomtxts(FILE * f)
{
	...
	if (head.ul[0] > 0x10000) {	/* D1 1.4 */
	    fseek(f, head.ul[0], SEEK_SET);
	    if (fread(&numshort, sizeof(unsigned long), (size_t) 1, f) !=
		1)
		return 0;
	...
	for (i = 0; i < numshort; i++) {
	    ...
	    for (j = 0, k = 0;
		 ((k < (D1_txt.ruxsize & 0x0F))
		  || (strcasecmp(s, pig.pig_txts[j].rname) != 0))
		 && (j < pig.num_pigtxts); j++)
		if (strcasecmp(s, pig.pig_txts[j].rname) == 0)
		    k++;
	    if (j < pig.num_pigtxts) {
		pig.pig_txts[j].rlxsize =
		    D1_txt.ruxsize == 0x80 ? 0x40 : D1_txt.rxsize;
		pig.pig_txts[j].ruxsize =
		    D1_txt.ruxsize == 0x80 ? 0x01 : 0;
		pig.pig_txts[j].rysize = D1_txt.rysize;
		pig.pig_txts[j].type1 = D1_txt.type1;
		pig.pig_txts[j].type2 = D1_txt.type2;
		pig.pig_txts[j].offset =
		    D1_txt.offset + 8 + 17 * numshort + 20 * numlong;
		pig.pig_txts[j].xsize =
		    pig.pig_txts[j].rlxsize +
		    (pig.pig_txts[j].ruxsize << 8);
		pig.pig_txts[j].ysize =
		    (unsigned short) pig.pig_txts[j].rysize;
...
void copytxtnums(enum txttypes dest, enum txttypes source)
{
    ...
    memmove(&pig.txtlist[dest][pig.num_txtlist[dest]],
	    pig.txtlist[source],
	    pig.num_txtlist[source] * sizeof(struct ham_txt *));
...
{
    struct ham_txt *txt1 = *(struct ham_txt **) t1, *txt2 =
	*(struct ham_txt **) t2;
    if (txt1 == NULL || txt1->pig == NULL)
	return -1;
    if (txt2 == NULL || txt2->pig == NULL)
	return 1;
...
void inittxts(void)
{
	...
	if (pig.rdl_txts[rdlnum].pigno == 0) {
	    pig.rdl_txts[rdlnum].pig = NULL;
	    continue;
	}
	my_assert(pig.rdl_txts[rdlnum].pigno >= 0 &&
		  pig.rdl_txts[rdlnum].pigno < pig.num_pigtxts);
	pig.rdl_txts[rdlnum].pig =
	    &pig.pig_txts[pig.rdl_txts[rdlnum].pigno];
	if (init_test & 1)
	    fprintf(errf, "rdlnum=%d pignum=%d name=%s type1=%x\n",
		    rdlnum, pig.rdl_txts[rdlnum].pigno,
		    pig.rdl_txts[rdlnum].pig->name,
	...
	if (strstr(pig.anim_txt_names, text) == NULL) {
	    ...
	    case 2:
	    case 3:
		pig.num_txtlist[txt2_wall]++;
		break;
	    }
	} else if (firstanimtxt < 0)
	    firstanimtxt = rdlnum;
	else if (strcmp(pig.rdl_txts[rdlnum].pig->name,
			pig.rdl_txts[firstanimtxt].pig->name)) {
	    pig.rdl_txts[firstanimtxt].pig->num_anims =
		rdlnum - firstanimtxt;
	...
	checkmem(pig.txtlist[txttyp] =
		 MALLOC(sizeof(struct ham_txt *) *
			pig.num_txtlist[txttyp]));
	typnum[txttyp] = 0;
    }
    for (rdlnum = 0; rdlnum < pig.num_rdltxts; rdlnum++) {
	if (pig.rdl_txts[rdlnum].pig == NULL)
	    continue;
	strcpy(text, pig.rdl_txts[rdlnum].pig->name);
	if ((pos = strpbrk(text, " 0123456789")) != NULL)
	    *pos = 0;
	if (strstr(pig.anim_txt_names, text) == NULL ||
	    (init.d_ver >= d2_12_reg
	     && pig.rdl_txts[rdlnum].pig->frame_num == 0))
	    switch (pig.rdl_txts[rdlnum].pig->type1 & 0x03) {
	    case 0:
		pig.txtlist[txt1_normal][typnum[txt1_normal]] =
		    &pig.rdl_txts[rdlnum];
		pig.rdl_txts[rdlnum].txtlistno[txt1_normal] =
		    typnum[txt1_normal]++;
		break;
	    case 1:		/* transparent */
		pig.txtlist[txt2_normal][typnum[txt2_normal]] =
		    &pig.rdl_txts[rdlnum];
		pig.rdl_txts[rdlnum].txtlistno[txt2_normal] =
		    typnum[txt2_normal]++;
		break;
	    case 2:
	    case 3:		/* super-transparent */
		pig.txtlist[txt2_wall][typnum[txt2_wall]] =
		    &pig.rdl_txts[rdlnum];
		pig.rdl_txts[rdlnum].txtlistno[txt2_wall] =
		    typnum[txt2_wall]++;
		break;
	    }
    }
	...
	if (rdlnum != 0) {
	    ...
	    pig.anims[animnum] = NULL;
    }
    checkmem(pig.txtlist[txt_door] = CALLOC(sizeof(struct ham_txt *),
					    pig.num_txtlist[txt_door]));
    for (animnum = 0, typnum[txt_door] = 0; animnum < pig.num_anims;
	 animnum++)
    ...
    ptxt_nothing.num_anims = 0;
    ptxt_nothing.anim_t2 = 1;
    ptxt_nothing.pigno = -1;
    ptxt_nothing.f = NULL;
    ptxt_nothing.frame_num = 0;
    ptxt_nothing.xsize = ptxt_nothing.ysize = 64;
    ptxt_nothing.offset = 0;
    ptxt_nothing.type1 = ptxt_nothing.type2 = 0;
    strcpy(ptxt_default.name, " Default");
    ptxt_default.num_anims = 0;
    ptxt_default.anim_t2 = 1;
    ptxt_default.pigno = -1;
    ptxt_default.f = NULL;
    ptxt_default.frame_num = 0;
    ptxt_default.xsize = ptxt_default.ysize = 64;
    ptxt_default.offset = 0;
	...
	txt_nothing.txtlistno[txttyp] = -1;
    txt_nothing.pig = &ptxt_nothing;
    txt_default = txt_nothing;
    txt_default.pig = &ptxt_default;
    txt_default.rdlno = -1;
    ...
    for (txttyp = 0; txttyp < txt_number; txttyp++) {
	qsort(pig.txtlist[txttyp], pig.num_txtlist[txttyp],
	      sizeof(struct ham_txt *), cmp_txts);
	for (i = 0; i < pig.num_txtlist[txttyp]; i++) {
	    if (init_test & 1)
		fprintf(errf, "txttyp=%d i=%d name=%s rdlno=%d\n", txttyp,
			i, pig.txtlist[txttyp][i]->pig->name,
			pig.txtlist[txttyp][i]->rdlno);
...
int newpigfile(char *pigname, FILE * pogfile)
{
    ...
    int i;
    FILE *pf;
    if (!pig.current_pigname || strcmp(pig.current_pigname, pigname) || pogfile != pig.pogfile) {	/* new pig-file */
	checkmem(pigfname = MALLOC(strlen(pigname) + strlen(init.pigpaths[init.d_ver]) + 2));
	...
	if (!readtxts(pf)) {
	    printf("Error reading textures in newpigfile\n");
	    fclose(pf);
	    FREE(pigfname);
	    return 0;
	}
		...
		struct ham_txt *ham_sd, int dir)
{
	...
	if (fread(&size, sizeof(uint32_t), (size_t) 1, f) != 1) {
	...
	for (i = 0; i < size; i++) {
	    ws_bmdrawline8(bm, startx + addx, starty + addy, endx + addx,
			  endy + addy, view.color[WHITE], 0);
	    ws_bmdrawline8(bm, startx + addx, starty + addy, x + addx,
			  y + addy, view.color[WHITE], 0);
	    if (mx > 0)
		addx -= 2;
	    else if (mx < 0)
		addx += 2;
		...
		addy += 2;
	    ws_bmdrawline8(bm, startx + addx, starty + addy, endx + addx,
			  endy + addy, view.color[BLACK], 0);
	    ws_bmdrawline8(bm, startx + addx, starty + addy, x + addx,
			  y + addy, view.color[BLACK], 0);
	    if (mx > 0)
		addx -= 2;
	    else if (mx < 0)
		addx += 2;
