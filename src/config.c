...
int savelvlconfig(FILE * f, struct leveldata *ld, int no)
{
    ...
    if (fprintf(f, "{%s} * Levelfilename\n",
		ld->filename != NULL ? ld->filename : "") < 0)
	return 0;
    if (fprintf(f, "%d * Level properly illuminated?\n", ld->levelillum) <
	0)
	return 0;
    if (fprintf(f, "%d %d %d %d %d %d * current objects\n",
		CUROBJNO(ld->pcurrcube), CUROBJNO(ld->pcurrpnt),
		ld->currwall, ld->curredge, CUROBJNO(ld->pcurrthing),
		CUROBJNO(ld->pcurrdoor)) < 0)
	return 0;
    if (fprintf
	(f, "%g %g %g * position of the user\n", ld->e0.x[0], ld->e0.x[1],
	 ld->e0.x[2]) < 0)
	return 0;
    for (i = 0; i < 3; i++)
	if (fprintf
	    (f, "%g %g %g * orientation of the user\n", ld->e[i].x[0],
	     ld->e[i].x[1], ld->e[i].x[2]) < 0)
	    return 0;
    for (n = ld->cubes.head, num_locked = 0; n->next != NULL; n = n->next)
	for (i = 0; i < 6; i++)
	    num_locked += (n->d.c->walls[i] && n->d.c->walls[i]->locked);
    if (fprintf(f, "%d * Number of locked sides\n", num_locked) < 0)
	return 0;
    for (n = ld->cubes.head, num_locked = 0; n->next != NULL; n = n->next)
	for (i = 0; i < 6; i++)
	    if (n->d.c->walls[i] && n->d.c->walls[i]->locked)
		if (fprintf(f, "%d ", n->no * 6 + i) < 0)
		    return 0;
    for (i = 0; i < tt_number; i++) {
	if (fprintf
	    (f, "%d * Number of tagged objects %d\n", ld->tagged[i].size,
	     i) < 0)
	    return 0;
	for (n = ld->tagged[i].head; n->next != NULL; n = n->next)
	    switch (i) {
	    case tt_edge:
		if (fprintf(f, "%d ", (n->d.n->no) * 24 + n->no % 24) < 0)
		    return 0;
		break;
	    case tt_wall:
		if (fprintf(f, "%d ", (n->d.n->no) * 6 + n->no % 6) < 0)
		    return 0;
		break;
	    default:
		if (fprintf(f, "%d ", n->d.n->no) < 0)
		    return 0;
	    }
	if (fprintf(f, "\n") < 0)
	    return 0;
...
int savestatus(int playlevel)
{
    ...
    in_changecurrentlevel(ld);	// so the data in the level structure is correct
	...
	(f, "%g %g %g %g %g %g %d %g %g\n", view.dist, view.maxvisibility,
	 ...
	 view.bm_movefactor, view.flatsideangle,
	 view.bm_stretchfactor) < 0)
	ERRORSAVECFG(f);
    if (fprintf(f, "%g %g %g %g %g %g %g %d %d %d %d %d\n",
		view.gridlength, view.maxconndist, view.maxuserconnect,
		view.illum_quarterway, view.illum_brightness,
		view.distcenter, view.minclicksodist,
		(int) view.illum_minvalue, fb_savedata.xpos,
		fb_savedata.ypos, fb_savedata.xsize,
		fb_savedata.ysize) < 0)
	ERRORSAVECFG(f);
    for (i = 0; i < tlw_num; i++)
	if (fprintf(f, "%d %d %d %d %d\n", tl_win[i].zoom.selected,
		    tl_win[i].oldxpos, tl_win[i].oldypos,
		    tl_win[i].oldxsize < 0 ? 0 : tl_win[i].oldxsize,
		    tl_win[i].oldysize) < 0)
	    ERRORSAVECFG(f);
    fprintf(f, "\n");
    if (fprintf(f,
		...
		"%d %d %d %d %d %g %d %d\n",
		view.displayed_group, view.gridonoff, view.askone,
		view.asktagged, view.drawwhat | DW_ALLLINES, view.currmode,
		view.movemode, CUROBJNO(view.pdeflevel),
		CUROBJNO(view.pdefcube), view.defwall, view.warn_convex,
		view.warn_doublekeys, view.warn_tworeactors,
		view.warn_thingoutofbounds, view.warn_gridrot,
		view.doubleclick, view.whichdisplay, view.render,
		corr_win1_xpos, corr_win1_ypos, corr_win2_xpos,
		corr_win2_ypos, view.gamma_corr, view.coord_axis,
		view.flip_y_axis, view.draw_orig_lines, view.littlebulbson,
		view.mouse_flipaxis, view.blinkinglightson,
		(double) view.timescale, view.warn_frameratetoosmall,
		view.warn_illuminate) < 0)
	ERRORSAVECFG(f);
    if (pig.current_pogname != NULL) {
	strcpy(fname, init.pogpath);
	strcat(fname, "/");
	strcat(fname, pig.current_pogname);
    } else
	strcpy(fname, "NoPogFile");
    if (fprintf
	(f, "%s\n%s\n%s\n%s\n%s\n%s\n%s\n", init.levelpath, init.pogpath,
	 init.macropath, init.missionpath, init.txtlistpath,
	 init.lightname, fname) < 0)
	ERRORSAVECFG(f);
    if (!playlevel) {
	for (n = view.levels.head, i = 0, j = -1, k = -1; n->next != NULL;
	     n = n->next)
	    if (n->d.lev->filename) {
		if (n->d.lev == l)
		    j = i;
		if (n->d.lev == view.pcurrmacro)
		    k = i;
		i++;
	    }
    } else {
    ...
    if (fprintf
	(f, "%d %d %d * Number of levels, current level, current macro\n",
	 i, j, k) < 0)
	ERRORSAVECFG(f);
	...
	if (playlevel) {
		...
		&& l != NULL) {
		char dr[3], pa[1024], fi[256], ex[32];
		char pg1filename[1024], pg1tmpname[1024];
...
int readlvlconfig(FILE * f, struct leveldata *ld)
{
    ...
    skipline(f);
    if (fscanf
	(f, "%d%d%d%d%d%d", &n0, &n1, &ld->currwall, &ld->curredge, &n2,
	 &n3) != 6)
	return 0;
    skipline(f);
    if (n0 > 0)
	ld->pcurrcube = findnode(&ld->cubes, n0);
    if (n1 > 0)
	ld->pcurrpnt = findnode(&ld->pts, n1);
    if (!ld->pcurrcube && ld->cubes.size)
	ld->pcurrcube = ld->cubes.head;
    ld->pcurrwall = ld->pcurrcube == NULL ? NULL :
	ld->pcurrcube->d.c->walls[ld->currwall];
    if (n2 > 0)
	ld->pcurrthing = findnode(&ld->things, n2);
    if (!ld->pcurrthing && ld->things.size)
	ld->pcurrthing = ld->things.head;
    if (n3 > 0)
	ld->pcurrdoor = findnode(&ld->doors, n3);
    if (!ld->pcurrdoor && ld->doors.size)
	ld->pcurrdoor = ld->doors.head;
    if (fscanf(f, "%g%g%g", &ld->e0.x[0], &ld->e0.x[1], &ld->e0.x[2]) != 3)
	return 0;
    skipline(f);
    for (n1 = 0; n1 < 3; n1++) {
	if (fscanf
	    (f, "%g%g%g", &ld->e[n1].x[0], &ld->e[n1].x[1],
	     &ld->e[n1].x[2]) != 3)
	    return 0;
	skipline(f);
    }
	...
	if (fscanf(f, "%d", &n2) != 1)
	    return 0;
	if ((n = findnode(&ld->cubes, n2 / 6)) != NULL
	    && n->d.c->walls[n2 % 6])
	...
	for (n2 = 0; n2 < n1; n2++) {
	    ...
	    switch (n0) {
	    case tt_cube:
		if ((n = findnode(&ld->cubes, n3)) != NULL)
		    tag(tt_cube, n);
		break;
	    case tt_wall:
		if ((n = findnode(&ld->cubes, n3 / 6)) != NULL)
		    tag(tt_wall, n, n3 % 6);
		break;
	    case tt_edge:
		if ((n = findnode(&ld->cubes, n3 / 24)) != NULL)
		    tag(tt_edge, n, (n3 % 24) / 4, (n3 % 24) % 4);
		break;
	    case tt_pnt:
		if ((n = findnode(&ld->pts, n3)) != NULL)
		    tag(tt_pnt, n);
		break;
	    case tt_thing:
		if ((n = findnode(&ld->things, n3)) != NULL)
		    tag(tt_thing, n);
		break;
	    case tt_door:
		if ((n = findnode(&ld->doors, n3)) != NULL)
		    tag(tt_door, n);
		break;
	    default:
		return 0;
...
void readstatus(char *lfname)
{
	...
	printmsg(TXT_CANTREADSTATUSFILE, init.lastname);
	return;
    }
    my_assert(findmarker(f, "INITDATA", &lno));
    if (fscanf(f, "%g%g%g%g%g%g%d%g%g", &view.dist,
	       &view.maxvisibility, &view.movefactor, &view.pmovefactor,
	       &view.rotangle, &view.protangle, &view.bm_movefactor,
	       &view.flatsideangle, &view.bm_stretchfactor) != 9)
	ERRORREADCFG(f);
    skipline(f);
    if (fscanf
	(f, "%g%g%g%g%g%g%g%hd%d%d%d%d", &view.gridlength,
	 &view.maxconndist, &view.maxuserconnect, &view.illum_quarterway,
	 &new_brightness, &view.distcenter, &view.minclicksodist,
	 &view.illum_minvalue, &fb_savedata.xpos, &fb_savedata.ypos,
	 &fb_savedata.xsize, &fb_savedata.ysize)
	!= 12)
	ERRORREADCFG(f);
    for (i = 0; i < tlw_num; i++) {
	if (fscanf(f, "%d%d%d%d%d", &tw_savedata[i].zoom,
		   &tw_savedata[i].xpos, &tw_savedata[i].ypos,
		   &tw_savedata[i].xsize, &tw_savedata[i].ysize) != 5)
	    ERRORREADCFG(f);
	if (tw_savedata[i].xsize > 0)
	    tl_win[i].oldxsize = -1;
    }
    if (fscanf(f,
	       "%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%hd%d%d%d%d%hd%hd%g%d%d",
	       &view.displayed_group, &view.gridonoff, &view.askone,
	       &view.asktagged, &view.drawwhat, &d, &e, &lno, &c,
	       &view.defwall, &view.warn_convex, &view.warn_doublekeys,
	       &view.warn_tworeactors, &view.warn_thingoutofbounds,
	       &view.warn_gridrot, &view.doubleclick, &view.whichdisplay,
	       &view.render, &corr_win1_xpos, &corr_win1_ypos,
	       &corr_win2_xpos, &corr_win2_ypos, &view.gamma_corr,
	       &view.coord_axis, &view.flip_y_axis, &view.draw_orig_lines,
	       &view.littlebulbson, &view.mouse_flipaxis,
	       &view.blinkinglightson, &view.timescale,
	       &view.warn_frameratetoosmall, &view.warn_illuminate) != 32)
	ERRORREADCFG(f);
	...
	printmsg(TXT_LOOKINGFORPOGFILE, fname);
	changepogfile(fname);
    }
    if (c > 0 && lno > 0)
	if ((view.pdeflevel = findnode(&view.levels, lno)) != NULL)
	    if ((view.pdefcube =
		 findnode(&view.pdeflevel->d.lev->cubes, c)) == NULL)
		view.pdeflevel = NULL;
    if (fscanf(f, "%d%d%d", &lno, &d, &mno) != 3)
	ERRORREADCFG(f);
    skipline(f);
    for (c = 0; c < lno; c++) {
	if (fscanf(f, "%255s", fname) != 1)
    ...
    fclose(f);
    if (d >= 0) {
	for (e = 0, n = view.levels.head; e < d && n->next;
	     n = n->next, e++);
	if (n->next != NULL)
	    changecurrentlevel(n->d.lev);
    }
    if (mno >= 0) {
	for (e = 0, n = view.levels.head; e < mno && n->next;
	     n = n->next, e++);
	if (n->next != NULL)
	    view.pcurrmacro = n->d.lev;
    }
    drawopt(in_internal);
    changemovemode(view.movemode);
    changecurrmode(view.currmode);
}

const char *txtgroup_txt[11] =
    { NULL, TXT_TOP, TXT_UP, TXT_DOWN, TXT_LIST, TXT_TXT1,
    TXT_TOP, TXT_UP, TXT_DOWN, TXT_LIST, TXT_TXT2
};

int initinforead(FILE * f, struct infoitem **is, int num,
		 enum infos infonr)
{
	...
	*is = NULL;
	...
	if (read_txt > 0) {
	    ...
	    *i2 = *(i2 - read_txt);
	    ...
	    if (++read_txt == (i2->type == it_texture ? 11 :
			       (i2->type == it_thingtexture ? 6 : 5)))
		read_txt = 0;
	} else
	    b = iniread(f, "i", i2);
	i2->tagnr = infonr;
	if ((i2->type == it_texture || i2->type == it_dooranim
	     || i2->type == it_thingtexture)
	    && i2->multifuncnr == 0) {
	    switch (i2->type) {
	    case it_texture:
		num += 10;
	    ...
	    a = i2 - *is;
	    if ((*is =
		 REALLOC(*is, num * sizeof(struct infoitem))) == NULL) {
	...
	if (i2->numchildren > 0) {
	    ...
	    if ((i2->itemchildren =
		 MALLOC(sizeof(unsigned long) * i2->numchildren)) ==
		NULL) {
		printf("No mem for info item children.\n");
		exit(2);
	    }
	    if ((i2->numinchildren =
		 MALLOC(sizeof(int) * i2->numchildren)) == NULL) {
		printf("No mem for info numinchildren.\n");
		exit(2);
	    }
	} else {
	    i2->children = NULL;
	    i2->itemchildren = NULL;
	...
	if (b > 0) {
		...
		if (a > 0) {
			...
			printf("Too many children %d/%d: %d>=%d\n", b,
			       i2->od->size, iinr + 1, i2->numchildren);
			exit(2);
		    }
		    i2->itemchildren[iinr] = i2->od->data[b]->no;
		    i2->numinchildren[iinr] =
			initinforead(f, &i2->children[iinr], a, infonr);
		    iinr++;
		}
	    }
	} else
				  ...
				  struct objtype **ot)
{
	...
	for (s = 0, ncuri = NULL; s < curi->numchildren; s++)
		...
		num = curi->numinchildren[s];
		ncuri = curi->children[s];
		break;
	    }
    if (ncuri == NULL) {
    ...
    if ((*ot)->str[0] != '!')
	return findcopytypelist(ncuri, num, ot + 1);
    return ncuri;
}
...
void initobjtypelists(struct infoitem *is, int num)
{
	...
	if (i->od == NULL)
	    continue;
	for (ot = i->od->data, lastot = NULL;
	     ot - i->od->data < i->od->size; ot++) {
	    if ((*ot)->str[0] == '>' || (*ot)->str[0] == '#'
		|| (*ot)->str[0] == '!') {
		lastot = ot;
		break;
	...
	if (lastot != NULL) {
	    if (i->od->data[i->od->size - 1]->str[0] != '!'
		|| (*lastot)->str[0] != '!') {
	    ...
	    if ((i->od->data =
		 REALLOC(i->od->data,
			 (oldnum +
			  curi->od->size) * sizeof(struct objtype **))) ==
		NULL) {
...
void printobjtypelists(struct infoitem *is, int num, int indent)
{
	...
	fprintf(errf, "%d %d %s %d %d %d %d %p\n", indent, i->type, i->txt,
		i->offset, i->length, i->sidefuncnr, i->numchildren,
		i->children);
	if (i->od != NULL)
	...
	for (a = 0; a < i->numchildren; a++) {
	    ...
	    printobjtypelists(i->children[a], i->numinchildren[a],
			      indent + 1);
	}
    }
}
...
int readconfig(void)
{
    ...
    */
	...
	init.lightname[strlen(init.lightname) - 1] =
	...
	checkmem(init.convtxts =
		 MALLOC(sizeof(int) * 2 * init.num_convtxts));
	for (i = 0; i < init.num_convtxts; i++)
	    if (fscanf
		(f, " %d %d", &init.convtxts[i * 2],
		 &init.convtxts[i * 2 + 1]) != 2) {
		fclose(f);
		printf(TXT_CANTREADCTTXT, i);
		return 0;
	    }
	...
	for (i = 0; i < init.num_convanims; i++)
	    if (fscanf
		(f, " %d %d", &init.convanims[i * 2],
		 &init.convanims[i * 2 + 1]) != 2) {
		fclose(f);
		printf(TXT_CANTREADCTANIM, i);
		return 0;
	    }
	fclose(f);
    }
	...
	init.infonum[i] =
	    initinforead(f, &init.info[i], init.infonum[i], i);
    }
	...
	checkmem(pig.rdl_txts =
		 MALLOC(pig.num_rdltxts * sizeof(struct ham_txt)));
	for (i = 0; i < pig.num_rdltxts; i++) {
	    pig.rdl_txts[i].rdlno = i;
	    pig.rdl_txts[i].pig = NULL;
	    for (j = 0; j < ILLUM_GRIDSIZE * ILLUM_GRIDSIZE; j++)
		pig.rdl_txts[i].my_light[j] = 0;
	    for (j = 0; j < txt_number; j++)
		pig.rdl_txts[i].txtlistno[j] = -1;
	    my_assert(fscanf(f, "%hd ", &pig.rdl_txts[i].pigno) == 1);
	    pig.rdl_txts[i].flags = pig.rdl_txts[i].light =
		pig.rdl_txts[i].hitpoints = pig.rdl_txts[i].xspeed =
		pig.rdl_txts[i].yspeed = 0;
    ...
    } else if (init.pigpaths[d2_10_reg]
	       && strlen(init.pigpaths[d2_10_reg]) > 0) {
	hamfname = makepath(init.pigpaths[d2_10_reg], hamname);
	if ((hamf = fopen(hamfname, "rb")) == NULL) {
	    printf(TXT_CANTFINDHAMFILE, hamfname);
	    return 0;
	}
	if (fread(buffer, 1, 4, hamf) != 4) {
	    printf(TXT_CANTREADHAMFILE, hamfname);
	    return 0;
	}
	if (strncmp(buffer, "HAM!", 4)) {
	    printf(TXT_NOHAMFILE, hamfname);
	    return 0;
	}
	...
	if (hamver != 3) {
	    printf(TXT_WRONGHAMVERSION, hamfname, (int) hamver, 3);
	    return 0;
	}
	...
	for (i = 0; i < pig.num_rdltxts; i++)
		...
		fprintf(errf,
			...
			i, pig.rdl_txts[i].flags & 0xff,
			pig.rdl_txts[i].light, pig.rdl_txts[i].hitpoints,
			pig.rdl_txts[i].anim_seq,
			pig.rdl_txts[i].shoot_out_txt,
			pig.rdl_txts[i].xspeed, pig.rdl_txts[i].yspeed);
	FREE(hamfname);
    } else {
	printf("No texturelist (No Descent 2 directory).\n");
	return 0;
    }
    FREE(hamname);
    if (!newpigfile(pig.default_pigname, NULL)) {
	strcpy(buffer, pig.default_pigname);
...
int saveplaymsn(int savetoddir)
{
	...
	printmsg(TXT_CANTSAVEPLAYMSN, fname);
	return 0;
    }
    fprintf(f, "name = %s\n",
    ...
    fprintf(f,
	    init.d_ver >= d2_10_sw ? "tmpdevil.rl2\n" : "tmpdevil.rdl\n");
    fclose(f);
    strcpy(fname, init.playmsnpath);
    ...
    if (fwrite
	(init.d_ver >= d2_10_sw ? "TMPDEVIL.RL2" : "TMPDEVIL.RDL", 1, 13,
	 f) != 13) {
	printmsg(TXT_CANTSAVEPLAYHOG, fname);
	fclose(f);
	return 0;
    }
    strcpy(sfname, init.cfgpath);
    strcat(sfname, "/");
    strcat(sfname, CFG_CURNAME);
	...
	fseek(pig.pogfile, 0, SEEK_END);
	size = ftell(pig.pogfile);
	rewind(pig.pogfile);
	if (fwrite(&size, 4, 1, f) != 1) {
	    printmsg(TXT_CANTSAVEPLAYHOG, fname);
	    fclose(f);
