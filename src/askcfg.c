...
int writeconfig(int olddata)
{
    ...
    missionpath[0] = 0;
    if (!askconfigdata
	(&d_ver, devilpath, d1path, d2path, missionpath, &grfxdriver,
	 &resolution)) {
    ...
    ws_makepath(d1path, d1path);
    ws_makepath(d2path, d2path);
	...
	fprintf(cfgfile, "%d %d %d {%s}\n", hotkeys[i].kbstat,
		hotkeys[i].key, hotkeys[i].event, hotkeys[i].txt);
    fprintf(cfgfile, "\n:DESCENTVERSION %d\n", d_ver);
    fprintf(cfgfile, "\n:RESOLUTION %d\n", resolution);
    fclose(cfgfile);
    ...
    makedospath(d1path, d1path);
    makedospath(d2path, d2path);
    ...
    fprintf(cfgfile, "set DESCENTSTART=%s\n",
	    d_ver >= d2_10_sw ? "D2.BAT" : "DESCENT.BAT");
