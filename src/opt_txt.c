...
const char *tlw_tm_options[TLW_TM_NUM_OPTIONS] =
    { TXT_TLW_ALL, TXT_TLW_LEVEL,
    TXT_TLW_USER
};
...
int next_txtno(struct txt_list_win *tlw, int offset, int add)
{
    ...
    switch (tlw->type) {
    case tlw_t1:
	tt = view.pcurrcube && view.pcurrcube->d.c->nc[view.currwall] ?
	    txt1_wall : txt1_normal;
	break;
    case tlw_t2:
	tt = view.pcurrcube && view.pcurrcube->d.c->nc[view.currwall] ?
	    txt2_wall : txt2_normal;
	break;
    case tlw_thing:
	tt = txt_thing;
	break;
    ...
    rdlno = offset >= 0
	&& offset < tlw->maxnum ? tlw->t[offset]->rdlno : -1;
    while (add != 0) {
	offset += incdec;
	if (offset < 0)
	    return -1;
	else if (offset >= tlw->maxnum)
	    return tlw->maxnum;
	if (tlw->t[offset]->txtlistno[tt] >= 0
	    && tlw->t[offset]->rdlno != rdlno) {
	    add -= incdec;
	    rdlno = tlw->t[offset]->rdlno;
	}
    }
    return offset;
}
...
void initdisplay(struct txt_list_win *tlw)
{
    int i;
    long rdl_curtxt = 0;
    if (tlw->display.selected == tlw->olddisplay)
	return;
    tlw->olddisplay = tlw->display.selected;
    if (tlw->mode.selected == 0) {
	if (!getno(tlw->i, &rdl_curtxt, NULL))
	    rdl_curtxt = -10;
	if (tlw->type == tlw_anim && rdl_curtxt >= 0
	    && rdl_curtxt < pig.num_anims)
	...
	case 1:		/* textures from level */
	    tlw->t = tlw->leveltxtlist;
	    tlw->maxnum = tlw->num_leveltxtlist;
	    break;
	case 2:
	case 3:		/* user defined, load/save */
	    tlw->t = tlw->usertxtlist;
	    tlw->maxnum = tlw->num_usertxtlist;
	    break;
	default:
	    my_assert(0);
	}
	...
	tlw->maxoffset = 0;
    tlw->txt_xoffset = 0;
    tlw->txt_yoffset = tlw->b_up->ysize;
    tlw->offset = next_txtno(tlw, -1, 1);
    tlw->curtxtno = -1;
    if (tlw->marked_txts)
	FREE(tlw->marked_txts);
    checkmem(tlw->marked_txts =
	     CALLOC(sizeof(unsigned char), tlw->maxnum));
    if (rdl_curtxt == 0
	&& (tlw->type == txt2_normal || tlw->type == txt2_wall))
	i = 0;
    else
	for (i = 0; i < tlw->maxnum; i++)
	    if (tlw->t[i] == &pig.rdl_txts[rdl_curtxt])
		break;
    if (i < tlw->maxnum) {
	tlw->marked_txts[i] = 1;
	tlw->offset =
	    next_txtno(tlw,
		       i - tl_xnumtxt(tlw) * tl_ynumtxt(tlw) / 2 <
		       0 ? -1 : i - tl_xnumtxt(tlw) * tl_ynumtxt(tlw) / 2,
		       1);
	tlw->curtxtno = i;
    }
}
...
void tl_drawmarktxt(struct w_button *b, int on)
{
    ...
    ws_drawbox(w_xwinincoord(tlw->win, b->xpos - 1),
	       w_ywinincoord(tlw->win, b->ypos - 1), b->xsize + 2,
	       b->ysize + 2,
...
void tl_refreshtxts(struct txt_list_win *tlw)
{
	...
	for (x = 0; x < tl_xnumtxt(tlw); x++) {
		...
		if (i < tlw->maxnum) {
		    ...
		    for (x2 = 0; x2 < 32; x2++)
			for (y2 = 0; y2 < 32; y2++)
			    txthalf[y2 * 32 + x2] = texture[y2 * 2 * TXT_REALSIZE + x2 * 2];
		    ws_translatepixels(txthalf, tlw->txt_buffer[y*tl_xnumtxt(tlw)+x], size * size);
		} else {
...
//          tlw->txt_buffer[y*tl_xnumtxt(tlw)+x][y2*16+x2]=
			    txtquarter[y2 * 16 + x2] = texture[y2 * 4 * TXT_REALSIZE + x2 * 4];
		    ws_translatepixels(txtquarter, tlw->txt_buffer[y*tl_xnumtxt(tlw)+x], size * size);
		} else {
...
void tl_refresh(struct w_window *w, void *d)
{
    ...
    case tlw_thing:
	if (!view.pcurrthing
	    || (view.pcurrthing->d.t->type1 != tt1_robot
	...
	if (tlw->b_texture != NULL) {
		...
		for (x = 0; x < tlw->oldxnumtxt; x++) {
		    w_deletebutton(tlw->
				   b_texture[y * tlw->oldxnumtxt + x]);
		    // this mem belongs to sdl surface and is now freed by itself
		    //FREE(tlw->txt_buffer[y*tlw->oldxnumtxt+x]);
		    ws_freebitmap(tlw->
				  txt_bitmap[y * tlw->oldxnumtxt + x]);
		}
	    FREE(tlw->txt_buffer);
	    FREE(tlw->txt_bitmap);
	    FREE(tlw->b_texture);
	...
	if (tl_xtxtspace(tlw) % tlw->txt_xsize != 0 ||
	    tl_ytxtspace(tlw) % tlw->txt_ysize != 0
	    || tlw->oldmode != tlw->mode.selected) {
	    w->xsize += tlw->txt_xsize / 2;
	    w->ysize += tlw->txt_ysize / 2;
	    x = tl_xnumtxt(tlw);
	    if (x * tlw->txt_xsize < 2 * TXT_XSIZE)
		x = (TXT_XSIZE + tlw->txt_xsize - 1) / tlw->txt_xsize * 2;
	    x *= tlw->txt_xsize;
	    if (x > w_xmaxwininsize(tlw->win))
		x = (w_xmaxwininsize(tlw->win) / tlw->txt_xsize) *
		    tlw->txt_xsize;
	    y = tl_ynumtxt(tlw);
		...
		y = ((w_ymaxwininsize(tlw->win) -
		      tlw->b_up->ysize * 2) / tlw->txt_ysize) *
		    tlw->txt_ysize + tlw->b_up->ysize * 2;
	    tlw->b_up->xsize = x - tlw->b_zoom->xpos - tlw->b_zoom->xsize;
	    tlw->b_down->xsize =
		x - tlw->b_display->xpos - tlw->b_display->xsize;
	    tlw->b_m_down->xsize =
		x - tlw->b_m_save->xpos - tlw->b_m_save->xsize;
	    tlw->b_down->ypos = tlw->b_display->ypos =
		tlw->b_m_clear->ypos = tlw->b_m_load->ypos =
		tlw->b_m_save->ypos = tlw->b_m_level->ypos =
		tlw->b_m_down->ypos = y - tlw->b_down->ysize;
	    tlw->dontcalltwice = 1;
	    w_resizeinwin(w, x, y);
	    tlw->dontcalltwice = 0;
	}
	checkmem(tlw->b_texture =
		 MALLOC(tl_xnumtxt(tlw) * tl_ynumtxt(tlw) *
			sizeof(struct w_button *)));
	checkmem(tlw->txt_buffer =
		 MALLOC(tl_xnumtxt(tlw) * tl_ynumtxt(tlw) *
			sizeof(char *)));
	checkmem(tlw->txt_bitmap =
		 MALLOC(tl_xnumtxt(tlw) * tl_ynumtxt(tlw) *
	...
	for (y = 0; y < tl_ynumtxt(tlw); y++)
		...
		checkmem(tlw->txt_bitmap[y * tl_xnumtxt(tlw) + x] =
			 ws_createbitmap(tlw->txt_xsize - TXT_EDGE * 2,
					 tlw->txt_ysize - TXT_EDGE * 2,
					 &(tlw->
					   txt_buffer[y * tl_xnumtxt(tlw) +
						      x])));
		checkmem(tlw->b_texture[y * tl_xnumtxt(tlw) + x] =
			 w_addimagebutton(tlw->win, w_b_press,
					  x * tlw->txt_xsize +
					  tlw->txt_xoffset + TXT_EDGE - 1,
					  y * tlw->txt_ysize +
					  tlw->txt_yoffset + TXT_EDGE - 1,
					  -1, -1,
					  tlw->txt_bitmap[y *
							  tl_xnumtxt(tlw) +
							  x],
	...
	initdisplay(tlw);
	tlw->oldxsize = w->xsize;
	tlw->oldysize = w->ysize;
	tlw->oldxnumtxt = tl_xnumtxt(tlw);
	tlw->oldynumtxt = tl_ynumtxt(tlw);
	tlw->oldmode = tlw->mode.selected;
    }
    tlw->oldxpos = w_xwinspacecoord(w->xpos);
    tlw->oldypos = w_ywinspacecoord(w->ypos);
    w_drawbutton(tlw->b_up);
    w_drawbutton(tlw->b_mode);
    w_drawbutton(tlw->b_zoom);
    if (tlw->mode.selected == 0) {
	w_drawbutton(tlw->b_display);
	w_drawbutton(tlw->b_down);
    } else {
	w_drawbutton(tlw->b_m_clear);
	w_drawbutton(tlw->b_m_load);
	w_drawbutton(tlw->b_m_save);
	w_drawbutton(tlw->b_m_level);
...
void tl_changeoffset(struct w_button *b, int amount)
{
    ...
    switch (amount) {
    case -1:
	add = tl_ynumtxt(tlw) > 1 ? -tl_xnumtxt(tlw) : -1;
	break;
    case -2:
	add = -tl_xnumtxt(tlw) * (tl_ynumtxt(tlw) / 2 + 1);
	break;
    case 1:
	add = tl_ynumtxt(tlw) > 1 ? tl_xnumtxt(tlw) : 1;
	break;
...
void tl_makelvllist(struct txt_list_win *tlw)
{
    ...
    FREE(tlw->leveltxtlist);
    tlw->leveltxtlist = NULL;
    tlw->num_leveltxtlist = 0;
    switch (tlw->type) {
    case tlw_t1:
    case tlw_t2:
	if (tlw->type == tlw_t2) {
	...
	for (n = l->cubes.head; n->next != NULL; n = n->next)
		...
		if (n->d.c->walls[w]) {
		    rdlno =
			tlw->type ==
			tlw_t1 ? n->d.c->walls[w]->texture1 : n->d.c->
			walls[w]->texture2;
		    if ((rdlno == 0 && tlw->type == tlw_t2) || rdlno < 0
			|| rdlno >= pig.num_rdltxts
			|| !pig.rdl_txts[rdlno].pig)
			continue;
	...
	for (n = l->things.head; n->next != NULL; n = n->next) {
		...
		rdlno = ((struct D2_robot *) n->d.t)->r.texture;
	    else if (n->d.t->type1 == tt1_reactor)
		rdlno = ((struct reactor *) n->d.t)->r.texture;
	    else
		continue;
	    if (rdlno < 0 || rdlno >= pig.num_rdltxts
		|| !pig.rdl_txts[rdlno].pig)
		continue;
	    INSERTINLVLTXTLIST(rdlno);
	}
	...
	for (n = l->doors.head; n->next != NULL; n = n->next) {
	    if (n->d.d->animtxt >= pig.num_anims
		|| !pig.anims[n->d.d->animtxt])
		continue;
    ...
    if (tlw->leveltxtlist)
	qsort(tlw->leveltxtlist, tlw->num_leveltxtlist,
	      sizeof(struct ham_txt *), cmp_txts);
}

void tl_l_up(struct w_button *b)
{
    tl_changeoffset(b, -1);
}

void tl_r_up(struct w_button *b)
{
    tl_changeoffset(b, -2);
}

void tl_l_down(struct w_button *b)
{
    tl_changeoffset(b, 1);
}

void tl_r_down(struct w_button *b)
{
    tl_changeoffset(b, 2);
}
...
void tl_change_texture(struct w_button *b, int withtagged)
{
	...
	if (tlw->type == tlw_anim) {
		...
		if (pig.anim_starts[animnum] == tlw->t[txt_num]->rdlno) {
		    b_changetexture(tlw->i, animnum, MWT(b, withtagged));
		    break;
		}
	} else
	    b_changetexture(tlw->i, tlw->t[txt_num]->rdlno,
			    MWT(b, withtagged));
	w_refreshend(optionwins[tlw->i->infonr]);
	tlw->marked_txts[tlw->curtxtno] = 0;
	for (j = 0, i = tlw->offset; j < tl_xnumtxt(tlw) * tl_ynumtxt(tlw);
	     j++, i = next_txtno(tlw, i, 1))
	    if (i == tlw->curtxtno) {
		tl_drawmarktxt(tlw->b_texture[j], 0);
		break;
	    }
	tlw->curtxtno = txt_num;
	tl_drawmarktxt(b, 1);
	tlw->marked_txts[tlw->curtxtno] = 1;
    }
}

void tl_l_texture(struct w_button *b)
{
    tl_change_texture(b, 0);
}

void tl_r_texture(struct w_button *b)
{
    tl_change_texture(b, 1);
}
...
unsigned short tl_readfile(unsigned short **data)
{
    ...
    fname =
	getfilename(&init.txtlistpath, NULL,
		    init.d_ver < d2_10_sw ? "TL1" : "TL2",
		    TXT_TLW_TITLELOAD, 0);
    if (fname == NULL)
	return 0;
    if ((f = fopen(fname, "rb")) == NULL) {
	printmsg(TXT_TLW_CANTLOADTL, fname);
	return 0;
    }
...
void tl_loadlist(struct txt_list_win *tlw)
{
	...
	if (data[i] < pig.num_txtlist[txt_all])
	    ...
	    checkmem(tlw->usertxtlist[i] = REALLOC(tlw->usertxtlist,
						   sizeof(struct ham_txt *)
						   *
						   (--tlw->
						    num_usertxtlist)));
...
void tl_loadmarkers(struct txt_list_win *tlw)
{
	...
	tlw->marked_txts[i] = 0;
    for (i = 0; i < size; i++)
	if (data[i] < tlw->maxnum)
	    tlw->marked_txts[data[i]] = 1;
    FREE(data);
}
...
void tl_savemarkers(struct txt_list_win *tlw)
{
    ...
    char *fname;
    FILE *f;
    for (i = 0, size = 0; i < tlw->maxnum; i++)
	size += tlw->marked_txts[i];
    if (size == 0) {
	printmsg(TXT_TLW_TXTLISTEMPTY);
	return;
    }
    fname =
	getfilename(&init.txtlistpath, NULL,
		    ...
		    TXT_TLW_TITLESAVE, 1);
    if (fname == NULL) {
	printmsg(TXT_TLW_TXTLISTNOTSAVED);
	return;
    }
    if ((f = fopen(fname, "wb")) == NULL) {
	printmsg(TXT_TLW_CANTSAVETL, fname);
	return;
    }
...
void tl_selectzoom(struct w_button *b)
{
    ...
    switch (b->d.ls->selected) {
    case 0:
	tlw->txt_xsize = tlw->txt_ysize = TXT_XSIZE;
	break;
    case 1:
	tlw->txt_xsize = tlw->txt_ysize = TXT_REALSIZE / 2 + 2 * TXT_EDGE;
	break;
    case 2:
	tlw->txt_xsize = tlw->txt_ysize = TXT_REALSIZE / 4 + 2 * TXT_EDGE;
	break;
    default:
	my_assert(0);
    }
...
void tl_marklvllist(struct txt_list_win *tlw)
{
	...
	for (n = l->cubes.head; n->next != NULL; n = n->next)
		...
		if (n->d.c->walls[w]) {
			...
			tlw_t1 ? n->d.c->walls[w]->texture1 : n->d.c->
			walls[w]->texture2;
		    if ((tlw->type == tlw_t2 && rdlno == 0) || rdlno < 0
			|| rdlno >= pig.num_rdltxts
			|| !pig.rdl_txts[rdlno].pig)
			continue;
		    MARKTXT(rdlno);
...
void tl_m_clear(struct w_button *b)
{
    tl_makelist(b->data, 0);
}

void tl_m_level(struct w_button *b)
{
    tl_makelist(b->data, 1);
}

void tl_m_load(struct w_button *b)
{
    tl_makelist(b->data, 2);
}

void tl_m_save(struct w_button *b)
{
    tl_makelist(b->data, 3);
}
...
void tl_changemode(struct txt_list_win *tlw, int mode)
{
	...
	w_undrawbutton(tlw->b_m_clear);
	w_undrawbutton(tlw->b_m_load);
	w_undrawbutton(tlw->b_m_save);
	w_undrawbutton(tlw->b_m_level);
	w_undrawbutton(tlw->b_m_down);
	break;
    case 1:			/* make texture list */
	tlw->texture.l_routine = tl_l_marktxt;
	tlw->texture.r_routine = tl_r_marktxt;
	w_undrawbutton(tlw->b_display);
	w_undrawbutton(tlw->b_down);
	break;
    default:
	my_assert(0);
    }
...
void init_texturehandling(enum tlw_types tlw_type)
{
    ...
    tl_win[tlw_type].up.delay = 25;
    tl_win[tlw_type].up.repeat = 10;
    ...
    tl_win[tlw_type].down.delay = 25;
    tl_win[tlw_type].down.repeat = 10;
    ...
    tl_win[tlw_type].texture.delay = 0;
    tl_win[tlw_type].texture.repeat = -1;
    ...
    tl_win[tlw_type].m_clear.delay = 0;
    tl_win[tlw_type].m_clear.repeat = -1;
...
void texture_list(struct infoitem *i, enum txttypes tt, int no)
{
	...
	tl_wininit.help = NULL;
	tl_wininit.buttons = wb_close | wb_shrink | wb_drag | wb_size;
	tl_wininit.refresh = wr_routine;
	tl_wininit.data = &tl_win[tlw_type];
	tl_wininit.refresh_routine = tl_refresh;
	tl_wininit.close_routine = tl_close;
	tl_wininit.click_routine = NULL;
	tl_win[tlw_type].win = w_openwindow(&tl_wininit);
	checkmem(tl_win[tlw_type].b_mode =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_choose, 0, 0, -1,
				-1, TXT_TLW_MODE, &tl_win[tlw_type].mode,
				1));
	checkmem(tl_win[tlw_type].b_zoom =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_choose,
				tl_win[tlw_type].b_mode->xsize, 0, -1, -1,
				NULL, &tl_win[tlw_type].zoom, 1));
	checkmem(tl_win[tlw_type].b_up =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_zoom->xpos +
				tl_win[tlw_type].b_zoom->xsize, 0,
				w_xwininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_zoom->xpos -
				tl_win[tlw_type].b_zoom->xsize,
				tl_win[tlw_type].b_mode->ysize, TXT_TLW_UP,
				&tl_win[tlw_type].up, 1));
	checkmem(tl_win[tlw_type].b_display =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_choose, 0,
				w_ywininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_mode->ysize - 1, -1,
				tl_win[tlw_type].b_mode->ysize,
				TXT_TLW_DISPLAY, &tl_win[tlw_type].display,
				1));
	checkmem(tl_win[tlw_type].b_m_clear =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press, 0,
				w_ywininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_mode->ysize - 1, -1,
				tl_win[tlw_type].b_mode->ysize,
				TXT_TLW_CLEAR, &tl_win[tlw_type].m_clear,
				0));
	checkmem(tl_win[tlw_type].b_m_level =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_m_clear->xpos +
				tl_win[tlw_type].b_m_clear->xsize,
				w_ywininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_mode->ysize - 1, -1,
				tl_win[tlw_type].b_mode->ysize,
				TXT_TLW_LEVEL, &tl_win[tlw_type].m_level,
				0));
	checkmem(tl_win[tlw_type].b_m_load =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_m_level->xpos +
				tl_win[tlw_type].b_m_level->xsize,
				w_ywininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_mode->ysize - 1, -1,
				tl_win[tlw_type].b_mode->ysize,
				TXT_TLW_LOAD, &tl_win[tlw_type].m_load,
				0));
	checkmem(tl_win[tlw_type].b_m_save =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_m_load->xpos +
				tl_win[tlw_type].b_m_load->xsize,
				w_ywininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_mode->ysize - 1, -1,
				tl_win[tlw_type].b_mode->ysize,
				TXT_TLW_SAVE, &tl_win[tlw_type].m_save,
				0));
	checkmem(tl_win[tlw_type].b_m_down =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_m_save->xpos +
				tl_win[tlw_type].b_m_save->xsize,
				tl_win[tlw_type].b_display->ypos,
				w_xwininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_m_save->xpos -
				tl_win[tlw_type].b_m_save->xsize,
				tl_win[tlw_type].b_display->ysize,
				TXT_TLW_DOWN, &tl_win[tlw_type].down, 0));
	checkmem(tl_win[tlw_type].b_down =
		 w_addstdbutton(tl_win[tlw_type].win, w_b_press,
				tl_win[tlw_type].b_display->xsize,
				tl_win[tlw_type].b_display->ypos,
				w_xwininsize(tl_win[tlw_type].win) -
				tl_win[tlw_type].b_display->xsize,
				tl_win[tlw_type].b_display->ysize,
	...
	tl_win[tlw_type].b_mode->data = tl_win[tlw_type].b_up->data =
	    tl_win[tlw_type].b_display->data =
	    tl_win[tlw_type].b_down->data = tl_win[tlw_type].b_zoom->data =
	    tl_win[tlw_type].b_m_clear->data =
	    tl_win[tlw_type].b_m_load->data =
	    tl_win[tlw_type].b_m_save->data =
	    tl_win[tlw_type].b_m_level->data =
	    tl_win[tlw_type].b_m_down->data = &tl_win[tlw_type];
...
void refresh_txtlist(struct infoitem *i)
{
	...
	tlw->oldtxttyp == newtxttyp)
	return;
    tlw->olddisplay = -1;
    tlw->oldtxttyp = newtxttyp;
    initdisplay(tlw);
    tl_refresh(tlw->win, tlw);
}
...
void b_changetexture(struct infoitem *i, long no, int withtagged)
{
    ...
    if (withtagged < 2)
	changedata(i, withtagged, &no);
    else
	tagfilter(i, withtagged - 2, &no);
    if (i->type == it_texture || i->type == it_thingtexture)
	drawoptbuttons(i - i->multifuncnr + 5);
    if (i->type == it_texture)
	drawoptbuttons(i - i->multifuncnr + 10);
...
void b_selecttexture(struct w_button *b, int withtagged)
{
    ...
    if (i->infonr == ds_thing) {
	tt = txt_thing;
	tlw = &tl_win[tlw_thing];
    } else {
	if (i->multifuncnr > 5) {
	    tt = view.pcurrcube->d.c->nc[view.currwall] !=
		NULL ? txt2_wall : txt2_normal;
	    tlw = &tl_win[tlw_t2];
	} else {
	    tt = view.pcurrcube->d.c->nc[view.currwall] !=
		NULL ? txt1_wall : txt1_normal;
	    tlw = &tl_win[tlw_t1];
	}
    }
    if (tlw->t == NULL) {
	...
	for (j = 0, txtno = -10; j < tlw->maxnum; j++)
	    if (tlw->t[j] == &pig.rdl_txts[rdlno]) {
		txtno = j;
		break;
	    }
    if (txtno == -10)
	txtno = (rdlno >= 0 && rdlno < pig.num_rdltxts
		 && pig.rdl_txts[rdlno].pig != NULL
		 && pig.rdl_txts[rdlno].txtlistno[tt] >=
		 0) ? pig.rdl_txts[rdlno].txtlistno[tt] : 0;
    switch (i->multifuncnr) {
    case 1:
    case 6:
	txtno = next_txtno(tlw, -1, 1);
	break;
    case 2:
    case 7:
	txtno = next_txtno(tlw, txtno, -1);
	if (txtno < 0)
	    txtno = next_txtno(tlw, tlw->maxnum, -1);
	break;
    case 3:
    case 8:
	txtno = next_txtno(tlw, txtno, 1);
...
int bi_gettxtno(struct infoitem *i)
{
    ...
    if (sscanf(i->b->d.str->str, "%*[^(]( %ld", &no) == 1)
	return no;
    strncpy(buffer, i->b->d.str->str, 8);
    buffer[8] = 0;
	...
	if (view.pcurrcube->d.c->nc[view.currwall] != NULL)
	    ...
	    tt = i->multifuncnr > 5 ? txt2_normal : txt1_normal;
    }
    for (n = 0; n < pig.num_txtlist[tt]; n++)
	if (pig.txtlist[tt][n]->pig &&
	    compstrs(pig.txtlist[tt][n]->pig->name, buffer) >= 0)
	    break;
    my_assert(pig.
	      txtlist[tt][n <
			  pig.num_txtlist[tt] ? n : pig.num_txtlist[tt] -
			  1]
	      != NULL);
...
// ffe added - init a texture preview with single color
void init_txt_buffer (uint32_t * buffer, int size, int r, int g, int b)
{
	uint32_t color = ws_getNativeColorRGB(r, g, b);
  	int i;
	for (i=0; i < size; i++)
		buffer[i] = color;
}
...
void drawdooranim(struct w_button *b, struct ham_txt *t, int anim, int t1,  int t2, int t2_d)
{
	...
	if (t->pig && t->pig->anim_t2) {
		if (t1 < pig.num_rdltxts && pig.rdl_txts[t1].pig != NULL) {
			...
			readbitmap(tmptxtdata, NULL, &pig.rdl_txts[t1], 0);
			ws_translatepixels(tmptxtdata, pig.door_buffer, 64*64);
		}
		if (t != NULL) {
			//readbitmap(pig.door_buffer, &pig.pig_txts[t->pigno + anim], NULL, t2_d);
			readbitmap(tmptxtdata, &pig.pig_txts[t->pigno + anim], NULL, t2_d);
			ws_translatepixels(tmptxtdata, pig.door_buffer, 64*64);
		}
	} else {
		//memset(pig.door_buffer, 0xff, 64 * 64);
		init_txt_buffer(pig.door_buffer, 64*64, 255, 255, 255);
		if (t != NULL) {
			//readbitmap(pig.door_buffer, &pig.pig_txts[t->pigno + anim], NULL, 0);
			readbitmap(tmptxtdata, &pig.pig_txts[t->pigno + anim], NULL, 0);
			ws_translatepixels(tmptxtdata, pig.door_buffer, 64*64);
		}
	}
...
void b_selectdooranim(struct w_button *b, int withtagged)
{
	...
	txtno = (rdlno >= 0 && rdlno < pig.num_rdltxts
		 && pig.rdl_txts[rdlno].pig != NULL
		 && pig.rdl_txts[rdlno].txtlistno[txt_door] >=
		 0) ? pig.rdl_txts[rdlno].txtlistno[txt_door] : 0;
    switch (i->multifuncnr) {
    case 1:
	txtno = next_txtno(tlw, -1, 1);
	break;
	...
	break;
    case 4:
	texture_list(i, txt_door, txtno);
	break;
    default:
	my_assert(0);
    }
    for (n = 0; n < pig.num_anims; n++)
	if (pig.anims[n] == tlw->t[txtno])
	    break;
    if (n >= pig.num_anims)
	n = 0;
...
void b_l_playdooranim(struct w_button *b)
{
	...
	b->d.p->repeat =
	    50 / pig.anims[view.pcurrdoor->d.d->animtxt]->pig->num_anims;
	drawdooranim(b, pig.anims[view.pcurrdoor->d.d->animtxt],
		     current_anim, view.pcurrdoor->d.d->w->texture1,
		     view.pcurrdoor->d.d->w->texture2,
		     view.pcurrdoor->d.d->w->txt2_direction);
	current_anim += add;
	if (current_anim >=
	    pig.anims[view.pcurrdoor->d.d->animtxt]->pig->num_anims - 1)
	    add = -1;
	if (current_anim <= 0)
	    add = 1;
    }
...
void drawtxtlines(struct w_button *b, struct wall *w)
{
    ...
    float l;
    ws_setclipping(w_xwinincoord(b->w, b->xpos + 1),
		   w_ywinincoord(b->w, b->ypos + 1), w_xwinincoord(b->w,
								   b->
								   xpos +
								   b->
								   xsize -
    ...
    c = &w->corners[3];
    l = sqrt((c->x[0] - c0->x[0]) * (c->x[0] - c0->x[0]) +
	     (c->x[1] - c0->x[1]) * (c->x[1] - c0->x[1])) / 32;
    if (l == 0)
	return;
    for (i = 0; i < 9; i++) {
	w_drawline(b->w, b->xpos + orig_x[i], b->ypos + orig_y[i],
	...
	w_drawline(b->w, b->xpos + orig_x[i] + 3, b->ypos + orig_y[i] + 3,
		   b->xpos + orig_x[i] - 3, b->ypos + orig_y[i] - 3,
		   view.color[WHITE], 0);
	w_drawline(b->w, b->xpos + orig_x[i] - 3, b->ypos + orig_y[i] + 3,
		   b->xpos + orig_x[i] + 3, b->ypos + orig_y[i] - 3,
		   view.color[WHITE], 0);
...
static struct fitbitmap_data {
    struct w_window *w;
    int changed, autoshrunk;
    struct wall *currwall;
    //char texture[64 * 64];
    uint32_t *texture;
...
void fb_refreshwin(struct w_window *w, void *d)
{
    ...
    fb_savedata.xsize = w->xsize;
    fb_savedata.ysize = w->ysize;
    if (!view.pcurrwall) {
	if (!w->shrunk)
	    w_shrinkwin(w);
	return;
    ...
    ws_setclipping(w_xwinincoord(w, 0), w_ywinincoord(w, 0),
		   w_xwinincoord(w, 0) + w_xwininsize(w) - 1,
		   w_ywinincoord(w, 0) + w_ywininsize(w) - 1);
	...
	for (x = -xn + 1; x < xn; x++)
	    ws_copybitmap(NULL, xo + x * 64, yo + y * 64, fb_data.bm_txt,
			  0, 0, 64, 64, 1);
    for (x = 0; x < 4; x++)
	ws_drawline(xo + fb_data.c[x].x[0] / 32,
		    yo + fb_data.c[x].x[1] / 32,
		    xo + fb_data.c[(x + 1) & 3].x[0] / 32,
		    yo + fb_data.c[(x + 1) & 3].x[1] / 32,
		    view.color[HILIGHTCOLORS +
			       (x == 0 ? 3 : (x == 3 ? 4 : 1))], 0);
    if (c != NULL)
	ws_drawcircle(xo + fb_data.c[*c].x[0] / 32,
		      yo + fb_data.c[*c].x[1] / 32, 3,
...
void fb_closewin(void)
{
    ...
    if (fb_data.w->shrunk)
	w_shrinkwin(fb_data.w);
    ws_freebitmap(fb_data.bm_txt);
    fb_data.bm_txt = NULL;
    fb_savedata.xpos = w_xwinspacecoord(fb_data.w->xpos);
    fb_savedata.ypos = w_ywinspacecoord(fb_data.w->ypos);
    fb_savedata.xsize = fb_data.w->xsize;
    fb_savedata.ysize = fb_data.w->ysize;
    w_closewindow(fb_data.w);
    fb_data.w = NULL;
    fb_data.currwall = NULL;
    if (view.movemode == mt_texture)
	changemovemode(mt_you);
}

void fb_closewinbutton(struct w_window *w, void *d)
{
    if (fb_data.changed && !yesnomsg(TXT_FB_CLOSEWIN))
	return;
    fb_closewin();
}

void fb_movetxtpic(void)
{
    fb_refreshwin(fb_data.w, NULL);
}
...
void fb_clickwin(struct w_window *w, void *d, struct w_event *we)
{
    ...
    struct wall wall;
    if (we->x < 0 || we->y < 0 || we->x >= w_xwininsize(w)
	|| we->y >= w_ywininsize(w))
	return;			/* click on a system button */
    if ((we->ws->buttons & ws_bt_left) == 0) {
	move_texture_with_mouse(w_xwinincoord(w, we->x),
				w_ywinincoord(w, we->y), fb_data.c, NULL,
				0, 0, 0.0, fb_movetxtpic);
	fb_data.changed = 1;
    } else {
	...
	for (i = 0, min = 16, min_i = -1; i < 4; i++) {
	    xd = xo + fb_data.c[i].x[0] / 32 - we->ws->x;
	    yd = yo + fb_data.c[i].x[1] / 32 - we->ws->y;
	    if (xd * xd + yd * yd < min) {
		min = xd * xd + yd * yd;
		min_i = i;
	    }
	...
	if (min_i >= 0) {
	    ws_erasemouse();
	    fb_data.changed = 1;
	    ws.x = fb_data.c[min_i].x[0] / 32 + xo;
	    ws.y = fb_data.c[min_i].x[1] / 32 + yo;
	    do {
	...
	/* no pnt hit, so check for a double click */
		...
		if (ws_getevent(&ws, -view.doubleclick))
			...
			if (ws.kbstat & ws_ks_shift) {
				...
				wall = *view.pcurrwall;
				recalcwall(view.pcurrcube->d.c,
					   view.currwall);
				for (i = 0; i < 4; i++)
				    fb_data.c[i] =
					view.pcurrwall->corners[i];
				fb_data.changed = 0;
				*view.pcurrwall = wall;
			    }
...
void fb_initwin(void)
{
	...
	//ws_translatepixels(pig.txt_buffer, fb_data.texture, 64*64);

    memcpy(fb_data.texture, pig.txt_buffer, 64 * 64 * sizeof(uint32_t));
    ...
    cy /= 4;
    for (c = 0; c < 4; c++) {
	fb_data.c[c].x[0] -= (cx / 2048) * 2048;
	fb_data.c[c].x[1] -= (cy / 2048) * 2048;
    }
    if (fb_data.w == NULL) {	/* open the window */
	if (fb_savedata.xsize == 0) {
	    wi.xpos = 0;
	    wi.ypos = 0;
	    wi.xsize = w_xmaxwinsize() / 2;
	    wi.ysize = w_ymaxwinsize() / 2;
	} else {
	    wi.xpos = fb_savedata.xpos;
	    wi.ypos = fb_savedata.ypos;
	    wi.xsize = fb_savedata.xsize;
	    wi.ysize = fb_savedata.ysize;
	}
	wi.maxxsize = wi.maxysize = -1;
	wi.shrunk = 0;
	wi.title = TXT_FB_TITLE;
	wi.hotkey = -1;
	wi.help = NULL;
	wi.buttons = wb_drag | wb_size | wb_close | wb_shrink;
	wi.refresh = wr_routine;
	wi.data = NULL;
	wi.refresh_routine = fb_refreshwin;
	wi.close_routine = fb_closewinbutton;
	wi.click_routine = fb_clickwin;
...
void b_fitbitmap(struct w_button *b, int withtagged)
{
    struct node *n;
    int c;
    if (view.pcurrcube == NULL || view.pcurrwall == NULL || l == NULL)
	return;
...
{
    int c;
    if (!fb_data.w || fb_data.currwall == view.pcurrwall)
	return;
...
{
    if (fb_data.w == NULL || fb_data.w->shrunk)
	return;
    switch (axis) {
    case 0:
	move_texture(fb_data.c, dir * 32, 0);
	break;
    case 1:
	move_texture(fb_data.c, 0, -dir * 32);
	break;
    case 2:
	stretch_texture(fb_data.c, -dir, -dir);
	break;
    default:
	my_assert(0);
    }
...
void fb_turn_texture(int x, int y, int z, int dir)
{
    ...
    switch (x) {
    case 0:
	rotate_texture(fb_data.c, dir * view.protangle);
	break;
    case 1:
	stretch_texture(fb_data.c, 0, -dir);
	break;
    case 2:
	stretch_texture(fb_data.c, dir, 0);
	break;
    }
    fb_movetxtpic();
}
...
{
    return fb_data.w != NULL && !fb_data.w->shrunk
	&& view.pcurrwall != NULL;
}
