...
void dec_enlargeshrink(int ec) {
    ...
    if (view.currmode != tt_cube && view.currmode != tt_wall) {
        printmsg(TXT_NOSHRINKMODE, init.bnames[view.currmode]);
        return;
    }
    if (l->tagged[view.currmode].size == 0) {
        printmsg(TXT_NOTHINGTAGGED, init.bnames[view.currmode]);
        return;
    }
    for (n = l->tagged[view.currmode].head; n->next != NULL; n = n->next) {
        ...
        if (view.currmode == tt_cube) {
            ...
            cubepnts[0] = 4;
            for (i = 0; i < 4; i++) cubepnts[i + 1] = wallpts[n->no % 6][i];
        }
...
void dec_setexit(int ec) {
    if (!view.pcurrcube) {
        printmsg(TXT_NOCURRCUBE);
        return;
    }
    if (view.pcurrcube->d.c->nc[view.currwall] != NULL) {
        printmsg(TXT_EXITNONEIGHBOUR);
        return;
    }
    l->levelsaved = 0;
    l->exitcube = view.pcurrcube;
    l->exitwall = view.currwall;
    plotlevel();
}
...
void dec_aligntxts(int ec) {
    ...
    if (!view.pcurrwall) {
        printmsg(TXT_NOCURRSIDE);
        return;
    }
    if (l->tagged[tt_wall].size == 0) {
        printmsg(TXT_NOTHINGTAGGED, init.bnames[tt_wall]);
        return;
    }
    l->levelsaved = 0;
...
void ed_initdatastr(int ds, char *offset, char *str) {
    ...
    memdata = getdata(ds, NULL);
    offnum = strtol(offset, &pos, 0);
    *pos = 0;
    if (!memdata) {
        strcpy(str, "");
        return;
...
void ed_dataentered(struct w_button *b) {
    ...
    if (ds > 4) ds += 2; /* Internal and Leveldata */
    memdata = getdata(ds, NULL);
    offnum = strtol(bs[1]->d.str->str, NULL, 0);
...
void dec_enterdata(int ec) {
    ...
    whichstruct.d_xsize = -1;
    whichstruct.num_options = 7;
    whichstruct.options = structnames;
    whichstruct.selected = save_struct;
...
void init_ce_coords(struct point *m, float twist, struct point *ref_coords,
    ...
    if (l >= 0.1) for (i = 0; i < 3; i++) e[0].x[i] /= l;
        ...
        VECTOR(&e[0], &ref_coords[0], &coords[2]);
        normalize(&e[0]);
    }
    ...
    ref_coords[0] = coords[0];
    ref_coords[1] = coords[1];
...
struct node *recalc_correlem(struct corridor *c, struct node *n,
    ...
    int i, j;
    if (n->next == NULL) {
        if (!new_cube_layer(c)) return NULL;
        n = c->elements.tail;
...
float corr_get_twist(struct track *t, struct point *ref_coords) {
    ...
    float twist;
    coords[0] = ref_coords[0];
    coords[1] = ref_coords[1];
...
struct node *recalc_track(struct corridor *c, struct track *st,
    ...
    initlist(&curve);
    ref_coords[0] = st->coords[0];
    ref_coords[1] = st->coords[1];
    ...
    for (tn = curve.head->next, twist = 0.0; tn != NULL; tn = tn->next, twist += d_twist) {
        ...
        if (tn->prev->no > 0) d_twist /= 2;
        else if (tn->prev->no < 0) d_twist *= 2;
        FREE(tn->prev->d.p);
        killnode(&curve, tn->prev);
    }
    ...
    switch (c->b_stdform->d.ls->selected) {
        ...
        case 2:
            ...
            if (ct[4]->fixed) /* ->t=mt1 */ {
                ...
                for (i = 0; i < 3; i++) {
                    ct[0]->x.x[i] += r.x[i];
                    ct[1]->x.x[i] += r.x[i] / 2.0;
                    ct[4]->x.x[i] += r.x[i] / 2.0;
                }
            }
            ok = recalc_corridor(c);
            if (!ok) {
                for (i = 0; i < 5; i++) *ct[i] = oldt[i];
                recalc_corridor(c);
            }
            break;
    }
...
int turncorr(struct point *t_axis, float fx, float fy, int with_left,
    ...
    plotcorridor(c);
    oldx = t->x;
    for (i = 0; i < 3; i++) coords[i] = t->coords[i];
    ...
    if (!ok) {
        t->x = oldx;
        for (i = 0; i < 3; i++) t->coords[i] = coords[i];
        recalc_corridor(c);
    }
    plotcorridor(c);
    return ok;
...
void change_cube_list(struct node *n, struct list *lc1, struct list *lp1,
    ...
    listnode_tail(lc2, n);
    for (i = 0; i < 8; i++) {
        unlistnode(lp1, n->d.c->p[i]);
        listnode_tail(lp2, n->d.c->p[i]);
    }
}
...
int make_start_corridor(struct corridor *c) {
    ...
    add_correlem(c, c->cubes.head, 1);
    add_correlem(c, c->cubes.head, 0);
...
void delete_corridor(struct corridor *c) {
    ...
    for (n = c->elements.head->next; n != NULL; n = n->next) {
        FREE(n->prev->d.ce->pnts);
        FREE(n->prev->d.ce->rel_pnts);
        FREE(n->prev->d.ce->cubes);
        FREE(n->prev->d.ce);
        FREE(n->prev);
    }
...
void finish_corridor(struct corridor *c) {
    ...
    for (n = c->cubes.head->next; n != NULL; n = n->next) {
        unlistnode(&c->cubes, np = n->prev);
        listnode_tail(&l->cubes, np);
    }
    for (n = c->points.head->next; n != NULL; n = n->next) {
        unlistnode(&c->points, np = n->prev);
...
void corr_setend(struct w_button *b) {
    ...
    end = l->cur_corr->tracking.tail->d.ct;
    end->old_twist = 0.0;
    oldend = *end;
    for (j = 0; j < 3; j++) {
        end->x.x[j] = 0.0;
...
void corr_form(struct w_button *b) {
    ...
    switch (b->d.ls->selected) {
        case 0: /* user defined */
            w_activatebutton(c->b_ins);
            w_drawbutton(c->b_ins);
            w_activatebutton(c->b_del);
            w_drawbutton(c->b_del);
            w_activatebutton(c->b_setend);
            w_drawbutton(c->b_setend);
            for (n = c->tracking.head->next; n->next != NULL; n = n->next) n->d.ct->fixed = 0;
            st = c->tracking.head->d.ct;
            et = c->tracking.tail->d.ct;
            st->fixed = -1;
            ...
            break;
        case 1: /* straight */
            w_undrawbutton(c->b_ins);
            w_deactivatebutton(c->b_ins);
            w_undrawbutton(c->b_del);
            w_deactivatebutton(c->b_del);
            w_undrawbutton(c->b_setend);
            w_deactivatebutton(c->b_setend);
            if (c->tracking.size > 2)
                for (n = c->tracking.head->next->next; n->next != NULL; n = n->next) {
                    FREE(n->prev->d.ct);
                    killnode(&c->tracking, n->prev);
                }
            st = c->tracking.head->d.ct;
            et = c->tracking.tail->d.ct;
...
void init_corr_window(struct corridor *c, const char *levname) {
    ...
    if (view.whichdisplay)
        if (corr_win2_xpos < 0) {
            ...
            init_corr_win.xpos = corr_win2_xpos;
            init_corr_win.ypos = corr_win2_ypos;
        } else
        if (corr_win1_xpos < 0) {
...
void dec_makecorridor(int ec) {
    ...
    untagall(tt_cube);
    tag(tt_wall, view.pcurrcube, view.currwall);
    ...
    c->num_cubes = 0;
    c->front_wall = NULL;
    c->startcube = view.pcurrcube;
    c->startwall = view.currwall;
    c->depth = 20.0 * 65536.0;
    if (!make_start_corridor(c)) {
        FREE(c);
        return;
    }
    initlist(&c->tracking);
    ...
    st->fixed = -1;
    st->x = c->elements.head->d.ce->center;
    et->fixed = 0;
    et->x = c->elements.tail->d.ce->center;
    addnode(&c->tracking, -1, st);
    addnode(&c->tracking, -1, et);
    for (i = 0; i < 3; i++) {
    ...
    normalize(&st->coords[2]);
    et->coords[2] = st->coords[2];
    st->length = 0.0;
    st->l_c2 = et->l_c2 = et->length;
    normalize(&st->coords[0]);
    VECTOR(&st->coords[1], &st->coords[2], &st->coords[0]);
    for (i = 0; i < 3; i++) {
        c->e[i] = et->coords[i] = st->coords[i];
    }
    /*
    ref_coords[0] = st->coords[0];
    ref_coords[1] = st->coords[1];
    ...
    for (n = l->tagged[tt_edge].head->next; n != NULL; n = n->next) {
        for (i = 0; i < 4; i++)
            if (i != (n->prev->no % 24) % 4 && testtag(tt_edge, n->prev->d.n,
                    (n->prev->no % 24) / 4, i))
                break;
        if (i == 4) { /* only one edge is tagged on this side. move it into the plane of the
        three others. */
            for (j = 0; j < 4; j++)
