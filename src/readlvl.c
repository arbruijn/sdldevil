...
0, 0, 0}, 0, 0, 0x2a, 0, 0, 0x14, 0, 0, 0x6, 0, 0, 0x8};

uint32_t sizeplayer = 0;
...
int findtxt(int old_txt)
{
	...
	if (init.convtxts[i * 2] == old_txt)
	    return init.convtxts[i * 2 + 1];
    return -1;
}
...
unsigned char findanim(unsigned old_anim)
{
	...
	if (init.convanims[i * 2] == old_anim)
	    return init.convanims[i * 2 + 1];
    return 0xff;
}
...
void convert_textures(struct leveldata *ld)
{
	...
	if ((new_anim = findanim(nc->d.d->animtxt)) != 0xff)
...
int D1_REG_readlvldata(FILE * lf, struct leveldata *ld, int version)
{
	...
	if ((d = D1_REG_readcube(lf)) == NULL)
	    return 1;		/* try to init level */
...
void makelightsources(struct leveldata *ld, struct turnoff *to, int tonum,
		      struct changedlight *cl, int clnum,
		      struct flickering_light *fl, int flnum)
{
    int nto, ncl, i;
	...
	return;
    checkmem(cubelse =
	     MALLOC(sizeof(struct ls_effect *) * ld->cubes.size));
    checkmem(cubes = MALLOC(sizeof(struct node *) * ld->cubes.size));
    for (n = ld->cubes.head, i = 0; n->next != NULL && i < ld->cubes.size;
	 n = n->next, i++) {
	cubes[i] = n;
	cubelse[i] = NULL;
    }
	...
	if (to[nto].cube >= ld->cubes.size || to[nto].side >= 6 ||
	    cubes[to[nto].cube]->d.c->walls[to[nto].side] == NULL)
	    continue;
	...
	initlist(&ls->effects);
	ls->fl = NULL;
	checkmem(ls->cube->d.c->walls[(int) ls->w]->ls =
		 addnode(&ld->lightsources, -1, ls));
	for (ncl = to[nto].offset;
	     ncl < to[nto].offset + to[nto].num_changed && ncl < clnum;
	     ncl++) {
	    if (cl[ncl].cube >= ld->cubes.size || cl[ncl].side >= 6)
		continue;
	...
	for (ncl = to[nto].offset;
	     ncl < to[nto].offset + to[nto].num_changed && ncl < clnum;
	     ncl++)
	    if (cl[ncl].cube < ld->cubes.size)
		cubelse[cl[ncl].cube] = NULL;
    }
    if (fl != NULL)
	for (i = 0; i < flnum; i++) {
	    if (fl[i].cube < 0 || fl[i].cube >= ld->cubes.size ||
		cubes[fl[i].cube]->d.c->walls[fl[i].wall] == NULL ||
		cubes[fl[i].cube]->d.c->walls[fl[i].wall]->ls == NULL)
		continue;
	    checkmem(cubes[fl[i].cube]->d.c->walls[fl[i].wall]->ls->d.ls->
		     fl = nfl = MALLOC(sizeof(struct flickering_light)));
	    *nfl = fl[i];
...
int D2_REG_readlvldata(FILE * lf, struct leveldata *ld, int version)
{
	...
	if (fread(&lfh.flickering_lights, sizeof(uint32_t), 1, lf) !=
	    1) {
	    fclose(lf);
	    return 0;
	}
	if (lfh.flickering_lights > 0) {
		...
		if ((fl = readflickeringlight(lf)) != NULL) {
		    fl_lights[j] = *fl;
		    FREE(fl);
		} else {
		    FREE(fl_lights);
	...
	lfh.flickering_lights = 0;
    if (fread(&lfh.secret_cubenum, sizeof(uint32_t), 10, lf) != 10) {
	fclose(lf);
	return 0;
    }
    for (j = 0; j < 3; j++) {
	...
	if (i == lfh.secret_cubenum)
	    ld->secretcube = n;
    }
    for (n = ld->cubes.head; n->next != NULL; n = n->next)
	if (!D2_REG_readcube2(lf, n))
	    return 1;
    fseek(lf, lfh.gamedata_offset, SEEK_SET);
    ...
    if (gd.numturnoff > 0) {
	checkmem(turnoffs =
		 MALLOC(sizeof(struct turnoff) * gd.numturnoff));
	for (j = 0; j < gd.numturnoff; j++)
	    if ((to = readturnoff(lf)) != NULL) {
		turnoffs[j] = *to;
		FREE(to);
	    } else {
		FREE(turnoffs);
	...
	for (j = 0; j < gd.numchangedlight; j++)
	    if ((cl = readchangedlight(lf)) != NULL) {
		changedlights[j] = *cl;
		FREE(cl);
	    } else {
		FREE(changedlights);
...
int readasciilevel(char *filename, struct leveldata *ld)
{
    ...
    for (n = ld->cubes.head; n->next != NULL; n = n->next)
	for (i = 0; i < 6; i++)
	    if (n->d.c->nextcubes[i] > ld->cubes.size)
		n->d.c->nextcubes[i] = 0xffff;
    initlevel(ld);
    return 1;
}
...
int initlevel(struct leveldata *ld)
{
    ...
    for (n = ld->cubes.head; n->next != NULL; n = n->next) {
	if (n->no % 10 == 0)
	    printmsg(TXT_INITCUBE, n->no, ld->cubes.size);
	switch (initcube(n)) {
	case 0:
	    n = n->prev;
	    freenode(&ld->cubes, n->next, freecube);
	    break;
	case -1:
	    return 0;
	}
    ...
    for (n = ld->doors.head; n->next != NULL; n = n->next) {
	if (n->no % 10 == 0)
	    printmsg(TXT_INITDOOR, n->no, ld->doors.size);
	if (!initdoor(n)) {
	    n = n->prev;
	    freenode(&ld->doors, n->next, freedoor);
	}
    }
    if (init.d_ver >= d2_10_sw) {
	...
	for (n = ld->lightsources.head; n->next != NULL; n = n->next)
	    if (n->d.ls->fl != NULL)
		for (n2 = n->d.ls->effects.head; n2->next != NULL;
		     n2 = n2->next)
		    checkmem(addnode
			     (&n2->d.lse->cube->d.c->fl_lights, -1,
			      n->d.ls->fl));
    for (i = 0; i < (ld->edoors ? ld->edoors->num : 0); i++) {
	if ((n = findnode(&ld->cubes, ld->edoors->cubes[i])) == NULL) {
	    waitmsg(TXT_ERROPENATEND, ld->edoors->cubes[i]);
	    continue;
	}
	if (n->d.c->d[ld->edoors->walls[i]] == NULL) {
	    waitmsg(TXT_ERROAENODOOR, ld->edoors->cubes[i],
		    ld->edoors->walls[i]);
	    continue;
	}
	n->d.c->d[ld->edoors->walls[i]]->d.d->edoor = 1;
    }
    for (n = ld->things.head, found_start = 0; n->next != NULL;
	 n = n->next) {
	t = n->d.t;
	if (t->type1 == tt1_dmstart && t->type2 == 0) {	/* starting place */
	    ...
	    for (i = 0; i < 3; i++) {
		ld->e0.x[i] = t->p[0].x[i];
		ld->e[0].x[i] = t->orientation[i];
		ld->e[1].x[i] = t->orientation[i + 3];
	    }
	    normalize(&ld->e[0]);
	    normalize(&ld->e[1]);
    ...
    ld->pcurrwall =
	(ld->pcurrcube != NULL) ? ld->pcurrcube->d.c->walls[0] : NULL;
	...
	for (i = 0; i < 3; i++) {
	    ...
	    ld->e[0].x[i] =
		ld->pcurrcube->d.c->p[1]->d.p->x[i] - ld->e0.x[i];
	    ld->e[1].x[i] =
		ld->pcurrcube->d.c->p[3]->d.p->x[i] - ld->e0.x[i];
	}
	normalize(&ld->e[0]);
	normalize(&ld->e[1]);
	VECTOR(&ld->e[2], &ld->e[0], &ld->e[1]);
	...
	if (n->prev->d.cp->c == NULL)
	    killnode(&ld->producers, n->prev);
    l = oldl;
...
void in_changecurrentlevel(struct leveldata *ld)
{
	...
	return;
    if (l != NULL) {
	l->pcurrcube = view.pcurrcube;
	l->pcurrthing = view.pcurrthing;
	l->pcurrdoor = view.pcurrdoor;
	l->pcurrwall = view.pcurrwall;
	l->pcurrpnt = view.pcurrpnt;
	l->curredge = view.curredge;
	l->currwall = view.currwall;
	l->e0 = view.e0;
	for (i = 0; i < 3; i++)
	    l->e[i] = view.e[i];
    ...
    if (ld != NULL) {
	view.pcurrcube = ld->pcurrcube;
	view.pcurrthing = ld->pcurrthing;
	view.pcurrdoor = ld->pcurrdoor;
	view.pcurrwall = ld->pcurrwall;
	view.pcurrpnt = ld->pcurrpnt;
	view.curredge = ld->curredge;
	view.currwall = ld->currwall;
	view.e0 = ld->e0;
	for (i = 0; i < 3; i++)
	    view.e[i] = ld->e[i];
    } else {
	view.pcurrthing = view.pcurrcube = view.pcurrdoor = view.pcurrpnt =
	    NULL;
	view.pcurrwall = NULL;
...
void b_clicklevel(struct w_window *w, void *d, struct w_event *we)
{
	...
	if (ld != l) {
	if (we->ws->buttons == ws_bt_none)
	    changecurrentlevel(ld);
	return;
    }
...
struct leveldata *emptylevel(void)
{
    struct leveldata *ld;
    int i;
    if ((ld = MALLOC(sizeof(struct leveldata))) == NULL)
	return NULL;
    for (i = 0; i < tt_number; i++)
	initlist(&ld->tagged[i]);
    initlist(&ld->cubes);
    initlist(&ld->things);
    initlist(&ld->doors);
    initlist(&ld->pts);
    initlist(&ld->sdoors);
    initlist(&ld->producers);
    initlist(&ld->lines);
    initlist(&ld->lightsources);
    checkmem(ld->edoors = MALLOC(sizeof(struct edoor)));
    ld->edoors->num = 0;
    for (i = 0; i < 10; i++) {
	ld->edoors->cubes[i] = ld->edoors->walls[i] = 0;
    }
    ...
    ld->exitcube = NULL;
    ld->exitwall = 0;
    ld->reactor_time = 0x1e;
    ld->reactor_strength = 0xffffffff;
    ld->levelsaved = 1;
    ld->levelillum = 0;
    ld->secretcube = ld->secretstart = NULL;
    for (i = 0; i < 9; i++)
	ld->secret_orient[i] = stdorientation[i];
    ld->pcurrpnt = ld->pcurrcube = ld->pcurrthing = ld->pcurrdoor =
	ld->rendercube = NULL;
    ld->inside = 0;
    ld->pcurrwall = NULL;
    ld->curredge = ld->currwall = 0;
    ld->n = NULL;
    ld->whichdisplay = view.whichdisplay;
    ld->cur_corr = NULL;
    for (i = 0; i < 3; i++) {
...
struct w_window plot_win = { 0, 0, 0, 0, 0, 0, 0, 0, NULL, 0, NULL,
    wb_drag | wb_size | wb_close | wb_shrink, wr_routine, NULL,
	b_plotlevel,
...
void newlevelwin(struct leveldata *ld, int shrunk)
{
	...
	if (n->d.lev->fullname
	    && !strcmp(ld->fullname, n->d.lev->fullname))
	    i++;

    if (i == 0)
	sprintf(buffer, "%s", ld->fullname);
    else
	sprintf(buffer, "%s / %d", ld->fullname, i);
    ...
    qsort(&view.b_levels->options[1], view.b_levels->num_options - 1,
	  sizeof(char *), qs_compstrs);
    plot_win.title = buffer;
    plot_win.data = ld;
	...
	plot_win.ysize = shrunk ? w_ymaxwinsize() - plot_win.xsize / 2 :
	    plot_win.xsize / 2;
	plot_win.xpos = 0;
	plot_win.ypos = shrunk ? plot_win.ysize : 0;
    } else {
	plot_win.xsize = plot_win.ysize =
	    shrunk ? w_xmaxwinsize() - w_ymaxwinsize() -
	    10 : w_ymaxwinsize();
	plot_win.xpos = shrunk ? w_ymaxwinsize() + 5 : 0;
	plot_win.ypos = shrunk && isdigit(buffer[strlen(buffer) - 1]) ?
	    (((buffer[strlen(buffer) - 1] - '0') % 5) + in_number) * 20 +
	    30 : 0;
    }
    plot_win.shrunk = shrunk;
    checkmem(ld->w = w_openwindow(&plot_win));
    ld->w->data = ld;
    if (!shrunk) {
	changecurrentlevel(ld);
	drawopts();
...
int closelevel(struct leveldata *ld, int warn)
{
	...
	if (view.b_macros->selected == i)
	    view.b_macros->selected = 0;
	else if (view.b_macros->selected > i)
	    view.b_macros->selected--;
	for (i += 1; i < view.b_levels->num_options; i++)
	    view.b_levels->options[i - 1] = view.b_levels->options[i];
	checkmem(view.b_levels->options = REALLOC(view.b_levels->options, sizeof(char *) * (--view.b_levels->num_options + 1)));
	if (ld == l)
...
int makedoors(struct leveldata *ld)
{
	...
	if (n->prev->d.sd->d != NULL)
		...
		break;
	    case sdtype_door:
		if (n->prev->d.sd->num == 0)
		    ok = 0;
		for (i = 0; i < n->prev->d.sd->num; i++) {
		    n->prev->d.sd->cubes[i] =
			n->prev->d.sd->target[i]->d.d->c->no;
		    n->prev->d.sd->walls[i] =
			n->prev->d.sd->target[i]->d.d->wallnum;
		}
		break;
	    case sdtype_cube:
	    case sdtype_side:
...
{
    struct node *n, *nsd, *c, *starts[12], *keys[3], *flags[2], *reactor =
	NULL;
    void *data;
    int i, no, no_coop_starts, secret = -1;
    sortlist(&ld->cubes, 0);	/* so we get no higher numbers than size */
    sortlist(&ld->things, 0);
    sortlist(&ld->doors, 0);
    sortlist(&ld->pts, 0);
    sortlist(&ld->producers, 0);
    sortlist(&ld->sdoors, 0);
	...
	printmsg(TXT_LEAVEADJLIGHTMODE, adjust_light_nc->no,
		 adjust_light_wall);
	return 0;
    }
    ...
    if (ld->pts.size > MAX_DESCENT_VERTICES && !yesnomsg(TXT_TOOMANYPTS,
							 ld->pts.size,
							 MAX_DESCENT_VERTICES))
	return 0;
    if (ld->things.size > MAX_DESCENT_OBJECTS
	&& !yesnomsg(TXT_TOOMANYTHINGS, ld->things.size,
		     MAX_DESCENT_OBJECTS))
	return 0;
    if (ld->doors.size > MAX_DESCENT_WALLS && !yesnomsg(TXT_TOOMANYWALLS,
							...
							MAX_DESCENT_WALLS))
	return 0;
    if (ld->sdoors.size > MAX_DESCENT_SWITCHES
	&& !yesnomsg(TXT_TOOMANYSWITCHES, ld->sdoors.size,
		     MAX_DESCENT_SWITCHES))
	return 0;
    if (ld->producers.size > MAX_DESCENT_RCENTERS
	&& !yesnomsg(TXT_TOOMANYRCENTERS, ld->producers.size,
		     MAX_DESCENT_RCENTERS))
	return 0;
    if (init.d_ver >= d2_10_sw) {
	if (turnoffs->size > MAX_DESCENT_TURNOFFS
	    && !yesnomsg(TXT_TOOMANYTURNOFFS, turnoffs->size,
			 MAX_DESCENT_TURNOFFS))
	    return 0;
	...
	for (n = ld->things.head; n->next != NULL; n = n->next) {
		...
		view.pcurrthing = n;
		if (view.warn_thingoutofbounds
		    && !yesnomsg(TXT_THINGOUTOFBOUNDS, n->no))
		    return 0;
		else
		    c = ld->cubes.head;
	    }
		...
		if (secret >= 0) {
		    waitmsg(TXT_ONLYONESECRETSTART, secret);
		    break;
		}
		for (nsd = l->sdoors.head; nsd->next != NULL;
		     nsd = nsd->next)
		    if (nsd->d.sd->type == switch_secretexit)
			break;
		if (nsd->next == NULL)
		...
		ld->secretcube = c;
		for (i = 0; i < 9; i++)
		    ld->secret_orient[i] = n->d.t->orientation[i];
		break;
	    case tt1_dmstart:
		if (n->d.t->type2 > 7) {
		    if (!yesnomsg
			(TXT_ILLNOFORSTART, (int) n->d.t->type2, n->no))
			return 0;
		} else {
		    if (starts[n->d.t->type2] == NULL)
			starts[n->d.t->type2] = n;
		    else {
			view.pcurrthing = n;
			if (!yesnomsg(TXT_SAMENOFORSTART, n->d.t->type2,
				      starts[n->d.t->type2]->no, n->no))
			    return 0;
		    }
		}
		break;
	    case tt1_coopstart:
		if (++no_coop_starts > 4
		    && !yesnomsg(TXT_TOOMANYCOOPSTARTS, n->no))
		    return 0;
		if (no_coop_starts <= 4)
		    starts[7 + no_coop_starts] = n;
		n->d.t->type2 = no_coop_starts + 7;
		break;
	    case tt1_item:
		if (n->d.t->type2 == item_id_bluekey
		    || n->d.t->type2 == item_id_redkey
			...
			if (view.warn_tworeactors)
			    waitmsg(TXT_TWOREACTORS,
				    flags[n->d.t->type2 -
					  item_id_blueflag]->no, n->no);
		    }
		}
		break;
	    case tt1_reactor:
	...
	if (n->d.sd->type == switch_secretexit)
	    break;
    if (n->next && secret < 0
	&& !yesnomsg(TXT_NOSECRETSTART, n->d.sd->d->no))
	return 0;
    if (n->next == NULL && secret >= 0
	&& !yesnomsg(TXT_NOSECRETEXIT, secret))
	return 0;
...
int D1_REG_savelevel(FILE * f, struct leveldata *ld)
{
    ...
    md.numcubes = ld->cubes.size;
    md.numpts = ld->pts.size;
    ...
    if (!savelist(f, &ld->pts, savepoint, 0)) {
	fclose(f);
	return 0;
    }
    /* and cubes */
    if (!savelist
	(f, &ld->cubes, D1_REG_savecube, 0, ld->exitcube,
	 (int) ld->exitwall)) {
	fclose(f);
	return 0;
    }
    ...
    if (ld->secretstart)
	unlistnode(&ld->things, ld->secretstart);
    gd.numthings = ld->things.size;
    gd.numdoors = ld->doors.size;
    gd.numsdoors = ld->sdoors.size;
    gd.numproducer = ld->producers.size;
    gd.numedoors = 1;
    if (sizeplayer == 0) {
    ...
    if (strlen(ld->fullname) > 30)
	ld->fullname[30] = 0;
    if (fwrite(ld->fullname, 1, strlen(ld->fullname), f) !=
	strlen(ld->fullname)) {
	fclose(f);
	return 0;
    }
    ...
    gd.posdoors = ftell(f);
    if (!savelist(f, &ld->doors, savedoor, 0)) {
	fclose(f);
	return 0;
    }
    ...
    if (fwrite(ld->edoors, sizeof(struct edoor), gd.numedoors, f) !=
	gd.numedoors) {
	fclose(f);
	return 0;
    }
...
void createturnoff(struct leveldata *ld, struct lightsource *ls,
		   struct list *turnoffs, struct list *changedlights,
		   int notall)
{
    struct turnoff *to;
    ...
    if (notall
	&& (i < 0 || i >= pig.num_rdltxts || j < 0 || j >= pig.num_rdltxts
	    || (pig.rdl_txts[i].shoot_out_txt < 0
		&& pig.rdl_txts[j].shoot_out_txt < 0
		&& pig.rdl_txts[i].anim_seq < 0
		&& pig.rdl_txts[j].anim_seq < 0))) {
	/* is the light a flickering light ? */
	j = ls->fl != NULL ? 1 : 0;
	/* now check if the light is turned out by a switch */
	for (n = ld->sdoors.head, j = 0; n->next != NULL && !j;
	     n = n->next)
	    if (n->d.sd->type == switch_turnofflight
		|| n->d.sd->type == switch_turnonlight)
		for (i = 0; i < n->d.sd->num && !j; i++)
		    if (n->d.sd->target[i] == ls->cube
			&& (int) n->d.sd->walls[i] == ls->w)
			j = 1;
	if (!j)
	    return;
    }
	...
	for (i = 0; i < 6; i++)
	    if ((n->d.lse->add_light[i * 4] >> 10) +
		(n->d.lse->add_light[i * 4 + 1] >> 10) +
		(n->d.lse->add_light[i * 4 + 2] >> 10) +
		(n->d.lse->add_light[i * 4 + 3] >> 10) >
		theMinDeltaLight) {
		checkmem(lc = MALLOC(sizeof(struct changedlight)));
		lc->cube = n->d.lse->cube->no;
		lc->side = i;
		lc->stuff = 0;
		for (j = 0; j < 4; j++)
		    lc->sub[j] = n->d.lse->add_light[i * 4 + j] >> 10;
		...
		int notall)
{
	...
	if (init.d_ver >= d2_10_sw)
	    createturnoff(ld, ntc->d.ls, turnoffs, changedlights, notall);
	if (ntc->d.ls->fl)
	    checkmem(addnode(fl_lights, -1, ntc->d.ls->fl));
    }
}
		     ...
		     struct list *changedlights, struct list *fl_lights)
{
	...
	if (fl_lights->size > 0 &&
	    !yesnomsg
	    ("WARNING: If you save this level for Descent 2 V1.0,\n"
	     "you will loose all data about flickering lights. Continue?"))
	    return 0;
    }
    md.numcubes = ld->cubes.size;
    md.numpts = ld->pts.size;
    md.version = 0;
    if (fwrite
	(&lh, sizeof(struct fileheadversion) + sizeof(uint32_t) * 2,
	 1, f) != 1) {
	fclose(f);
	return 0;
    }
    ...
    if (fwrite(&lh.secret_cubenum, sizeof(uint32_t), 10, f) != 10) {
	fclose(f);
	return 0;
    }
    lh.minedata_offset = ftell(f);
    ...
    /* and cubes */
    if (!savelist
	(f, &ld->cubes, D2_REG_savecube, 0, ld->exitcube,
	 (int) ld->exitwall)) {
	fclose(f);
	return 0;
    }
    ...
    gd.numsdoors = ld->sdoors.size;
    gd.numproducer = ld->producers.size;
    gd.numturnoff = turnoffs->size;
    gd.numchangedlight = changedlights->size;
    gd.numedoors = 1;
    if (sizeplayer == 0) {
    ...
    if (fwrite(&pofdatasize, 2, 1, f) != 1) {
	fclose(f);
	return 0;
    }
    for (i = 0; i < pofdatasize; i++) {
	if (fwrite(pofdata[i], 1, strlen(pofdata[i]) + 1, f) !=
	    strlen(pofdata[i]) + 1) {
	    fclose(f);
	    return 0;
	}
...
int savelevel(char *fname, struct leveldata *ld, int testlevel,
	      int changename, int descent_version, int notalllightinfo)
{
    FILE *f;
    int ret;
    struct list turnoffs, changedlights, fl_lights;
    if (ld == NULL || fname == NULL)
	return 0;
    initlist(&turnoffs);
    initlist(&changedlights);
    initlist(&fl_lights);
	...
	if (ld->levelillum > 0)
	    makelights(ld, &turnoffs, &changedlights, &fl_lights,
		       notalllightinfo);
    }
    if (!checklvl(ld, testlevel, &turnoffs, &changedlights))
	return 0;
    ...
    case d2_12_reg:
	ret =
	    D2_REG_savelevel(f, ld, &turnoffs, &changedlights, &fl_lights);
	freelist(&turnoffs, free);
	freelist(&changedlights, free);
	freelist(&fl_lights, NULL);
	break;
    case d1_10_reg:
    case d1_14_reg:
	ret = D1_REG_savelevel(f, ld);
	break;
    default:
	return 0;
    }
