...
struct node *insertsingledoor(struct node *c, struct node *pd, int wallnum) {
    ...
    if (c->d.c->nc[wallnum] != NULL) {
        ...
        if (pd && pd->d.d->type1 == door1_switchwithwall) {
            ...
            *d = stdnonncdoor;
            c->d.c->walls[wallnum]->texture2 = std_switch_t2;
        }
    }
    d->cubenum = c->no;
    d->wallnum = wallnum;
    ...
    if (c->d.c->nc[wallnum]) {
        for (i = 0, wall = -1; i < 6; i++)
            if (c->d.c->nc[wallnum]->d.c->nc[i] != NULL)
                if (c->d.c->nc[wallnum]->d.c->nc[i]->no == c->no) {
                    wall = i;
                    break;
                }
        my_assert(wall != -1);
...
void justdelconnect(struct node *c, int wn) {
    ...
    my_assert(nwn < 6);
    checkmem(w = insertwall(nc, nwn, -1, -1, -1));
    nc->d.c->nc[nwn] = NULL;
    checkmem(w = insertwall(c, wn, -1, -1, -1));
    c->d.c->nc[wn] = NULL;
}
...
void cleansdoor(struct sdoor *sd) {
    ...
    for (k = 0; k < sd->num; k++) {
        switch (getsdoortype(sd)) {
            case sdtype_door: tl = &sd->target[k]->d.d->sdoors;
                break;
            case sdtype_cube: case sdtype_side:
                tl = &sd->target[k]->d.c->sdoors;
                break;
            default: tl = NULL;
        }
        if (tl) {
...
void deletecube(struct list *pcubes, struct list *ppts, struct node *n) {
    ...
    for (k = 0; k < 6; k++) {
        ...
        if (c->nc[k] != NULL) {
            nc = c->nc[k]->d.c;
            for (m = 0, w = -1; m < 6; m++)
                if (nc->nc[m] != NULL && nc->nc[m]->no == n->no) {
                    w = m;
                    break;
                }
...
int connectcubes(struct list *pts, struct node *nc1, int w1, struct node *nc2,
    ...
    if (nc1->no == nc2->no) {
        printmsg(TXT_CONNCUBEITSELF, nc1->no);
        return 0;
    }
    if (c1->d[w1] != NULL || c2->d[w2] != NULL) {
        printmsg(TXT_CONNWALLINWAY, nc1->no, w1, nc2->no, w2);
        return 0;
    }
    ...
    for (j = 0; j < 4; j++) {
        ...
        if (cp[j] == -1) {
            printmsg(TXT_CONNCUBETOOFARAWAY, nc1->no, w1, nc2->no, w2);
            return 0;
        }
        for (i = 0; i < j; i++)
            if (cp[i] == cp[j]) {
                printmsg(TXT_CONNAMBIGUOUS, nc1->no, w1, nc2->no, w2);
                return 0;
            }
    }
    ...
    for (j = 0; j < 4; j++) {
        untag(tt_edge, nc1, w1, j);
        untag(tt_edge, nc2, w2, j);
    }
    c2->nc[w2] = nc1;
    freewall(pts == NULL ? l : NULL, c2, w2);
    c1->nc[w1] = nc2;
    freewall(pts == NULL ? l : NULL, c1, w1);
...
int initdoor(struct node *n) {
    ...
    if ((d->c = findnode(&l->cubes, d->cubenum)) == NULL) {
        waitmsg(TXT_NOCUBEFORDOOR, d->cubenum);
        return 0;
    }
    if (d->wallnum > 5) {
        waitmsg(TXT_WRONGWALLINDOOR, d->wallnum, d->cubenum);
        return 0;
    }
    d->w = d->c->d.c->walls[d->wallnum];
    if (d->w == NULL) {
        waitmsg(TXT_NOWALLFORDOOR, d->cubenum, d->wallnum);
        return 0;
    }
    ...
    for (i = 0; i < 6; i++)
        if (d->c->d.c->nc[d->wallnum] && d->c->d.c->nc[d->wallnum]->d.c->nc[i] == d->c) {
            d->d = d->c->d.c->nc[d->wallnum]->d.c->d[i];
            break;
        }
    if (d->sdoor != 0xff) {
        if ((d->sd = findnode(&l->sdoors, d->sdoor)) == NULL) {
            ...
            for (i = 0; i < sd->num; i++) {
                if ((sdn = findnode(&l->cubes, sd->cubes[i])) == NULL) {
                    ...
                    sd->num--;
                    for (j = i; j < sd->num; j++) {
                        sd->cubes[j] = sd->cubes[j + 1];
                        sd->walls[j] = sd->walls[j + 1];
                    }
                    continue;
                }
                switch (getsdoortype(sd)) {
...
int initcube(struct node *n) {
    ...
    for (j = 0; j < 6; j++) {
        c->polygons[j * 2] = NULL;
        c->polygons[j * 2 + 1] = NULL;
        c->recalc_polygons[j] = 1;
...
int testcube(struct node *nc, int withmsg) {
    ...
    for (newpnum = 0; newpnum < 8; newpnum++) {
        ...
        if (SCALAR(&e, &l1) <= view.mincorner) {
            if (withmsg) printmsg(TXT_TPCUBENOTCONVEX, nc->no, newpnum);
            return 0;
        }
    ...
    for (j = 0; j < 6; j++) {
        ...
        if (fabs(SCALAR(&l1, &l2)) <= view.minweirdwall * LENGTH(&l1) * LENGTH(&l2)) {
            ...
            /* weird wall? */
            if (fabs(SCALAR(&l1, &l2)) <= view.minweirdwall * LENGTH(&l1) * LENGTH(&l2)) {
                if (withmsg) printmsg(TXT_TPWEIRDWALL, j, nc->no);
                return 0;
            }
            /* weird wall! */
        }
...
int move_pntlist(struct list *l, struct point *r) {
    ...
    if ((save = MALLOC(sizeof (struct point) *l->size)) == NULL) {
        printmsg(TXT_NOMEMFOROLDCOORDS);
        return 1;
    }
...
struct node *newcube(struct leveldata *ld) {
    ...
    for (j = 0; j < 6; j++) {
        nc->nc[j] = NULL;
        nc->d[j] = NULL;
        nc->tagged_walls[j] = NULL;
        nc->nextcubes[j] = 0xffff;
        nc->doors[j] = 0xff;
    }
    checkmem(nnc = addnode(&ld->cubes, -1, nc));
    for (j = 0; j < 8; j++) {
        checkmem(lp = MALLOC(sizeof (struct listpoint)));
        checkmem(nlp = addnode(&ld->pts, -1, lp));
        initlist(&lp->c);
        lp->tagged = NULL;
...
struct node *insertcube(struct list *cubes, struct list *pts, struct node *c,
    ...
    c->d.c->nc[wallnum] = nnc;
    nc->nc[oppwalls[wallnum]] = c;
...
void makesidestdshape(struct cube *cube, int w) {
    ...
    my_assert(cube != NULL);
    for (pn = 0; pn < 4; pn++) {
        p[pn] = cube->p[wallpts[w][pn]]->d.p;
        op[pn] = *p[pn];
    }
    for (i = 0; i < 3; i++) {
        c.x[i] = 0.0;
        for (pn = 0; pn < 4; pn++) c.x[i] += p[pn]->x[i] / 4.0;
    ...
    for (pn = 0, a = b = 0.0; pn < 4; pn++) {
        ...
        a += SCALAR(&np1[pn], &np1[pn]);
        b += SCALAR(&np2[pn], &np2[pn]);
    }
    for (pn = 0; pn < 4; pn++) {
        for (i = 0; i < 3; i++)
