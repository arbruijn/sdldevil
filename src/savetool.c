...
struct wall *readwall(FILE *lf, int no) {
    ...
    if (fread(&w->texture1, sizeof (int16_t), 1, lf) != 1) {
        FREE(w);
        return NULL;
    }
    if ((w->texture1 & 0x8000) != 0) {
        if (fread(&w->texture2, sizeof (int16_t), 1, lf) != 1) {
            FREE(w);
            return NULL;
        }
...
void *D1_REG_readsdoor(FILE *lf) {
    ...
    if (fread(&D1_sd, sizeof (struct D1_sdoor), 1, lf) != 1) {
        FREE(sd);
        return NULL;
    }
    sd->d = NULL;
...
void *readthing(FILE *lf) {
    ...
    if (init_test & 2)
        fprintf(errf, "%x %x size: %ld pos: %ld %ld %ld cube: %hd\n",
            (int32_t) t->type1, (int32_t) t->type2, (int32_t) getsize(ds_thing, t), t->pos[0],
            t->pos[1], t->pos[2], t->cube);
    switch (t->type1) {
        case tt1_coopstart: t->color = 1;
            if (t->size == 0) t->size = stdcoopstart.t.size;
            if (t->movement == mt_none) special = 1;
            break;
        case tt1_reactor: t->color = 3;
            if (t->size == 0) t->size = stdreactor.t.size;
            break;
        case tt1_hostage: t->color = 2;
            if (t->size == 0) t->size = stdhostage.t.size;
            break;
        case tt1_item: t->color = 2;
            if (t->size == 0) t->size = view.tsize;
            break;
        case tt1_dmstart: t->color = 1;
            if (t->size == 0) t->size = stdstart.t.size;
            if (t->movement == mt_none) special = 1;
            break;
        case tt1_mine: t->color = 0;
            if (t->size == 0) t->size = view.tsize;
            break;
        ...
        default: waitmsg("Unknown thing-type: %.2x.", (unsigned) t->type1);
            ...
            if (t->size == 0) t->size = view.tsize;
            switch (t->control) {
                case ct_mine: size += sizeof (struct control_mine);
                    break;
                case ct_ai: size += init.d_ver < d2_10_sw ?
                    ...
                    if (init.d_ver >= d2_10_sw && loading_level_version < d2_10_sw)
                        special = 3;
                    break;
                case ct_powerup: size += sizeof (struct control_powerup);
                    break;
                case ct_none: case ct_slew: case ct_control: break;
                default: waitmsg("Unknown thing control-type: %.2x.\n"\
     "Can't load level complete.", (unsigned) t->control);
                    FREE(t);
                    return NULL;
            }
            switch (t->movement) {
                case mt_none: break;
                case mt_physics: size += sizeof (struct movement_physics);
                    break;
                case mt_spinning: size += sizeof (struct movement_spinning);
                    break;
                default: waitmsg("Unknown thing movement-type: %.2x.\n"\
     "Can't load level complete.", (unsigned) t->movement);
                    FREE(t);
                    return NULL;
            }
            switch (t->render) {
                case rt_polyobj: size += sizeof (struct render_polyobj);
                    break;
                case rt_hostage: case rt_powerup:
                    size += sizeof (struct render_powerup);
                    break;
                default: waitmsg("Unknown thing render-type: %.2x.\n"\
     "Can't load level complete.", (unsigned) t->render);
                    FREE(t);
                    return NULL;
            }
    }
    ...
    switch (special) {
        case 1: /* a start with movement mt_none */
            if (fread((unsigned char *) t + sizeof (struct thing) +
                    ...
                    sizeof (struct movement_physics), 1, lf) != 1) {
                FREE(t);
                return NULL;
            }
        ...
        case 3: /* loading a Descent 1 level in Descent 2 mode */
            ...
            D2_r->c.hide_cube = D1_r.c.hide_cube;
            D2_r->c.hide_index = D1_r.c.hide_index;
...
struct edoor *readedoors(FILE *f, uint32_t num, uint32_t size) {
...
void *readpnt(FILE *lf) {
    int32_t coords[3];
    struct listpoint *p;
    if (fread(coords, sizeof (int32_t), 3, lf) != 3) return NULL;
...
void *D1_REG_readproducer(FILE *f) {
    ...
    p->prodwhat[0] = D1_p.prodwhat;
    p->prodwhat[1] = 0;
    p->hitpoints = D1_p.hitpoints;
    p->interval = D1_p.interval;
    p->cubenum = D1_p.cubenum;
    p->stuffnum = D1_p.stuffnum;
    if (init_test & 2) {
...
void *readturnoff(FILE *lf) {
    ...
    if (fread(to, getsize(ds_turnoff, NULL), 1, lf) != 1) {
        FREE(to);
        return NULL;
    }
    if (init_test & 2)
...
void *readchangedlight(FILE *lf) {
    ...
    if (fread(cl, getsize(ds_changedlight, NULL), 1, lf) != 1) {
        FREE(cl);
        return NULL;
    }
    if (init_test & 2)
...
void *D2_REG_readcube(FILE *lf) {
    ...
    if (init_test & 2) fprintf(errf, "Offset %lx Read cube: %x ",
            (int32_t) ftell(lf) - 1, (int) controlbyte);
    for (j = 0; j < 6; j++) {
        if ((controlbyte & (1 << j)) != 0) {
            if (fread(&c->nextcubes[j], sizeof (unsigned short int), 1, lf) != 1) {
                FREE(c);
...
int savepoint(FILE *f, struct node *n, va_list args) {
    ...
    coords[1] = p->x[1];
    coords[2] = p->x[2];
    return (fwrite(coords, sizeof (int32_t), 3, f) == 3);
}
    ...
    for (j = 0; j < 6; j++) {
        if (c->nextcubes[j] == 0xffff || c->doors[j] != 0xff) {
            ...
            if (c->walls[j]->texture2 != 0) {
                t1 |= 0x8000;
                t2 = c->walls[j]->texture2 | (c->walls[j]->txt2_direction << 14);
            } else t2 = 0;
...
int D1_REG_savesdoor(FILE *f, struct node *d, va_list args) {
    ...
    struct D1_sdoor D1_sd;
    int i;
    D1_sd.value = sd->value;
    D1_sd.link_num = sd->link_num;
    D1_sd.num = sd->num;
    D1_sd.type_0 = 0;
    D1_sd.time = -1;
    for (i = 0; i < 10; i++) {
        D1_sd.cubes[i] = sd->cubes[i];
        D1_sd.walls[i] = sd->walls[i];
    }
    switch (sd->type) {
        case switch_opendoor: D1_sd.flags = 1;
            break;
        case switch_producer: D1_sd.flags = 0x40;
            break;
        case switch_exit: D1_sd.flags = 0x8;
            break;
...
int D1_REG_saveproducer(FILE *f, struct node *n, va_list args) {
    ...
    D1_p.prodwhat = p->prodwhat[0];
    D1_p.hitpoints = p->hitpoints;
    D1_p.interval = p->interval;
    D1_p.cubenum = p->cubenum;
...
int savedata(FILE *f, struct node *d, va_list args) {
    int ds = va_arg(args, int);
    return (fwrite(d->d.d, getsize(ds, NULL), 1, f) == 1);
}
