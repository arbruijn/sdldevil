...
struct thing *changething(struct thing *t, struct thing *clone, int newtype,
    ...
    switch (newtype) {
        case tt1_robot: size = init.d_ver >= d2_10_sw ? sizeof (struct D2_robot):
            ...
            break;
        case tt1_hostage: size = sizeof (struct item);
            data = &stdhostage;
            break;
        ...
        case tt1_dmstart: case tt1_coopstart:
            ...
            if (init.d_ver >= d2_10_sw && newtype == tt1_coopstart) {
                ...
                for (i = (newtype == 4 ? 0 : 8), j = 0; i < (newtype == 4 ? 8 : 11) && !j; i++)
                    if (starts[i] == 0) {
                        j = 1;
                        ((struct thing *) data)->type2 = newtype == 4 ? i : 0;
                    }
                if (!j) waitmsg(TXT_TOOMANYSTARTS);
            }
            break;
        case tt1_mine: size = sizeof (struct mine);
            data = &stdmine;
            break;
        case tt1_item: size = sizeof (struct item);
            data = &stditem;
            break;
        case tt1_reactor: size = sizeof (struct reactor);
            data = &stdreactor;
            break;
...
struct node *getnode(enum datastructs it) {
    switch (it) {
        ...
        case ds_cube: case ds_flickeringlight: return view.pcurrcube;
            break;
        case ds_point: return view.pcurrpnt;
            break;
        case ds_sdoor:
        case ds_door: return view.pcurrdoor;
            break;
        case ds_thing: return view.pcurrthing;
            break;
        case ds_internal:
...
unsigned char *vl_getdata(enum datastructs it, struct node *n, va_list args) {
    ...
    switch (it) {
        ...
        case ds_thing:
            ...
            if (n != NULL) data = &n->d.t->type1;
            break;
        case ds_internal: data = &view;
            break;
        case ds_leveldata: data = l;
            break;
        case ds_sdoor:
            if (n == NULL) n = view.pcurrdoor;
...
size_t getsize(enum datastructs it, struct thing *t) {
    size_t size = 0;
    switch (it) {
        case ds_producer: size = 20;
            break;
        case ds_sdoor: size = 52;
            break;
        case ds_leveldata: size = sizeof (struct leveldata);
            break;
        case ds_internal: size = sizeof (struct viewdata);
            break;
        case ds_cube: size = 40;
            break;
        case ds_wall: size = 28;
            break;
        case ds_door: size = 24;
            break;
        case ds_corner: size = 6;
            break;
        case ds_turnoff: size = 6;
            break;
        case ds_changedlight: size = 8;
            break;
        case ds_flickeringlight: size = 16;
            break;
        case ds_thing:
            switch (t->type1) {
                case tt1_reactor: size = sizeof (struct reactor);
                    break;
                case tt1_hostage: case tt1_item: size = sizeof (struct item);
                    break;
