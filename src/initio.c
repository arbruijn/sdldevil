...
int iniread(FILE * f, const char *t, ...)
{
	...
	case 'p':
	    ...
	    break;
	case 'U':
	    if (fscanf(f, "%lu", va_arg(alist, uint32_t *)) != 1) {
		printf("Can't read uint32.\n");
		exit(2);
	    }
	    break;
	case 'X':
	    if (fscanf(f, "%lx", va_arg(alist, uint32_t *)) != 1) {
		printf("Can't read uint32 hex.\n");
		exit(2);
	    }
	    break;
	case 'd':
	    if (fscanf(f, "%d", va_arg(alist, int16_t *)) != 1) {
		printf("Can't read int16.\n");
		exit(2);
	    }
	    break;
	case 'x':
...
int readpalettes(FILE * f)
{
	...
	palettes[i].lighttables = (unsigned char *) ((uint32_t) palettes[i].mem_lighttables & 0xffffff00);
	...
	for (j = 0; j < NUM_SECURITY; j++) {
	    memcpy(&palettes[i].lighttables[256 * j], &palettes[i].lighttables[NUM_SECURITY * 256], 256);
	    memcpy(&palettes[i].lighttables[256 * (NUM_LIGHTCOLORS + NUM_SECURITY + j)], &palettes[i].lighttables[(NUM_SECURITY + NUM_LIGHTCOLORS - 1) * 256], 256);
	}
    }
...
void initeditor(const char *fn, int c)
{
    ...
    iniread(f,
	    ...
	    &view.e[0], &view.e[1], &view.doubleclick, &view.distcenter,
	    &view.dist, &view.distscala, &view.movefactor,
	    &view.pmovefactor, &view.movescala, &view.rotangle,
	    &view.protangle, &view.rotscala, &view.bm_movefactor,
	    &view.flatsideangle, &view.bm_stretchfactor,
	    &view.maxvisibility, &view.visscala, &view.tsize, &view.dsize,
	    &view.minclicksodist, &view.mincorner, &view.minweirdwall,
	    &view.maxconndist, &view.maxuserconnect, &view.gridlength,
	    &view.gridscala, &view.mapangle, &view.illum_quarterway,
	    &view.illum_brightness, &n, &view.gridonoff, &view.coord_axis,
	    &view.clickradius, &view.displayed_group, &view.askone,
	    &view.asktagged, &view.warn_convex, &view.warn_doublekeys,
	    &view.warn_tworeactors, &view.warn_thingoutofbounds,
	    &view.warn_gridrot, &txtoffsets[0], &txtoffsets[1],
	    &txtoffsets[2], &txtoffsets[3], &init.waittime,
	    &init.macropath, &init.levelpath, &init.pogpath,
	    &init.txtlistpath, &init.cfgpath, &init.fontname, &palname,
	    &init.cfgname, &init.lightname, &init.convtablename,
	    &init.lastname, &init.batchfilename, &init.menuname,
	    &pig.bulbname, &pig.brokenbulbname);
