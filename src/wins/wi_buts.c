...
int wi_choosecoords(struct wi_button *b, int *xp, int *yp, int *xs, int *ys,
    ...
    if (b->b.type == w_b_tag) {
        ...
        d_xsize = 0;
        one_ys = shapes.titlebar_height + 2;
    } else {
        num_options = b->b.d.ls->num_options;
        options = b->b.d.ls->options;
        d_xsize = b->b.d.ls->d_xsize;
        one_ys = shapes.titlebar_height;
    }
    ...
    *yp = w_ywinincoord(b->b.w, b->b.ypos) + b->b.ysize + 1;
    ...
    while ((*ys + columns - 1) / columns >= notes.winspace.ys - 10) 
        columns++;
    
    *colwidth = *xs;
    *collength = (num_options + columns - 1) / columns;
    *xs = *xs*columns;
    *ys = *collength * one_ys + 10;
    
    if (*xs > notes.winspace.xs) 
        *xs = notes.winspace.xs;
...
int wi_b_press(struct ws_event *ws, struct wi_button *b) {
    ...
    do {
        ws_getevent(ws, 0);
        x = w_xscreencoord(b->b.w, ws->x);
        y = w_yscreencoord(b->b.w, ws->y);
        t2 = clock();
...
int wi_b_drag(struct ws_event *ws, struct wi_button *b) {
    ...
    if (ws->flags == ws_f_lbutton) {
        p1 = b->b.d.dr->l_pressroutine;
        p2 = b->b.d.dr->l_dragroutine;
        p3 = b->b.d.dr->l_relroutine;
    } else {
        p1 = b->b.d.dr->r_pressroutine;
        p2 = b->b.d.dr->r_dragroutine;
...
int wi_b_tag(struct ws_event *ws, struct wi_button *b) {
    ...
    do {
        ws_getevent(ws, 0);
        xp = w_xscreencoord(b->b.w, ws->x);
        yp = w_yscreencoord(b->b.w, ws->y);
...
int wi_b_list(struct ws_event *ws, struct wi_button *b) {
    ...
    do
        ws_getevent(ws, 1); while (ws->buttons != ws_bt_none);
...
int wi_handlestringbutton(struct ws_event *ws, struct wi_button *b, int p,
    ...
    oldstr = b->b.d.str->str;
    b->b.d.str->str = buffer;
    if (p < 0) p = 0;
        ...
        if (ws->flags == ws_f_keypress) {
            switch (ws->key) {
                ...
                case 278: p = 0;
                    b->b.d.str->offset = 0;
                    break; /* home */
                case 279: p = strlen(buffer);
                    b->b.d.str->offset = p - b->b.d.str->length + 1;
                    break; /* end */
                default:
                    if (isprint(keychar) && (b->b.d.str->allowed_char == NULL ||
                            b->b.d.str->allowed_char(keychar)) && p < b->b.d.str->max_length) {
