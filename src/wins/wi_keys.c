...
int wi_bk_choose(struct ws_event *ws, struct wi_button *b) {
    ...
    do {
        ...
        if (ws->flags == ws_f_keypress)
            switch (ws->key) {
                case 0x248: i = i <= 0 ? b->b.d.ls->num_options - 1: i - 1;
                    break; /* up */
                case 0x250: i = i >= b->b.d.ls->num_options - 1 ? 0: i + 1;
                    break; /* down */
                case 0x1b: end = 1;
                    break; /* esc */
                default:
...
int wi_bk_tag(struct ws_event *ws, struct wi_button *b) {
    ...
    do {
        ...
        if (ws->flags == ws_f_keypress)
            switch (ws->key) {
                case 0x248: pos = pos <= 0 ? b->b.d.tag->num_options - 1: pos - 1;
                    break; /*up*/
                case 0x250: pos = pos >= b->b.d.tag->num_options - 1 ? 0: pos + 1;
                    break; /*down*/
                case 0x1b: end = 1;
                    break; /* esc */
                default:
        ...
        if (ws->flags == ws_f_keypress) {
            switch (ws->key) {
                ...
                case 0x247: pos = 0;
                    ...
                    break; /*home*/
                case 0x24f: pos = b->b.d.sls->no_strings - 1;
                    break; /* end*/
                case 0x248: --pos;
                    break; /* up */
                case 0x249: pos -= (b->b.d.sls->no_rows - 1) / 2;
                    break; /*pdown*/
                case 0x250: ++pos;
                    break; /* down */
                case 0x251: pos += (b->b.d.sls->no_rows + 1) / 2;
                    break; /*pup*/
                default:
                    if (isprint(ws->key)) {
                        for (i = 0; i < b->b.d.sls->no_strings; i++)
