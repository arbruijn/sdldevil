...
void drawmenubar(void)
{
    ...
    ws_drawframedbox(0, 0, notes.xres, shapes.titlebar_height +
		     shapes.width_menuframe * 2 + 1,
		     shapes.width_menuframe, notes.colindex[cv_buttonlt],
		     notes.colindex[cv_buttonrb],
	...
	if (notes.titlebar_text) {
	    strcpy(buffer, notes.titlebar_text);
	    strcat(buffer, "  / ");
	} else
	    buffer[0] = 0;
	strcat(buffer, notes.cur_win->w.title);
	ws_drawtext(notes.xres - ws_pixstrlen(buffer) -
		    shapes.titlebar_height / 4, 2, ws_pixstrlen(buffer),
		    buffer, notes.colindex[cv_textbg],
		    notes.colindex[cv_buttonin]);
	ws_drawtext(notes.xres - ws_pixstrlen(buffer) -
		    shapes.titlebar_height / 4 + 1, 3,
		    ws_pixstrlen(buffer), buffer,
...
void w_newpalette(unsigned char *palette)
{
	...
	return;
    for (i = 0; i < 256; i++)
	ws_setcolor(i, palette[i * 3], palette[i * 3 + 1],
		    palette[i * 3 + 2]);
    ws_resetmousecolors();
    notes.colindex[cv_bg] = w_makecolor(0, 0, 0);
    notes.colindex[cv_winfill] = w_makecolor(0, 0, 0);
	...
	wi_drawwin(n->d.w_w);
	for (nb = n->d.w_w->buttonlist.head; nb->next != NULL;
	     nb = nb->next)
...
{
    if (w->xpos < notes.winspace.xp)
	w->xpos = notes.winspace.xp;
    if (w->ypos < notes.winspace.yp)
	w->ypos = notes.winspace.yp;
    if (w->xsize > notes.winspace.xs)
	w->xsize = notes.winspace.xs;
    if (w->ysize > notes.winspace.ys)
	w->ysize = notes.winspace.ys;
}

void wi_closeroutine(struct w_button *b)
{
    if (b->w->close_routine != NULL)
	b->w->close_routine(b->w, b->w->data);
}
...
{
    if (!(b->w->buttons & wb_drag))
	return;
    ws_erasemouse();
    ws_mousewarp(b->w->xpos, b->w->ypos);
...
{
    if (!(b->w->buttons & wb_size))
	return;
    ws_mousewarp(b->w->xpos + b->w->xsize, b->w->ypos);
    ws_erasemouse();
    ws_drawbox(b->w->xpos, b->w->ypos, b->w->xsize, b->w->ysize,
	       notes.colindex[cv_windrag], 1);
    ox = b->w->xpos + b->w->xsize;
    oy = b->w->ypos;
    dx = x - b->w->xpos - b->w->xsize;
    dy = y - b->w->ypos;
}
...
void wi_resize_size(struct w_button *b, int x, int y)
{
    if (x == ox && y == oy)
	return;
    ws_drawbox(b->w->xpos, oy, ox - b->w->xpos,
	       b->w->ypos + b->w->ysize - oy, notes.colindex[cv_windrag],
	       1);
    if (b->w->maxxsize > 0) {
	if (x - b->w->xpos > b->w->maxxsize)
	    x = b->w->xpos + b->w->maxxsize;
	else if (x - b->w->xpos < -b->w->maxxsize)
	    x = b->w->xpos - b->w->maxxsize;
    } 
    if (b->w->maxysize > 0) {
...
void w_resizewin(struct w_button *b)
{
    ...
    if (b->w->xsize <
	ws_pixstrlen(b->w->title) + shapes.closebutton_width +
    ...
    if (b->w->ypos < notes.winspace.yp)
	b->w->ypos = notes.winspace.yp;
    if (b->w->xpos < notes.winspace.xp)
	b->w->xpos = notes.winspace.xp;
    ((struct wi_window *) (b->w))->oysize = b->w->ysize;
    b->xsize = b->w->xsize;
    if (b->w->buttons & (wb_close | wb_shrink))
	b->xsize -= shapes.closebutton_width;
    if ((n = b->inscription.head) != NULL)
	n->d.w_d->x =
	    (b->xsize - ws_pixstrlen(n->d.w_d->contents.txt)) / 2 - 1;
...
void wi_resize_rel(struct w_button *b, int x, int y)
{
    ...
    wi_undrawwin((struct wi_window *) b->w);
    if (x > b->w->xpos) {
	b->w->xsize = x - b->w->xpos;
    } else {
	b->w->xpos = x;
	b->w->xsize = b->w->xpos - x;
    }
    if (y < b->w->ypos + b->w->ysize) {
	b->w->ysize = b->w->ypos + b->w->ysize - y;
	b->w->ypos = y;
    } else {
	p = b->w->ysize;
	b->w->ysize = y - b->w->ypos - b->w->ysize;
	b->w->ypos = b->w->ypos + p;
    }
    w_resizewin(b);
    wi_drawwin((struct wi_window *) b->w);
...
struct w_window *w_openwindow(struct w_window *w)
{
	...
	wi_p_closebutton->l_routine =
	    wi->w.buttons & wb_close ? wi_closeroutine : NULL;
	wi_p_closebutton->r_routine =
	    wi->w.buttons & wb_shrink ? wi_shrinkroutine : NULL;
	checkmem(b =
		 (struct wi_button *) w_addstdbutton(&wi->w, w_b_press,
						     w_xscreencoord(&wi->w,
								    wi->w.
								    xpos),
						     w_yscreencoord(&wi->w,
	...
	wi_d_sizebutton->l_pressroutine =
	    wi->w.buttons & wb_drag ? wi_drag_press : NULL;
	wi_d_sizebutton->l_dragroutine =
	    wi->w.buttons & wb_drag ? wi_drag_drag : NULL;
	wi_d_sizebutton->l_relroutine =
	    wi->w.buttons & wb_drag ? wi_drag_rel : NULL;
	wi_d_sizebutton->r_pressroutine =
	    wi->w.buttons & wb_size ? wi_resize_press : NULL;
	wi_d_sizebutton->r_dragroutine =
	    wi->w.buttons & wb_size ? wi_resize_size : NULL;
	wi_d_sizebutton->r_relroutine =
	    wi->w.buttons & wb_size ? wi_resize_rel : NULL;
	checkmem(b =
		 (struct wi_button *) w_addstdbutton(&wi->w, w_b_drag,
						     w_xscreencoord(&wi->w,
								    wi->w.
								    xpos) +
						     x,
...
void w_buttoninwin(struct w_window *w, struct w_button *b, int draw)
{
    ...
    bi->drawn = draw;
    if (draw)
	my_assert(wi_drawbutton(bi, 0));
    checkmem(bi->n =
	     addnode(&((struct wi_window *) bi->b.w)->buttonlist, -1, bi));
    bi->b.no = bi->n->no;
}
...
struct w_button *w_addstdbutton(struct w_window *w, enum but_types type,
				...
				const char *txt, void *data, int draw)
{
	...
	if (b->xsize < 0)
	    b->xsize =
		txt !=
		NULL ? ws_pixstrlen(txt) + 4 : shapes.closebutton_width;
	if (b->ysize < 0)
	...
	if (b->xsize < 0)
	    b->xsize =
		txt !=
		NULL ? ws_pixstrlen(txt) + 6 : shapes.closebutton_width;
	if (b->ysize < 0)
	...
	if (b->xsize < 0)
	    b->xsize =
		b->d.str->max_length * ws_pixstrlen("M") +
		b->d.str->d_xsize + 10;
	if (b->ysize < 0)
	    b->ysize = shapes.titlebar_height + 5;
	b->d.str->length =
	...
	case w_b_string:
		...
		x = 3;
		l = b->d.str->d_xsize <
		    4 ? 0 : ws_charstrlen(b->d.str->d_xsize - 4);
	    }
	    y = (b->ysize - shapes.titlebar_height) / 2;
	    break;
	default:
...
struct w_button *w_addimagebutton(struct w_window *w, enum but_types type,
				  ...
				  int with_bg, int draw)
{
    struct w_button *b, *nb;
    struct w_description *d1, *d2;
    if (abm == NULL || bm == NULL || abm->xsize != bm->xsize
	|| abm->ysize != bm->ysize)
	return NULL;
    checkmem(b = MALLOC(sizeof(struct w_button)));
    b->xpos = xpos;
...
void w_deletebutton(struct w_button *b)
{
    ...
    my_assert(bi != NULL);
    if (bi->n != NULL)
	killnode(&((struct wi_window *) b->w)->buttonlist, bi->n);
...
void w_closewindow(struct w_window *w)
{
    ...
    if (w == NULL)
	return;
    if (notes.cur_but && notes.cur_but->d.w_b->b.w == w)
	notes.cur_but = NULL;
    if (notes.cur_win && &notes.cur_win->w == w)
	notes.cur_win = NULL;
    w_wintofront(w);
    wi_undrawwin(wi);
    for (n = wi->buttonlist.head->next; n != NULL; n = n->next) {
	if (n->prev->d.w_b->sys_button)
	    FREE(n->prev->d.w_b->b.d.d);
	w_deletebutton(&n->prev->d.w_b->b);
    }
    if (wi->n)
	killnode(&notes.windows, wi->n);
    if (wi->in)
	ws_freebitmap(wi->in);
    if (wi->bg)
	ws_freebitmap(wi->bg);
...
int wi_readmenu(FILE * mfile, int no, void (**action) (int), int max_anr,
		int xoffset, int yoffset, int mainline, struct list *l,
		int level)
{
    int i, lastpos = 0;
	...
	*(pos + 2) = 0;
	...
	if (mainline) {
		...
		lastpos)
		lastpos =
		    ws_pixstrlen(m->txt) + shapes.titlebar_height / 2;
	}
	if (m->actionnr >= 0) {
	    ...
	    if (!wi_readmenu
		(mfile, -m->actionnr, action, max_anr,
		 mainline ? m->x1 : m->x1 + ws_pixstrlen(m->txt) + 1,
		 mainline ? m->y1 + shapes.titlebar_height +
		 shapes.width_menuframe + 1 : m->y1, 0, &m->submenus,
		 level + 1)) {
...
int w_initmenu(FILE * mfile, void (**action) (int), int max_anr)
{
	...
	return 0;
    if (!wi_readmenu
	(mfile, no, action, max_anr, shapes.width_menuframe * 2,
    ...
    drawmenubar();
    notes.winspace.yp +=
	shapes.titlebar_height + shapes.width_menuframe * 2;
    notes.winspace.ys -=
	shapes.titlebar_height + shapes.width_menuframe * 2;
    return 1;
}
...
struct ws_bitmap *w_createlinesbm(int xsize, int ysize, int n, int *lines)
{
    ...
    for (i = 0; i < n; i++)
	ws_bmdrawline(bm, lines[i * 5 + 1], lines[i * 5 + 2],
		      lines[i * 5 + 3], lines[i * 5 + 4], lines[i * 5], 0);
    return bm;
}
...
void w_butwin(struct w_window *w)
{
    ...
    if (wi == NULL || wi->buttonlist.size <= 0)
	n = NULL;
    else {
	w_wintofront(w);
	if (w->shrunk)
	...
	while (n->next != NULL
	       && (!n->d.w_b->drawn || !n->d.w_b->activated
		   || n->d.w_b->sys_button
...
int w_savecfg(FILE * f)
{
	...
	if (n->d.w_w->w.title)
	    if (fprintf
		(f, "%s\n%d %d %d %d %d %d\n", n->d.w_w->w.title,
		 n->d.w_w->w.xpos, n->d.w_w->w.ypos, n->d.w_w->w.xsize,
		 n->d.w_w->w.ysize, n->d.w_w->oysize,
		 n->d.w_w->w.shrunk) < 0)
		return 0;
    return 1;
}
...
void w_readcfg(FILE * f)
{
	...
	if (!fgets(buffer, 255, f))
	    return;
	buffer[255] = 0;
	buffer[strlen(buffer) - 1] = 0;	/* eliminate the \n */
	for (n = notes.windows.tail; n->prev != NULL; n = n->prev)
	    if (n->d.w_w->w.title
		&& strcmp(n->d.w_w->w.title, buffer) == 0)
		break;
			...
			enum kbstattypes bitmask)
{
	...
	if (notes.kbstatcursors[i].kbstat == kbstat &&
	    notes.kbstatcursors[i].bitmask == bitmask)
	    kbsc = &notes.kbstatcursors[i];
    if (!kbsc) {
	checkmem(notes.kbstatcursors = REALLOC(notes.kbstatcursors,
					       ++notes.num_kbstatcursors *
					       sizeof(struct
						      kbstatcursor)));
	kbsc = &notes.kbstatcursors[notes.num_kbstatcursors - 1];
	kbsc->kbstat = kbstat;
	kbsc->bitmask = bitmask;
    } else
	ws_killcursor(kbsc->cursor);
