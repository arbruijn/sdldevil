...
#define SYS_COMPILER_NAME __VERSION__
...
struct ws_event {
    enum flagtypes { ws_f_none, ws_f_lbutton, ws_f_rbutton,
	    ws_f_keypress } flags;
    int x, y;			/* coordinates of mouse */
    enum buttontypes { ws_bt_none = 0, ws_bt_left = 1, ws_bt_right =
	    2 } buttons;
    /* button state */

	uint16_t unicode;
    int key;			/* key code from keyboard */
    enum kbstattypes { ws_ks_shift = 1, ws_ks_alt = 2, ws_ks_ctrl =
	    4, ws_ks_numlock = 8,
	ws_ks_capslock = 16, ws_ks_scrollock = 32
    } kbstat;
};
