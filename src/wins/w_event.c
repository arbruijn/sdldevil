...
struct wi_button *wi_checkforbutton(struct wi_window *wi, int x, int y)
{
	...
	if (n->d.w_b->drawn && x > n->d.w_b->b.xpos &&
	    x < n->d.w_b->b.xpos + n->d.w_b->b.xsize - 1
	    && y > n->d.w_b->b.ypos
	...
	my_assert(bi->b.type < w_b_number);
	return bi;
    }
    return NULL;
}
...
int w_handleuser(int num_endbuts, struct w_button **endbuttons,
		 int num_wins, struct w_window **wins, int num_keycodes,
		 struct w_keycode *ec_keys,
		 void (**keyhandling) (int evcode))
{
	...
	if (i == notes.num_kbstatcursors && notes.current_cursor >= 0) {
	    ws_changecursor(NULL);
	    notes.current_cursor = -1;
	}
	if (msg_win) {
	    if (ws.buttons != ws_bt_none && ws.x >= msg_win->xpos &&
		ws.x < msg_win->xpos + msg_win->xsize
		&& ws.y >= msg_win->ypos
		&& ws.y < msg_win->ypos + msg_win->ysize) {
		ws_sleep();
		continue;
	    }
	    i = (!event
		 || (ws.x >= msg_win->xpos
		     ...
		     && ws.y >= msg_win->ypos
		     && ws.y < msg_win->ypos + msg_win->ysize));
	    if (msg_win_x < ws.x - 20 || msg_win_y < ws.y - 20	
		|| msg_win_x > ws.x + 20 || msg_win_y > ws.y + 20
		|| (event && (ws.flags == ws_f_keypress || ws.buttons != ws_bt_none)))
	    {
		w_closewindow(msg_win);
		msg_win = NULL;
	    }
	...
	if (event && ws.flags == ws_f_keypress) {
	    if ((ws.kbstat & ws_ks_alt) != 0 && isalnum(ws.key)
		&& num_wins == 0) {
		wi_handlemenu(&ws);
	 	ws_sleep();
		continue;
	    }
	    if (notes.cur_but != NULL
		&& (ws.key == 273 || ws.key == 274 || ws.key == 0xd)) {
		n = notes.cur_but;
		switch (ws.key) {
		case 273:	/* up */
		    do {
			n = n->prev;
			if (n->prev == NULL)
			    n = *(&n->prev + 1);	/*this is the tail of the list */
		    }
		    while (n != notes.cur_but
			   && (!n->d.w_b->drawn || !n->d.w_b->activated
			       || n->d.w_b->sys_button
			       || !n->d.w_b->tobechanged
			       || wi_butkeyhandling[n->d.w_b->b.type] ==
			       NULL));
		    break;
		case 274:	/* down */
		    do {
			n = n->next;
			if (n->next == NULL)
			    n = *(&n->next - 1);	/*this is the head of the list */
		    }
		    while (n != notes.cur_but
			   && (!n->d.w_b->drawn || !n->d.w_b->activated
			       || n->d.w_b->sys_button
		...
		case 0xd:	/* return */
			...
			n->d.w_b->b.event = ws;
			if (wi_butkeyhandling[n->d.w_b->b.type]
			    (&ws, n->d.w_b) && endbuttons)
			    for (i = 0; i < num_endbuts; i++)
				if (n->d.w_b ==
				    (struct wi_button *) endbuttons[i]) {
				    end = i;
				    break;
				}
		    }
		...
		for (i = 0; i < num_keycodes; i++) {
		    if (ec_keys[i].key == ws.key && ((ec_keys[i].kbstat & ws.kbstat) == ec_keys[i].kbstat)) {
		    //if (ec_keys[i].key == ws.key && ((ec_keys[i].kbstat & ws.kbstat) == ws.kbstat)) {
			if (ec_keys[i].event >= 0)
			    keyhandling[ec_keys[i].event] (ec_keys[i].
							   event);
			if (ec_keys[i].flags & w_kf_exit)
			    end = i << 16;
			break;
		    }
		}
	...
	we->w = NULL;
	if (num_wins == 0
	    && ws.y < shapes.titlebar_height + 2 * shapes.width_menuframe
	    && event)
	    wi_handlemenu(&ws);
	else {
	    for (n = notes.windows.head; n->next != NULL; n = n->next)
		if (ws.x > n->d.w_w->w.xpos
		    && ws.x < n->d.w_w->w.xpos + n->d.w_w->w.xsize - 1
		    && ws.y > n->d.w_w->w.ypos
		    && ws.y < n->d.w_w->w.ypos + n->d.w_w->w.ysize - 1) {
		    wi = n->d.w_w;
		    we->w = &wi->w;
		    we->x = w_xscreencoord(we->w, ws.x);
		    we->y = w_yscreencoord(we->w, ws.y);
		    break;
		}
	    if (we->w == NULL) {
		ws_sleep();
	    ...
	    if (wins) {
		for (winnum = 0; winnum < num_wins; winnum++)
		    if (we->w == wins[winnum])
			break;
		if (winnum >= num_wins) {
			ws_sleep();
		...
		w_wintofront(we->w);
		if (we->w->click_routine)
		    we->w->click_routine(we->w, we->w->data, we);
		/* Test if a button was hit */
		if ((bi = wi_checkforbutton(wi, we->x, we->y)) != NULL) {
		    if (ws.kbstat & ws_ks_alt) {
			if (bi->b.helptxt)
			    w_okcancel(bi->b.helptxt, NULL, NULL, NULL);
			ws_sleep();
			continue;
		    }
		    bi->b.event = ws;
		    ...
		    winclosed = (bi->sys_button && !bi->b.data
				 && ws.flags == ws_f_lbutton
				 && (wi->w.buttons & wb_close) != 0);
		    if (bi->tobechanged
			&& wi_buttonhandling[bi->b.type] != NULL
	       ...
	       const char *help)
{
    ...
    if (!notes.print_msg && ok == NULL && cancel == NULL)
	return 0;
    bxsize =
	ws_pixstrlen(ws_pixstrlen(ok) > ws_pixstrlen(cancel) ? ok : cancel) + 10;
    i = 0;
    numlines = -1;
    win.xsize = 20 + 2 * bxsize;
    while (++numlines < 30 && i < strlen(txt)) {
	start = strchr(txt + i, '\n');
	if (start == NULL)
	    start = txt + strlen(txt);
    ...
    win.maxysize = win.ysize = 10 + (ok != NULL ? numlines + 2 : numlines) * shapes.titlebar_height;
    win.ypos = (notes.yres - win.ysize) / 1.9;	/* this looks better then /2 */
    if (ok == NULL && cancel == NULL) {
	ws_getevent(&ws, 0);
	ws.x = w_xwinspacecoord(ws.x);
	ws.y = w_ywinspacecoord(ws.y);
    ...
    win.help = help;
    win.buttons = 0;
    win.refresh = wr_normal;
    win.data = NULL;
    win.refresh_routine = NULL;
    win.click_routine = NULL;
    win.close_routine = NULL;
    if (msg_win != NULL) {
	w_closewindow(msg_win);
	msg_win = NULL;
    }
	...
	ws_drawtext(w_xwinincoord
		    (w, (w_xwininsize(w) - ws_pixstrlen(pos[i])) / 2),
		    w_ywinincoord(w, 5 + i * shapes.titlebar_height),
		    w->xsize, pos[i], notes.colindex[cv_textfg], -1);
	FREE(pos[i]);
    }
    if (ok != NULL) {
	...
	checkmem(b_arr[0] =
		 w_addstdbutton(w, w_b_press,
				cancel !=
				NULL ? 5 : (w_xwininsize(w) -
					    ws_pixstrlen(ok) + 4) / 2,
				w_ywininsize(w) - shapes.titlebar_height -
				10, bxsize, -1, ok, &b_press, 1));
	if (cancel != NULL)
	    checkmem(b_arr[1] =
		     w_addstdbutton(w, w_b_press,
				    w_xwininsize(w) - bxsize - 5,
				    w_ywininsize(w) -
				    shapes.titlebar_height - 10, bxsize,
				    -1, cancel, &b_press, 1));
	i = w_handleuser(cancel != NULL ? 2 : 1, b_arr, 1, &w, 0, NULL,
			 NULL);
	w_closewindow(w);
    } else {
	msg_win = w;
