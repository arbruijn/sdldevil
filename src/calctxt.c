...
void calc_txtcoords(struct wall *w, int corner, struct point *new_p,
    ...
    normalize(&turnaxis);
    x = SCALAR(&turnaxis, &paraxis);
    for (j = 0; j < 3; j++) paraxis.x[j] -= x * turnaxis.x[j];
    normalize(&paraxis);
    for (j = 0; j < 2; j++) t_uv[j] = w->corners[t_pe].x[j] - w->corners[t_ps].x[j];
    x = sqrt(t_uv[0] * t_uv[0] + t_uv[1] * t_uv[1]);
    t_uv[0] /= x;
    t_uv[1] /= x;
    ot_uv[0] = -t_uv[1];
    ot_uv[1] = t_uv[0];
    x = SCALAR(&turnaxis, &p) / 640.0;
    y = SCALAR(&paraxis, &p) / 640.0;
...
void newwall_offset(struct cube *c, struct wall *w, int op0, int op1) {
    ...
    p1xoffs = w->corners[op0].x[0];
    p1yoffs = w->corners[op0].x[1];
    ...
    centerx = centery = 0;
    for (j = 0; j < 4; j++) {
        centerx += w->corners[j].x[0];
        centery += w->corners[j].x[1];
    }
    for (j = 0; j < 4; j++) {
...
void arrangebitmaps(struct node *start_c, struct wall *start_w) {
    ...
    for (run = walls.head; run->next != NULL; run = run->next) {
        ...
        for (wp1 = 0; wp1 < 4; wp1++) {
            ...
            save[0] = nc;
            save[1] = (struct node *) w;
            checkmem(addnode(&walls, nc->no, save));
        }
        // FFE - avoid freeing memory we use above in the for loop
        // freelist below should free the memory
...
int findpath(struct list *dest) {
    ...
    for (s = 0, nc = NULL; s < 6; s++) {
        if (cc->d.c->nc[s] != NULL && (snc = cc->d.c->nc[s]->d.c->tagged) != NULL) {
            if (nc != NULL) {
                printmsg(TXT_AMBIGUOUSPATH, cc->no, snc->no, nc->no);
                return 0;
            } else 
                nc = snc;
        }
