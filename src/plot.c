...
void makeview(int lr) {
    ...
    max_xcoord = scr_xsize / 2;
    max_ycoord = scr_ysize / 2;
    scr_xsize = max_xcoord * 2 + 1;
    scr_ysize = max_ycoord * 2 + 1;
    ...
    if (max_axis * tan(MIN_VIEWANGLE) >= min_axis * tan(MAX_VIEWANGLE)) {
        max_angle = MAX_VIEWANGLE;
        min_angle = atan(tan(max_angle) * min_axis / max_axis);
    } else {
        min_angle = MIN_VIEWANGLE;
        max_angle = atan(tan(min_angle) * max_axis / min_axis);
    }
...
int in_getpixelcoords(struct point *p, struct pixel *pix) {
    ...
    for (i = 0; i < 3; i++) {
        x.x[i] = d.x[i] - fy * er[1].x[i];
        y.x[i] = d.x[i] - fx * er[0].x[i];
    }
    if (SCALAR(&x, &er[2]) < xviewphi || SCALAR(&y, &er[2]) < yviewphi) return 0;
    pix->x = fx * z_dist / pix->d + 0.5;
    pix->y = fy * z_dist / pix->d + 0.5;
    return 1;
}
...
int getpixelcoords(int lr, struct point *p, struct pixel *pix) {
    makeview(lr);
    return in_getpixelcoords(p, pix);
}
...
int calchit(float *r, float ax, float ay, float ex, float ey,
    ...
    if (D == 0.0) {
        *r = 0;
        return 0;
    } /* no hit at all, lines are parallel */ \
 *r = (ay * dx - ax * dy) / D;
    s = (ex * ay - ey * ax) / D;
...
int getscreencoords(int lr, struct point *sp, struct point *ep,
    ...
    s_s[0] = SCALAR(&s_d, &er[0]);
    e_s[0] = SCALAR(&e_d, &er[0]);
    s_s[1] = SCALAR(&s_d, &er[1]);
    e_s[1] = SCALAR(&e_d, &er[1]);
    l2_s = SCALAR(&s_d, &s_d);
    l2_e = SCALAR(&e_d, &e_d);
    ...
    if (s_s[2] < z_dist && e_s[2] >= z_dist) { /* clip against viewplane */
...
void plotmarker(struct point *p, int hilight) {
    makeview(0);
    in_plotmarker(0, p, hilight);
    if (view.whichdisplay) {
        makeview(1);
        in_plotmarker(1, p, hilight);
    }
}
...
void in_plotcube(int w, struct node *n, int hilight, int withdoors, int xor,
    ...
    if ((hilight == 0 || hilight == -1) && c->type != cube_normal) {
        withalllines = 1;
        switch (c->type) {
            case cube_fuel: hilight = 5;
                break;
            case cube_reactor: hilight = 4;
                break;
            case cube_producer: hilight = 2;
                break;
            case cube_blueflag: hilight = 8;
                break;
            case cube_redflag: hilight = 7;
                break;
        }
    }
    ...
    if (!withalllines) {
        for (j = 0; j < 6; j++)
            if (c->nc[j] != NULL && c->nc[j]->no < n->no)
                switch (j) {
                    case 0: next &= ~(0x4 | 0x40 | 0x400 | 0x800);
                        break;
                    case 1: next &= ~(0x8 | 0x80 | 0x100 | 0x800);
                        break;
                    case 2: next &= ~(0x1 | 0x10 | 0x100 | 0x200);
                        break;
                    case 3: next &= ~(0x2 | 0x20 | 0x200 | 0x400);
                        break;
                    case 4: next &= ~(0x10 | 0x20 | 0x40 | 0x80);
                        break;
                    case 5: next &= ~(0x1 | 0x2 | 0x4 | 0x8);
                        break;
                }
        if (next == 0) return;
    }
...
void plotthing(struct thing *t, int hilight) {
    makeview(0);
    in_plotthing(0, t, hilight);
    if (view.whichdisplay) {
        makeview(1);
        in_plotthing(1, t, hilight);
    }
}
...
void plotcurrent(void) {
    ...
    copytoscreen();
    w_refreshend(l->w);
    oldpcurrthing = view.pcurrthing;
    oldpcurrdoor = view.pcurrdoor;
    oldpcurrcube = view.pcurrcube;
    oldpcurrpnt = view.pcurrpnt;
    oldcurredge = view.curredge;
    oldcurrwall = view.currwall;
}
...
void in_plotcoordaxis(int lr) {
    ...
    ep = sp;
    ep.x[0] += size;
    in_plot3dline(lr, &sp, &ep, TCOLORNUM(0, 1), 0, 0);
    ep = sp;
    ep.x[1] += size;
    in_plot3dline(lr, &sp, &ep, TCOLORNUM(0, 2), 0, 0);
    ep = sp;
    ep.x[2] += size;
    in_plot3dline(lr, &sp, &ep, TCOLORNUM(0, 3), 0, 0);
}
...
void plotpnt(struct node *n, int w, int c, int hilight) {
    makeview(0);
    in_plotpnt(0, n, w, c, hilight);
    if (view.whichdisplay) {
        makeview(1);
        in_plotpnt(1, n, w, c, hilight);
    }
}
        ...
        for (i = 0; i < 6; i++)
            if (n->d.c->walls[i] != NULL) {
                for (j = 0; j < 4; j++) {
                    ...
                    else { /* ok, wall is there, neighbour cube is also there. Let's have a
          look if there's a wall in the neighbourcube */
                        ...
                        for (k = 0; k < 3; k++) {
                            ...
                            c1.x[k] = c1.x[k] / 4.0 - c0.x[k];
                            c2.x[k] = c2.x[k] / 4.0 - c0.x[k];
                        }
        ...
        if (n->d.ct->fixed <= 0) {
            ...
            switch (c->b_stdform->d.ls->selected) {
                case 0: make_o_marker(&n->d.ct->x, n->d.ct->coords, size, p);
                    ...
                    plot_pnts = &p[1];
                    break;
                case 1: case 2: makemarker(&n->d.ct->x, plot_pnts = p);
                    num_pts = 8;
                    break;
            }
...
void move_render(struct point *wanted_e0, struct point *cur_e0) {
    ...
    for (w = 0; w < 6; w++) for (j = 0; j < 2; j++)
            if ((p = l->rendercube->d.c->polygons[w * 2 + j]) != NULL) {
                ...
                for (pp = 2; pp < p->num_pnts; pp++) {
                    b = c;
                    for (i = 0; i < 3; i++) c.x[i] = p->pnts[pp].p_3d->x[i] - a.x[i];
                    VECTOR(&n, &b, &c);
                    det = -SCALAR(&n, &m);
                    if (det > 0.0) {
                        det = 1.0 / det;
    ...
    if (min_w >= 0) /* there is a collision with a wall during this step */ {
        if (l->rendercube->d.c->nc[min_w] != NULL) /* a neighbour */ {
            ...
            move_render(wanted_e0, cur_e0);
            /*start again from the new cube*/
