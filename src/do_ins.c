...
void dec_newlevel(int ec) {
    ...
    l = old_l;
    ld->currwall = 3;
    ld->pcurrwall = ld->pcurrcube->d.c->walls[ld->currwall];
        ...
        case tt_wall:
            ...
            if (view.pcurrcube->d.c->nc[view.currwall] != NULL) {
                ...
                if (view.pcurrcube->d.c->d[view.currwall] != NULL) {
                    printmsg(TXT_DOORINTHEWAY, view.currwall, view.pcurrcube->no);
                    return;
                }
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_cube].tail->prev; n != NULL; n = n->prev) {
                    ...
                    if (l->cubes.size > 1) {
                        l->levelsaved = l->levelillum = 0;
                        deletecube(NULL, NULL, n->next->d.n);
                    }
                }
                ...
                if (l->cubes.size > 1) {
                    ...
                    deletecube(NULL, NULL, view.pcurrcube);
                    l->levelsaved = l->levelillum = 0;
        ...
        case tt_wall:
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_wall].head->next; n != NULL; n = n->next)
                    if (n->prev->d.n->d.c->nc[n->prev->no % 6] == NULL)
                        if (connectsides(n->prev->d.n, n->prev->no % 6) > 0) {
                            untag(tt_wall, n->prev->d.n, n->prev->no % 6);
                            l->levelsaved = 0;
                        }
                ...
                printmsg(TXT_CUBEALREADYCONN, view.currwall, view.pcurrcube->no);
                return;
            }
            l->levelsaved = l->levelillum = 0;
            drawopt(in_wall);
            ...
            if (ec == ec_deletefast) {
                ...
                if (tn->d.t->nc)
                    for (n2 = tn->d.t->nc->d.c->things.head->next; n2 != NULL; n2 = n2->next)
                        if (n2->prev->d.t == tn->d.t)
                            freenode(&tn->d.t->nc->d.c->things, n2->prev, NULL);
                freenode(&l->things, view.pcurrthing, free);
                view.pcurrthing = n;
                l->levelsaved = 0;
            ...
            if (ec == ec_deletefast) {
                for (n = l->tagged[tt_door].tail->prev; n != NULL; n = n->prev) {
                    if (n->next->d.n->d.d->d && n->next->d.n->d.d->d->d.d->tagged == n)
                        n = n->prev;
                    deletedoor(n->next->d.n);
                    l->levelsaved = l->levelillum = 0;
                }
            } else if (view.pcurrdoor) {
                deletedoor(view.pcurrdoor);
                l->levelsaved = l->levelillum = 0;
            }
            plotlevel();
            drawopt(in_door);
...
void dec_deletespecial(int ec) {
    ...
    switch (view.currmode) {
        case tt_cube:
            ...
            oldasktagged = view.asktagged;
            view.asktagged = 0;
            view.currmode = tt_thing;
            dec_delete(ec_deletefast);
            view.currmode = tt_door;
            dec_delete(ec_deletefast);
            view.currmode = tt_cube;
            dec_delete(ec_deletefast);
            view.currmode = oldmode;
            view.asktagged = oldasktagged;
            break;
        case tt_wall:
            if (l->tagged[tt_wall].size != 1) {
                printmsg(TXT_SDELTAGONEWALL);
                return;
            }
...
void dec_splitcube(int ec) {
    ...
    for (j = 0; j < 6; j++) if (cube->d[j]) {
            printmsg(TXT_SPLITDOOR, view.pcurrcube->no, j);
            return;
        }
    view.drawwhat |= DW_ALLLINES;
    for (j = 0; j < 3; j++) {
