...
void in_plotcube(int w,struct node *c,int hilight,int drawswitches,int xorval,
 int withalllines,int withlockedsides);
void in_plotpnt(int w,struct node *c,int wn,int pn,int hilight);
void in_plotdoor(int w,struct node *d,int hilight,int drawswitched,int xorval);
void in_plot3dline(int w,struct point *sp,struct point *ep,int color,int xorval,
 int checkdist);
