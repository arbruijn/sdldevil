...
int (*tag_something[tt_number])(struct node *n, va_list arg) ={tagcube, tagwall, tagedge, tagpnt, tagthing, tagdoor};
...
void untagall(enum tagtypes tt) {
    ...
    do {
        ...
        switch (tt) {
            case tt_cube: case tt_pnt: case tt_door: case tt_thing:
                untag(tt, n->d.n);
                break;
            case tt_wall: untag(tt, n->d.n, n->no % 6);
                break;
            case tt_edge: untag(tt, n->d.n, (n->no % 24) / 4, (n->no % 24) % 4);
                break;
    ...
    /* n->next accessing already freed memory? further investigation required
...
int testtag(enum tagtypes tt, struct node *n, ...) {
    ...
    va_start(args, n);
    switch (tt) {
        case tt_cube: ret = n->d.c->tagged != NULL;
            break;
        case tt_wall: ret = n->d.c->tagged_walls[va_arg(args, int)] != NULL;
            break;
        case tt_edge: ret = va_arg(args, int);
            ret = n->d.c->walls[ret] != NULL &&
                    n->d.c->walls[ret]->tagged[va_arg(args, int)] != NULL;
            break;
        case tt_pnt: ret = n->d.lp->tagged != NULL;
            break;
        case tt_thing: ret = n->d.t->tagged != NULL;
            break;
        case tt_door: ret = n->d.d->tagged != NULL;
            break;
        default: waitmsg("Unknown tagmode %d in testtag.", tt);
            ret = 0;
    }
    va_end(args);
    return ret;
...
void switch_tag(enum infos what, struct node *n, ...) {
    ...
    switch (what) {
        case tt_cube: case tt_thing: case tt_door: case tt_pnt:
            if (testtag(what, n)) untag(what, n);
            else tag(what, n);
            break;
        case tt_wall:
            w = va_arg(args, int);
            if (testtag(what, n, w)) untag(what, n, w);
            else tag(what, n, w);
            break;
        case tt_edge:
            w = va_arg(args, int);
            e = va_arg(args, int);
            if (testtag(what, n, w, e)) untag(what, n, w, e);
            else tag(what, n, w, e);
            break;
        default: my_assert(0);
    }
        ...
        case tt_cube: case tt_door:
            for (n = i->tagnr == tt_cube ? l->cubes.head : l->doors.head, found = tagged = 0;
                ...
                if (comp_tagfilter(i, getdata(i->infonr, n), data)) {
                    tagged += testtag(i->tagnr, n) ? -op : !op;
                    found++;
                    if (op) untag(i->tagnr, n);
                    else tag(i->tagnr, n);
                }
            drawopt(i->infonr);
            break;
                ...
                for (w = 0; w < 6; w++)
                    if (n->d.c->walls[w])
                        if (comp_tagfilter(i, getdata(i->infonr, n, w), data)) {
                            tagged += testtag(i->tagnr, n, w) ? -op : !op;
                            found++;
                            if (op) untag(i->tagnr, n, w);
                            else tag(tt_wall, n, w);
                        }
            drawopt(in_wall);
            break;
        case tt_pnt: case tt_edge: found = tagged = 0;
            break;
...
void mousetagbox(struct leveldata *ld, struct w_event *we, int op) {
    ...
    if (ld->whichdisplay) {
        ...
        xs = w_xwininsize(we->w);
        xo = 0;
    }
    ys = w_ywininsize(we->w);
    yo = 0;
    ws_drawbox(sx, sy, 0, 0, view.color[WHITE], 1);
    ox = w_xscreencoord(we->w, sx) - xo;
    oy = w_yscreencoord(we->w, sy) - yo;
    do {
        ws_getevent(&ws, 0);
        x = w_xscreencoord(we->w, ws.x);
        y = w_yscreencoord(we->w, ws.y);
...
int tagflatsides(struct node *cube, int wall) {
    ...
    for (tw = l->tagged[tt_wall].head; tw->next != NULL; tw = tw->next) {
        ...
        for (wp1 = 0; wp1 < 4; wp1++) {
            ...
            else { /* the next wall lies in the neighbour cube */
