...
void add_clickhit(struct clickhit **nearest, struct clickhit *ch) {
    ...
    for (i = 0; i < MAX_CLICKHITS; i++)
        if (!nearest[i] || nearest[i]->d > ch->d) {
            ...
            for (j = MAX_CLICKHITS - 1; j > i; j--) nearest[j] = nearest[j - 1];
            nearest[i] = ch;
            break;
        }
}
...
void start_scansequence(struct w_window *w, int wx) {
    yoffset = w_ywininsize(w) / 2;
    ysize = w_ywininsize(w);
    if (!l->whichdisplay) {
        xoffset = w_xwininsize(w) / 2;
        xsize = w_xwininsize(w);
        makeview(lr = 0);
    } else {
        xsize = w_xwininsize(w) / 2;
        if (wx > w_xwininsize(w) / 2) {
            xoffset = 3 * w_xwininsize(w) / 4;
            makeview(lr = 1);
        } else {
            xoffset = w_xwininsize(w) / 4;
            makeview(lr = 0);
        }
    }
...
void click_in_level(struct w_window *w, struct w_event *we) {
    ...
    do {
        ...
        if ((ws.kbstat & ws_ks_ctrl) == 0 && t == 1 && nearest[0] &&
            ...
            switch (view.currmode) {
                case tt_cube: case tt_thing: case tt_door: case tt_pnt:
                    switch_tag(view.currmode, nearest[0]->data);
                    break;
                case tt_wall: switch_tag(tt_wall, nearest[0]->data, nearest[0]->wall);
                    break;
                case tt_edge: switch_tag(tt_edge, nearest[0]->data,
                            (nearest[0]->wall - 100) / 4, (nearest[0]->wall - 100) % 4);
                    break;
                default: my_assert(0);
            }
        ...
        if (ws.buttons != ws_bt_none) {
            wx = w_xscreencoord(w, ws.x);
            wy = w_yscreencoord(w, ws.y);
            if (wx < (lr ? xsize : 0)) wx = (lr ? xsize : 0);
            else if (wx > (lr ? xsize * 2 : xsize)) wx = (lr ? xsize * 2 : xsize) - 1;
            if (wy < 0) wy = 0;
            else if (wy > ysize) wy = ysize - 1;
            x = w_xwinincoord(w, wx);
            y = w_ywinincoord(w, wy);
    ...
    switch (view.currmode) {
        ...
        case tt_edge:
            view.pcurrcube = nearest[0]->data;
            view.currwall = (nearest[0]->wall - 100) / 4;
