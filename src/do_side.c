...
int dsc_thing_type(struct infoitem *i, void *d, struct node *n, int wallno,
    ...
    if (*no != n->d.t->type1) {
        ...
        if (n->d.t == NULL) {
            printmsg(TXT_CANTMAKETHING);
            freenode(&l->things, n, NULL);
        }
...
int changedoortype(struct node *n, int no, int tagged) {
    ...
    /* FFE: set default parameters for door when changing from other type.
     * wrong dooranim crashes devil texture list and descent itself
     */
    if (no == door1_normal) {
        if (n->d.d->animtxt == 255) {
            n->d.d->animtxt = stdncdoor.animtxt;
            n->d.d->w->texture2 = pig.anims[stdncdoor.animtxt]->rdlno;
            n->d.d->w->texture1 = 0;
        }
    }
    ...
    if (no == door1_onlyswitch) /* only switch: set texture to empty/nothing */ { /* is not required because DESCENT ignores it anyway */
        ...
        case sdtype_cube:
            for (sn = l->tagged[tt_cube].head; sn->next != NULL; sn = sn->next) {
                if (sn->d.n->d.c->type != 4) {
                    if (!pntno) printmsg(TXT_TAGGEDCUBENOPROD, sn->d.n->no);
                    continue;
                }
                if (sd->num >= 10) {
                    if (!pntno) printmsg(TXT_TOOMUCHCUBESTAGGED);
                    break;
                }
        ...
        case sdtype_door:
            for (sn = l->tagged[tt_door].head; sn->next != NULL; sn = sn->next) {
                ...
                if ((no == switch_openwall || no == switch_closewall || no == switch_illusion_off
                    ...
                    printmsg(TXT_TAGGEDWALLNOSWITCHILLWALL, sn->d.n->no);
                    continue;
                }
                if (sd->num >= 10) {
                    printmsg(TXT_TOOMUCHDOORSTAGGED);
                    break;
                }
...
int dsc_flythrough(struct infoitem *i, void *d, struct node *n, int wallno,
    ...
    if (nft > 1) {
        view.whichdisplay = 0;
        if (l) l->whichdisplay = view.whichdisplay;
...
int (*dsc_sideeffect[sc_number])(struct infoitem *i, void *d,
        struct node *n, int wallno, int pntno, int tagged) ={dsc_cubetype, dsc_set_avg_cubelight, dsc_set_t2_dir, dsc_set_avg_sidelight,
