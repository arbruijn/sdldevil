...
int getavgcubelight(unsigned long *no)
{
	...
	if (n->d.c->walls[i] != NULL)
	    for (j = 0, div += 4; j < 4; j++)
		*no += n->d.c->walls[i]->corners[j].light;
    if (div > 0)
	*no /= div;
    else
	*no = 0;
...
int getavgsidelight(unsigned long *no)
{
    ...
    struct node *n;
    n = view.pcurrcube;
    wallno = view.currwall;
    if (n == NULL || n->d.c->walls[wallno] == NULL)
	return 0;
    for (j = 0, *no = 0; j < 4; j++)
	*no += n->d.c->walls[wallno]->corners[j].light;
    *no /= 4;
    return 1;
}
...
void changedata(struct infoitem *i, int withtagged, void *data)
{
    ...
    if (i->sidefuncnr == -2)
	return;
    if (i->tagnr < in_internal)
	l->levelsaved = 0;
    /* first the tagged objects. */
    if (withtagged && i->tagnr < in_internal) {
	for (n = l->tagged[i->tagnr].head, no_obj = 0; n->next != NULL;
	     n = n->next) {
	    switch (i->infonr) {
	    case ds_wall:
		...
		break;
	    case ds_door:
		if (view.pcurrdoor && (n->d.n->d.d->type1 == view.pcurrdoor->d.d->type1 || i->sidefuncnr == sc_walltype || i->sidefuncnr == sc_switch) && view.pcurrdoor != n->d.n) {
		    if (i->sidefuncnr >= 0 && i->sidefuncnr < sc_number)
...
void left_light_entered(struct w_button *b)
{
    light_entered(b, MWT(b, 0));
}

void right_light_entered(struct w_button *b)
{
    light_entered(b, MWT(b, 1));
}
		   ...
		   *fstr, int saveint, float lowerlimit, float upperlimit)
{
	...
	tagfilter(i, withtagged - 2,
		  saveint ? (void *) &no : (void *) &dno);
}
...
{
    float_entered(b, MWT(b, 0), 1.0, "%10.0f", 1, -16.0 * 32 * 64,
		  16.0 * 32 * 64);
}

void r_uvcoord_entered(struct w_button *b)
{
    float_entered(b, MWT(b, 1), 1.0, "%10.0f", 1, -16.0 * 32 * 64,
		  16.0 * 32 * 64);
}
...
void int_entered(struct w_button *b, int withtagged)
{
    ...
    int newno = 0, no = 0;
    getno(i, &no, NULL);
    if (sscanf(b->d.str->str, "%i", &newno) == 1)
	no = newno;
...
void left_int_entered(struct w_button *b)
{
    int_entered(b, MWT(b, 0));
}

void right_int_entered(struct w_button *b)
{
    int_entered(b, MWT(b, 1));
}
...
void fl_mask(struct w_button *b, int withtagged)
{
    ...
    b->d.str->offset = 0;
    if (withtagged < 2)
	changedata(i, withtagged, &new_mask);
    else
	tagfilter(i, withtagged - 2, &new_mask);
}

void l_fl_mask(struct w_button *b)
{
    fl_mask(b, MWT(b, 0));
}

void r_fl_mask(struct w_button *b)
{
    fl_mask(b, MWT(b, 1));
}
...
void left_select_button(struct w_button *b)
{
    select_button(b, MWT(b, 0));
}

void right_select_button(struct w_button *b)
{
    select_button(b, MWT(b, 1));
}
...
void tag_button(struct w_button *b, int withtagged)
{
    ...
    if (withtagged < 2)
	changedata(i, withtagged, data);
    else
	tagfilter(i, withtagged - 2, data);
    FREE(data);
}

void left_tag_button(struct w_button *b)
{
    tag_button(b, MWT(b, 0));
}

void right_tag_button(struct w_button *b)
{
    tag_button(b, MWT(b, 1));
}
...
void b_l_txt_char(struct w_button *b)
{
    if (MWT(b, 0) < 2)
	b_chartexture(b, 0);
}

void b_r_txt_char(struct w_button *b)
{
    if (MWT(b, 0) < 2)
	b_chartexture(b, 1);
}

void b_l_txt_string(struct w_button *b)
{
    b_stringtexture(b, MWT(b, 0));
}

void b_r_txt_string(struct w_button *b)
{
    b_stringtexture(b, MWT(b, 1));
}

void b_l_fitbitmap(struct w_button *b)
{
    b_fitbitmap(b, 0);
}

void b_r_fitbitmap(struct w_button *b)
{
    b_fitbitmap(b, 1);
}
...
int makeoptbuttons(struct w_window *w, int num, struct infoitem *is, int y)
{
	...
	case it_sidelight:
	case it_deltalight:
	    activate = i->type == it_light
		|| i->type == it_deltalight ? getno(i, &no,
						    NULL) : (i->type ==
							     it_cubelight ?
							     getavgcubelight
							     (&no) :
	    ...
	    checkmem(b_string->str =
		     MALLOC(sizeof(char) * (b_string->max_length + 1)));
	    sprintf(b_string->str, "%6.2f%%",
		    i->type == it_deltalight ? no / 0.32 : no / 327.67);
	    b_string->allowed_char = isfloat;
	    b_string->l_char_entered = b_string->r_char_entered = NULL;
	    b_string->l_string_entered =
		i->type ==
		it_deltalight ? left_dlight_entered : left_light_entered;
	    b_string->r_string_entered =
		i->type ==
		it_deltalight ? right_dlight_entered : right_light_entered;
	    checkmem(i->b =
		     w_addstdbutton(w, w_b_string, 0, y, w_xwininsize(w),
				    -1, i->txt, b_string, 0));
	    break;
	case it_coord:
	case it_size:
	...
	case it_fl_delay:
	case it_uvcoord:
	    activate = getno(i, i->type == it_size
			     || i->type == it_fl_delay
			     || i->type ==
			     it_uvcoord ? (void *) &sno : (void *) &dno,
			     NULL);
	    checkmem(b_string = MALLOC(sizeof(struct w_b_string)));
	...
	case it_float:
	    ...
	    checkmem(b_string->str = MALLOC(sizeof(char) * 11));
	    sprintf(b_string->str, "%6.2f",
		    (i->type == it_degree ? dno / M_PI * 180 : dno));
	    b_string->allowed_char = isfloat;
	    b_string->l_char_entered = b_string->r_char_entered = NULL;
	    b_string->l_string_entered =
		i->type ==
		it_degree ? left_degrees_entered : left_float_entered;
	    b_string->r_string_entered =
		i->type ==
		it_degree ? right_degrees_entered : right_float_entered;
	    checkmem(i->b =
		     w_addstdbutton(w, w_b_string, 0, y, w_xwininsize(w),
				    -1, i->txt, b_string, 0));
	    break;
	...
	case it_fl_mask:
	    activate =
		getno(i, i->type == it_int ? (void *) &sno : (void *) &no,
		      NULL);
	    checkmem(b_string = MALLOC(sizeof(struct w_b_string)));
	    b_string->max_length = i->type == it_fl_mask ? 32 : 10;
	    checkmem(b_string->str =
		     MALLOC(sizeof(char) * (b_string->max_length + 1)));
	    if (i->type == it_fl_mask)
		fl_printmask(b_string->str, no);
	    else
		sprintf(b_string->str, "%ld", sno);
	    b_string->allowed_char =
		i->type == it_fl_mask ? isbinary : isxdigit;
	    b_string->l_char_entered = b_string->r_char_entered = NULL;
	...
	case it_thingtexture:
		...
		b_press->delay = 0;
		b_press->repeat = 0;
		b_press->l_pressed_routine =
		    i->type == it_dooranim ? b_l_playdooranim : NULL;
		b_press->r_pressed_routine = NULL;
		b_press->l_routine =
		    i->type == it_texture ? b_l_fitbitmap : (i->type ==
							     it_dooranim ?
							     b_l_enddooranim
							     : NULL);
		b_press->r_routine =
		    i->type == it_texture ? b_r_fitbitmap : NULL;
		checkmem(i->b =
			 w_addimagebutton(w, w_b_press, 0, y, -1, -1,
					  i->type ==
					  it_texture ? pig.txt_bm : (i->
								     type
								     ==
								     it_thingtexture
								     ? pig.
		...
		if (i->multifuncnr > 5)
		    k += (w_xwininsize(w) - x) / 2;
		checkmem(b_press = MALLOC(sizeof(struct w_b_press)));
		b_press->delay = 25;
		b_press->repeat = (i->multifuncnr == 2
				   || i->multifuncnr == 3
				   || i->multifuncnr == 7
				   || i->multifuncnr == 8 ? 10 : -1);
		b_press->l_pressed_routine = b_press->r_pressed_routine =
		    b_press->l_routine = b_press->r_routine = NULL;
		if ((i->multifuncnr - 1) % 5 == 1 || (i->multifuncnr - 1) % 5 == 2) {	/* up or down */
		    b_press->l_routine = b_press->l_pressed_routine =
			i->type ==
			it_dooranim ? b_l_change_dooranim : b_l_change_txt;
		    b_press->r_routine = b_press->r_pressed_routine =
			i->type ==
			it_dooranim ? b_r_change_dooranim : b_r_change_txt;
		} else {
		    b_press->l_routine =
			i->type ==
			it_dooranim ? b_l_change_dooranim : b_l_change_txt;
		...
		if (no < pig.num_rdltxts && pig.rdl_txts[no].pig != NULL)
		    sprintf(b_string->str, "%s (%lu)",
			    (no == 0
			     && i->offset ==
			     2) ? TXT_NOTHING : pig.rdl_txts[no].pig->name,
			    no);
		else
		...
		checkmem(i->b =
			 w_addstdbutton(w, w_b_string, 0, y,
					w_xwininsize(w), -1, i->txt,
					b_string, 0));
		break;
	    default:
		my_assert(0);
	...
	if (!activate)
	    w_deactivatebutton(i->b);
	if (i->sidefuncnr == -2)
	    w_dontchangebutton(i->b);
	if (i->b != NULL) {
	    i->b->data = i;
	    y += i->b->ysize;
	...
	for (k = 0, cy = 0; k < i->numchildren; k++)
	    if ((n =
		 makeoptbuttons(w, i->numinchildren[k], i->children[k],
				y)) > cy)
		cy = n;
	if (cy > maxy)
	    maxy = cy;
...
void b_clickoptwin(struct w_window *w, void *d, struct w_event *we)
{
    ...
    if (i == NULL || w == NULL)
	return;
    if (i->tagnr < tt_number && view.currmode != i->tagnr)
	changecurrmode(i->tagnr);
}

struct w_window options_win =
    { 0, 481, 140, 150, 300, 150, 300, 0, NULL, 0, "",
    wb_drag | wb_shrink | wb_close, wr_routine, NULL, b_refreshoptwin,
	NULL,
    b_clickoptwin
};
...
void makeoptwindow(enum infos what)
{
    ...
    if (what < in_internal)
	y = init_prev_no_next(what);
    else
	y = 0;
    y = makeoptbuttons(optionwins[what], init.infonum[what],
		       init.info[what], y);
    if (y > 0)
	w_resizeinwin(optionwins[what], w_xwininsize(optionwins[what]), y);
...
void drawoptbuttons(struct infoitem *i)
{
	...
	if ((activate = i->type == it_light ? getno(i, &no, NULL) :
	     (i->type ==
	      it_cubelight ? getavgcubelight(&no) : getavgsidelight(&no)))
	    != 0)
    ...
    case it_float:
	if ((activate = getno(i, &dno, NULL)) != 0)
	    sprintf(i->b->d.str->str, "%6.2f", dno);
	break;
    case it_int:
	if ((activate = getno(i, &sno, NULL)) != 0)
	    sprintf(i->b->d.str->str, "%ld", sno);
	break;
    case it_deltalight:
	if ((activate = getno(i, &no, NULL)) != 0)
	...
	if ((activate = getno(i, &no, NULL)) != 0) {
	    i->b->d.str->offset = 0;
	    fl_printmask(i->b->d.str->str, no);
	} else
	    sprintf(i->b->d.str->str, "No data");
	break;
    case it_fl_delay:
	...
	if ((i->multifuncnr == 5 || i->multifuncnr == 10) && activate)
		...
		sprintf(i->b->d.str->str, "%s (%lu)",
			(no == 0
			 && i->multifuncnr ==
			 10) ? " Nothing" : pig.rdl_txts[no].pig->name,
			no);
	    else
...
//		    memset(pig.txt_buffer, 0xff, 64 * 64);
	...
	/*
		    readbitmap(pig.txt_buffer, NULL,
			       &pig.rdl_txts[w->texture1], 0);
		*/
		if (w->texture2 > 0 && w->texture2 < pig.num_rdltxts && pig.rdl_txts[w->texture2].pig)
		    readbitmap(tmptxtdata, NULL,
			       &pig.rdl_txts[w->texture2],
			       w->txt2_direction);
		drawbmlines = 1;
		ws_translatepixels(tmptxtdata, pig.txt_buffer, 64*64);
	    }
	} else if (i->multifuncnr == 4 || i->multifuncnr == 8)
	    refresh_txtlist(i);
	break;
    case it_dooranim:
	activate = getno(i, &no, NULL);
	if (i->multifuncnr == 0) {
		memset(tmptxtdata, 0x00, 64 * 64);
		init_txt_buffer ((uint32_t *)pig.door_buffer, 64*64, 255, 255, 255);
		w = view.pcurrdoor == NULL ? NULL : view.pcurrdoor->d.d->w;
		if (activate && w != NULL && no >= 0 && no < pig.num_anims && pig.anims[no] != NULL)
			drawdooranim(NULL, pig.anims[no], 0, w->texture1, w->texture2, w->txt2_direction);
	}
	break;
    case it_thingtexture:
	activate = getno(i, &sno, NULL);
	if (i->multifuncnr == 5 && activate) {
		if (sno == -1 || (sno >= 0 && sno < pig.num_rdltxts && pig.rdl_txts[sno].pig != NULL))
			sprintf(i->b->d.str->str, "%s (%ld)", sno == -1 ? " Default" : pig.rdl_txts[sno].pig->name, sno);
		else
			sprintf(i->b->d.str->str, TXT_UNKNOWN, sno);
	}

	if (i->multifuncnr == 0) {
		init_txt_buffer ((uint32_t *)pig.thing_buffer, 64*64, 255, 255, 255);
		memset(tmptxtdata, 0x0, 64 * 64);
		if (activate && sno >= 0 && sno < pig.num_rdltxts && pig.rdl_txts[sno].pig) {
			//readbitmap(pig.thing_buffer, NULL, &pig.rdl_txts[sno], 0);
			readbitmap(tmptxtdata, NULL, &pig.rdl_txts[sno], 0);
			ws_translatepixels(tmptxtdata, pig.thing_buffer, 64*64);
		}	
	}
	break;
    default:
...
void drawopt(enum infos what)
{
	...
	if (what < in_internal) {
		...
		case in_cube:
			...
			checkmem(b_optwins[what][6]->d.ls->options =
				 REALLOC(b_optwins[what][6]->d.ls->options,
					 sizeof(char *) *
					 (view.b_levels->num_options -
					  1)));
			b_optwins[what][6]->d.ls->options[0] = TXT_NONE;
			b_optwins[what][6]->d.ls->selected = 0;
			for (j = 1, k = 1; j < view.b_levels->num_options;
			     j++)
			    if (view.b_levels->selected != j) {
				if (view.pcurrmacro &&
				    strcmp(view.b_levels->options[j],
					   view.pcurrmacro->fullname) == 0)
				    b_optwins[what][6]->d.ls->selected = k;
				b_optwins[what][6]->d.ls->options[k++] =
				    view.b_levels->options[j];
			    }
			b_refreshtagno(what);
		    }
		    break;
		case in_wall:
			...
			i = view.currwall;
			b_optwins[what][3]->d.s->on =
			    testtag(what, view.pcurrcube, view.currwall);
			b_refreshtagno(what);
			if (b_optwins[what][6])
			    b_optwins[what][6]->d.s->on =
				view.pdefcube == view.pcurrcube
				&& view.defwall == view.currwall
				&& view.pdefcube->d.c->walls[view.defwall];
			if (b_optwins[what][8])
			    b_optwins[what][8]->d.s->on =
				view.pcurrcube->d.c->walls[view.
							   currwall] !=
				NULL
				&& view.pcurrcube->d.c->walls[view.
							      currwall]->
			...
			if (b_optwins[what][9])
				...
				adjust_light_nc == view.pcurrcube
				&& adjust_light_wall == view.currwall;
		    }
		    break;
		case in_edge:
		...
		case in_door:
			...
			i = view.pcurrdoor->no;
			b_optwins[what][3]->d.s->on =
			    testtag(tt_door, view.pcurrdoor);
			b_refreshtagno(what);
		    }
		    break;
		case in_thing:
			...
			b_optwins[what][3]->d.s->on =
			    testtag(tt_thing, view.pcurrthing);
			b_refreshtagno(what);
		    }
		    break;
		default:
		    act = 0;
		    waitmsg("Unknown infotype %d in drawopt", what);
		}
	    if (i > 9999)
		sprintf(b_optwins[what][1]->d.str->str, "****");
	    else if (i < 0)
		sprintf(b_optwins[what][1]->d.str->str, "----");
	    else
		sprintf(b_optwins[what][1]->d.str->str, "%.4d", i);
	    for (i = 0; i < NUM_OPTBUTTONS; i++)
		if (b_optwins[what][i]) {
		    if (act)
			w_activatebutton(b_optwins[what][i]);
		    else
			w_deactivatebutton(b_optwins[what][i]);
		}
	    if ((what == in_door || what == in_thing) && l != NULL)
		if (b_optwins[what][4])
		    w_activatebutton(b_optwins[what][4]);
	    if (what == in_wall && act) {
		if (view.pcurrcube->d.c->walls[view.currwall] == NULL) {
		    if (b_optwins[what][6])
			w_deactivatebutton(b_optwins[what][6]);
		    if (b_optwins[what][8])
			w_deactivatebutton(b_optwins[what][8]);
		}
		if (b_optwins[what][9])
		    if (view.pcurrcube->d.c->walls[view.currwall] == NULL
			|| view.pcurrcube->d.c->walls[view.currwall]->ls ==
			NULL)
	...
	for (i = 0; i < init.infonum[what]; i++)
	    drawoptbuttons(&init.info[what][i]);
	w_refreshend(optionwins[what]);
    }
    if (what == in_cube || what == in_wall)
	fb_refreshall();
}
...
{
    enum infos what;
    for (what = 0; what < in_number; what++)
	drawopt(what);
}
