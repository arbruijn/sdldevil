...
int checkforline(struct point *p1, struct point *p2, int num_s,
    ...
    for (side = num_s - 1; side >= 0; side--) {
        ...
        VECTOR(&t, &r, &s);
        if (!checkcolltriangle(&a, &m, &r, &s, &t)) { /* missed the triangle at all */
            for (i = 0; i < 3; i++) {
...
int illum_checkwall(struct cube *c, int w) {
    ...
    switch (c->d[w]->d.d->type1) {
        ...
        case door1_onlytxt: case door1_shootthrough:
            ...
            break;
        case door1_onlyswitch: return 1;
            break;
    }
    return 0;
}
...
void calcillumwall(struct node *c, int wall, int *light) {
    ...
    d2 = e3x * e4y - e3y*e4x;
    if (d1 == 0 || d2 == 0) {
        waitmsg(TXT_CANTCALCLIGHT, c->no, wall);
        return;
    }
    for (i = 0; i < 3; i++) {
        center.x[i] = 0.0;
                ...
                for (y = 0; y < ILLUM_GRIDSIZE; y++) {
                    ...
                    for (x2 = 0; x2 < ILLUM_SUBGRIDSIZE; x2++)
                        for (y2 = 0; y2 < ILLUM_SUBGRIDSIZE; y2++) {
                            ...
                            sub_a[y2 * ILLUM_SUBGRIDSIZE + x2] = a;
                            sub_b[y2 * ILLUM_SUBGRIDSIZE + x2] = b;
                        }
                    if (init_test & 4)
...
int rec_getnbcorners(struct smoothcorner *sm_corners, int smc_length,
    ...
    sm_corners[smc_length].cube = nc;
    new_smcl = smc_length + 1;
...
void smoothlight(void) {
    ...
    for (nls = l->lightsources.head; nls->next != NULL; nls = nls->next) {
        if ((progress = (nls->no * 100 / l->lightsources.size) / 5 * 5) != last_progress) {
            printmsg(TXT_SMOOTHINGLIGHT, progress);
...
int read_lightsources(void) {
    ...
    if ((f = fopen(init.lightname, "r")) == NULL) {
        waitmsg(TXT_CANTOPENLSFILE, init.lightname);
        return 0;
    }
    if (!findmarker(f, "LightSources", &num)) {
        fclose(f);
        waitmsg(TXT_NOTALSFILE, init.lightname);
        return 0;
    }
    for (i = 0; i < num; i++) {
    ...
    for (ntc = l->tagged[tt_cube].head, per = 0, ldrawn = -10; ntc->next != NULL;
        ...
        for (w = 0; w < 6; w++)
            if (ntc->d.n->d.c->walls[w] != NULL) {
                fl.delay = 0;
                if (ntc->d.n->d.c->walls[w]->ls) { /* an old lightsource. Delete it but keep the fl */
                        ...
                        for (j = 0; j < ILLUM_GRIDSIZE; j++) {
                            switch (ntc->d.n->d.c->walls[w]->txt2_direction) {
                                case 0: y = i;
                                    x = j;
                                    break;
                                case 1: y = ILLUM_GRIDSIZE - 1 - j;
                                    x = i;
                                    break;
                                case 2: y = ILLUM_GRIDSIZE - 1 - i;
                                    x = ILLUM_GRIDSIZE - 1 - j;
                                    break;
                                case 3: y = j;
                                    x = ILLUM_GRIDSIZE - 1 - i;
                                    break;
                                default: y = i;
                                    x = j;
                                    break;
...
void setinnercubelight(void) {
    ...
    for (ntc = l->tagged[tt_cube].head->next; ntc != NULL; ntc = ntc->next) {
        ...
        if (i > 0) {
            ntc->prev->d.n->d.c->light = overall / i;
            untag(tt_cube, ntc->prev->d.n);
        }
    }
        ...
        for (ntc = l->tagged[tt_cube].head->next; ntc != NULL; ntc = ntc->next) {
            ...
            for (w = 0; w < 6; w++)
                if (ntc->prev->d.n->d.c->nc[w] && !ntc->prev->d.n->d.c->nc[w]->d.c->tagged) {
                    overall += ntc->prev->d.n->d.c->nc[w]->d.c->light;
                    i++;
                }
            if (i > 0) {
                ntc->prev->d.n->d.c->light = overall / i;
...
void dec_setcornerlight(int ec) {
    ...
    if (l->tagged[tt_cube].size == 0) {
        printmsg(TXT_NOCUBETAGGED);
        return;
    }
    time1 = clock();
...
void dec_mineillum(int ec) {
    ...
    time1 = clock();
    calccornerlight(isAlwaysSmoothing);
    setinnercubelight();
    l->levelsaved = 0;
    l->levelillum = 1;
    drawopts();
